...
python_bin="$(find_python)" || fail "no working Python interpreter was found (tried python3, python, py); a 'python3' that only prints a Microsoft Store advertisement does not count. Install Python and rerun scripts/init.sh"
