"""Offline Vercel provider pipeline against a real PostgreSQL database."""
...
@skip_without_db
def test_runner_persists_three_official_review_candidates_and_zero_offers(
    ...
    reviews = list(
        session.scalars(
            select(ReviewItem).where(
                ...
                ReviewItem.candidate_facts["service"]
                .as_string()
                .in_(("Vercel Hobby", "Vercel Sandbox", "Vercel Pro Trial")),
            )
        )
    )
    ...
    assert session.scalar(select(func.count()).select_from(Snapshot)) - snapshots_before == 3
    ...
    assert all("requires_card" not in row.candidate_facts for row in candidates)
    assert all("has_paid_dependencies" not in row.candidate_facts for row in candidates)
    assert all(row.admin_disposition == "pending" for row in reviews)
    assert all(
        "schema_complete" in (row.evidence_conflict or {}).get("failed_conditions", [])
        for row in reviews
    )

    vercel_offer_ids = select(Offer.id).join(Service).where(Service.provider_id == provider.id)
    assert (
        session.scalar(
            select(func.count())
            .select_from(Offer)
    ...
    assert (
        session.scalar(
            ...
            .where(OfferVersion.offer_id.in_(vercel_offer_ids))
        )
        == 0
    )
    assert (
...
def test_changed_case_creates_a_draft_change_event(session: Session, config) -> None:
    ...
    source = session.scalar(select(Source).where(Source.slug == "vercel-sandbox-pricing"))
    ...
    reconcile_scan(scan, source, session, now=datetime(2026, 8, 13, 20, 0, tzinfo=UTC))
    candidate_ids = list(
...
_PIPELINE_OFFERS: tuple[Mapping[str, object], ...] = (
    {
        ...
        "quotas": [{"metric": "sandbox_active_cpu", "exhaustion_behaviour": "hard_stop"}],
    },
    {
        "service": "Vercel Pro Trial",
        "offer_type": "trial",
        "requires_card": None,
        "has_paid_dependencies": None,
        "quotas": [{"metric": "active_cpu", "exhaustion_behaviour": "hard_stop"}],
    },
)
...
def _pipeline_source(session: Session) -> Source:
    source = Source(
        ...
        official=True,
        endpoint="https://vercel.com/docs/f008-pipeline-case",
        enabled=True,
        schedule="daily",
    )
    session.add(source)
