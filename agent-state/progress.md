...
## 2026-08-14 - Builder - F001 follow-up: the secrets route guard now constrains EFFECT, not presence

- **Defect confirmed before building on it [M]:** `test_every_route_invokes_the_validator` was `"check_secrets_baseline" in text`. Measured on base `8d525db` by driving the real test against real working-tree mutations, four route files x three cases: untouched PASSED (positive control, so the guard was narrow rather than vacuous), DELETING the invocation went RED in all four, COMMENTING IT OUT stayed GREEN in all four. The brief's premise is confirmed exactly. Commenting out is the likelier bypass of the two, so the guard was blind to the mode it most needed to catch.
- **Designed from the property, not from the bypass I was handed [M]:** the check does not reject a leading `#`. It extracts each file's EXECUTABLE content in that file's own language and then looks for the invocation. New `tests/support/source_scan.py` (stdlib only, no YAML parser, so no environment can break it open or shut) scans POSIX shell, PowerShell and YAML, and knows that a workflow `run:` block scalar is YAML data but shell CODE - which is the only reason the CI route is checkable at all.
- **Result, same twelve cases, predictions stated first and all four held [M]:** untouched PASSES in all four, deleted is RED in all four, commented-out is now RED in all four. Every mutated file restored byte-exactly, proved by `git hash-object` against `git rev-parse <base>:<path>`, never `numstat`.
- **Present-but-inert modes, split into caught and knowingly NOT caught [M]:** the list is in the test module so its limits cannot be discovered later. CAUGHT: line-comment in all three syntaxes; PowerShell `<# #>` block comments including nested; deletion; a deleted route FILE (clear message, not a traceback); demotion out of an executed field (a workflow mention must sit in `run:`, a pre-commit one in `entry:`/`args:`, so a `name:` mention is not a wiring); here-document and PowerShell here-string bodies, which are data; and an UNCONDITIONAL TOP-LEVEL early `exit`/`return` in the same executed body. NOT CAUGHT, deliberately: an always-false guard (`if false; then`), function shadowing, a CONDITIONAL or nested early exit, an invocation appearing only inside a display string, and anything disabling the machinery from outside these four files (`pre-commit uninstall`, a job-level `if:`). The last three need evaluation rather than parsing.
- **String bodies are KEPT on purpose [M]:** blanking them would catch `echo "... check_secrets_baseline ..."`, but it would also fire on `& $python "scripts/check_secrets_baseline.py"`, and quoting an interpreter path is idiomatic. That would be a false positive on correct work, which teaches people to delete the guard. The narrower loss was the better trade.
- **A naive stripper would have been a false positive on real code [M]:** `line.split("#")[0]` truncates `"${#FAILURES[@]}"` in `scripts/check.sh` and `"${#files[@]}"` in `.github/workflows/ci.yml`. Both are pinned as regression tests. That is why the scanner tracks quoting and shell word boundaries rather than splitting on a character.
- **Guard against the guard, borrowed from `tests/unit/test_no_live_fetcher_in_tests.py` [M]:** the vacuity risk here has a direction. Blanking too much makes the guard RED and is self-announcing; the dangerous direction is a silent fallback to returning a file unchanged, which would restore substring semantics invisibly. So `scan()` RAISES `UnknownLanguage` for an unrecognised suffix, and `test_the_route_scan_is_not_vacuous` proves per file that the scan ran and removed content. No exemption list exists in this design, so there is nothing to justify.
- **Wiring deliberately untouched [M]:** `scripts/check_secrets_baseline.py`, `scripts/refresh_secrets_baseline.py` and all four route files are byte-identical to base, verified by blob hash. This slice guards the wiring; it is not the wiring. Nothing suggested the wiring is defective.
- **Gates [M]:** Ruff lint clean, Ruff format clean across 211 files, full suite 1638 passed / 213 skipped. Collected tests 1826 -> 1851, measured by collecting the base commit in a throwaway worktree rather than inferred: exactly +25, none lost. `scripts/check_secrets_baseline.py` passes; `.secrets.baseline` byte-identical and NOT refreshed, since no tracked file's digests moved.
- **Three things I got wrong, recorded [M]:** (1) my first probe used a PowerShell `@"..."@` here-string, which interpolated `${#FAILURES[@]}` and died with a Python SyntaxError - the literal `@'...'@` form is required for any code containing `$` or a backtick; (2) both files I created were written CRLF by the Windows tooling and had to be normalised to LF before commit, which `git status` warned about rather than my own checks; (3) my blob-hash verification loop reported `apps/api/app/ingest/adapters/profiles/gcp.py` as CHANGED when it exists in NEITHER base nor working tree - a null-versus-empty comparison bug in the throwaway loop, not a real change. The concurrent Google Cloud slice's paths are untouched.
- **Protected state [M]:** `agent-state/feature_list.json` byte-identical, no flag flipped, F008 stays `passes:false`. Zero diff to `apps/web/**` (`package-lock.json` still blob `f492053a91df9a977f6fda838ee82c6c38f5eda7`), `docker-compose.yml`, `apps/api/**`, `.github/**`, `scripts/url-allowlist.txt`, `docs/PROVIDER_ADAPTERS.md` and every Google Cloud path. Unfiltered `git diff` run FIRST as a positive control: three files total.
- **Files changed:** `tests/support/source_scan.py` (new), `tests/unit/test_secrets_baseline.py`, `agent-state/current_contract.json`, and this handoff.
- **Rebased onto `80c1684` (PR #68, Google Cloud provider) after the measurements above [M]:** `main` moved while this was building. `progress.md` and `current_contract.json` conflicted; `.secrets.baseline` did NOT, because this slice never touched it, so #68's growth to 31 tracked files / 107 entries is preserved rather than reverted. `progress.md` resolved by taking main's blob VERBATIM (`d970247e`, verified by hash before appending) and re-appending this block. The route guard was then re-measured against the new base as a test of the guard itself: a slice that added a whole provider is exactly the kind of change that would expose a brittle matcher.
- **Boundary:** Draft PR only. Do not merge, do not flip any ledger flag.

## 2026-08-14 - Builder - F001 follow-up: a lexical proxy for a structural question (three false positives, one cause)

- **Found after the slice was reported complete [M]:** held at `8c5eca9`, three false positives were measured in the guard's own route rules - one by me probing read-only after standing down, two by the evaluator. All three are FALSE POSITIVES ON CORRECT WORK, the class this brief weights above every missed catch, because a guard that reds on a legitimate edit gets deleted rather than fixed.
  - **F:** `.pre-commit-config.yaml` written as `entry: python` plus an `args:` BLOCK sequence was REJECTED, though pre-commit executes it identically to the one-line form.
  - **FP-2:** collapsing the workflow's `run: |` to a single INLINE `run:` was REJECTED. Blast radius measured independently: `.github/workflows/ci.yml` has **2 block-scalar and 16 inline `run:` keys**, so the guard demanded the 2/18 MINORITY spelling while the file's own prevailing convention was the rejected one.
  - **FP-3:** `entry: |` as a block scalar was REJECTED, because `_YAML_SCRIPT_KEYS` marked `entry` content as `shell` while that file's route rule accepted only `yaml` - the two halves of the design contradicting each other about one construct.
- **One cause, and my own diagnostic had been printing it all along [M]:** the FP-2 message read `context=yaml, key=run`. The KEY was right; I was asserting the WRONG PROPERTY. The design conflated two orthogonal things: `context`, a LEXICAL fact about which language's comment rules produced a line, and executed position, a STRUCTURAL fact about whether the text forms the value of a key the tool runs. The YAML route rules asserted the lexical proxy. A proxy fails in BOTH directions - `yaml` where `shell` was demanded, `shell` where `yaml` was demanded - and each direction presents as its own separate bug.
- **This is the slice's own thesis one level down [M]:** the whole point of the slice is that a substring cannot answer a structural question. `context` was the same mistake in the route rule. Fixing it COMPLETES the slice rather than expanding it; shipping the substring fix while leaving a lexical proxy behind would have delivered half the thesis and called it done.
- **Fix: route rules assert OWNERSHIP, computed structurally [M]:** `SourceLine.key` is now the key whose VALUE a line contributes to - inline value, block-scalar body, or sequence item, all resolved the same way from indentation and nesting rather than from a list of spellings observed in these four files. `.github/workflows/ci.yml` requires ownership by `run` (18/18, not 2/18); `.pre-commit-config.yaml` requires `entry` or `args`; the two whole-file scripts keep the language constraint, since every line of them is executed code and no key concept applies.
- **`entry` removed from the shell-script key set - dead AND wrong [M]:** the evaluator established it was unreachable; measuring it showed it was also incorrect. pre-commit shlex-splits `entry` and execs it, with no shell involved, so a `#` there is a literal argument and stripping it as a comment would corrupt the value. Only GitHub Actions `run:` is genuinely a shell script. Pinned by `test_precommit_entry_is_not_shell_interpreted`.
- **Non-script block scalars are retained rather than blanked [M]:** a `description: |` mention is now rejected because it sits under a NON-EXECUTED KEY, not because it was made invisible. Same verdict, honest mechanism - and it is what lets `entry: |` pass without a special case.
- **The controls that stop this being a relaxation [M]:** five negative controls, across BOTH YAML routes, covering `name:` inline, `name: |` as a block scalar, and `description: |` prose. All stay RED while F, FP-2 and FP-3 go green. The coordinator put in writing that if any could not be held red it was to be reported as a finding rather than traded away; none had to be. Module test count 71 -> 85.
- **Re-verified after the re-base [M]:** the twelve-case matrix is UNCHANGED - untouched PASS x4, deleted RED x4, commented-out RED x4, every file restored byte-exact by blob hash. All six restructurings pass against the SHIPPED module.
- **A defect of my own inside the fix, caught by the controls [M]:** making `contexts` optional left `sorted(contexts)` in the failure-message builder, which raised `TypeError` on exactly the five negative controls. They failed LOUDLY at the moment they were meant to prove rejection. Worth recording that the controls earned their keep on their first run, and that a diagnostic path is code that also needs exercising.
- **The generalisable lesson, which outlives this YAML [M]:** when several findings look like separate gaps in a rule, check whether the rule asserts a PROXY for the property it means. A proxy fails in both directions and each direction looks like its own bug. Corollary already recorded here: my confidence was one experiment ahead of my evidence - a positional rule asserted from one observed spelling needs the counter-experiment before it is claimed, and 'the spelling that happens to be in the tree today' is not the property.
- **Evaluation economics, recorded because I got the call wrong [M]:** I leaned toward holding and letting the evaluator find F. The coordinator overrode that, correctly: an evaluation spent confirming a KNOWN defect is one not spent hunting unknown ones. Fix first when the fix is already measured; hold only for defects not yet characterised.
- **Files changed:** `tests/support/source_scan.py`, `tests/unit/test_secrets_baseline.py`, `agent-state/current_contract.json`, and this handoff. The four route files, both secrets scripts and `.secrets.baseline` remain byte-identical to `80c1684`.
- **Boundary:** Draft PR only. Do not merge, do not flip any ledger flag.

## 2026-08-14 - Builder - F001 follow-up: stop hand-rolling a YAML parser; walk the schema, and load it the way the tools do

- **Why a third pass on the same guard [M]:** the route rules were reworked twice inside this slice - lexical `context`, then nearest-key OWNERSHIP - and each rework was followed by a new spelling it could not handle. That pattern is the signature of a PARTIAL PARSER standing in for a real one. Ten distinct defects were measured across those revisions, in two families with one cause: **false negatives** `with: run:` (block and inline), `env: run:`, and a duplicate `entry:` overridden by a later one - all ACCEPTED though none of them executes; **false positives** CRLF files, both flow mappings, three alias shapes and a `<<:` merge - all REJECTED though every one is legitimate; plus **fail-OPEN as a class**, where a `.pre-commit-config.yaml` too broken for PyYAML to load at all was certified as correctly wired.
- **Fixed by composition, not by more rules [M]:** PyYAML resolves STRUCTURE; the existing shell scanner resolves the LEXICAL question of which characters inside a `run:` body are a comment - a question a YAML parser cannot answer, because there a '#' belongs to the shell. The `.sh` and `.ps1` scanners are untouched. No new dependency: `pyyaml==6.0.3` is a declared `[project] dependencies` pin in `pyproject.toml`, kept in step with `apps/api/requirements.txt` by `tests/unit/test_requirements_sync.py`, and CI installs it via `pip install -e ".[dev]"`. Verified rather than assumed - the earlier claim that it came from `apps/api/requirements.txt` would NOT have held, because the CI job installs the project, not that file.
- **Decision 1, WALK the schema rather than SEARCH it [M]:** selection consumes the path one segment at a time - `jobs.*.steps[].run`, `repos[].hooks[].entry|args[]` - so a step's `with:` and `env:` are never visited. Ancestry is enforced by CONSTRUCTION. Nothing in the rule names `with`, deliberately: a rule spelled 'not under `with`' would be fitted to the decoy in front of it, which is the 2/18 mistake in a new hat. A recursive hunt for any key called `run` would have reproduced the nearest-key defect with a parser underneath - harder to see, not easier.
- **Decision 2, `safe_load` rather than `compose` [M]:** the composer preserves line marks but ALSO preserves duplicate keys and does not expand `<<` merges. `safe_load` resolves last-wins and expands merges, which is literally what pre-commit and Actions execute. Measured decisively: with `entry:` written twice, the validator first and an override second, a compose-based rule ACCEPTS a file that runs something else. A verdict computed from anything but the executed semantics is a proxy for the truth - the exact mistake this slice exists to correct.
- **What that trade cost, stated rather than buried [M]:** line numbers. `safe_load` discards marks. The diagnostics now name the schema PATH (`repos.hooks.entry`, `jobs.*.steps.run`) instead, which arguably says more: a path explains WHY a position is not executed, where a line only says where it sits. A wrong line number is an annoyance; a wrong verdict is the failure. The gate that asked for both was withdrawn once measurement showed they conflict.
- **Fail-closed had to be BUILT, not preserved [M]:** `yaml.YAMLError` is caught explicitly and turned into a rejection with an actionable message. Without that the guard raises instead of rejecting. Four unparseable shapes are pinned - tab indentation, unclosed quote, undefined alias, outright garbage - and every one carries the needle at a REAL `entry:` first, with an intact-document control asserted in the same test. A fail-closed probe whose needle is absent rejects for the trivial reason 'absent entirely' and proves nothing.
- **The accept battery is now the definition of not-narrowing [M]:** fifteen legitimate spellings, NONE of which appears in any of the four route files, are permanent parametrized tests - dash-and-key, `run:` after a nested `env:`, four-space indent, matrix job, a step following another step's `with:`, keep-chomped `|-`, explicit indent indicator `|2`, double-quoted inline, both flow mappings, multi-item block-sequence `args:`, single-quoted `entry:`, a second hook after an unrelated first, three alias shapes and a `<<:` merge. A change that turns any of these red has narrowed the guard, whatever else it fixed.
- **Controls held, and that is the whole point [M]:** `name:` inline, `name: |`, `description: |` prose, `with: run:` x2, `env: run:`, `defaults.run.shell`, an alias landing only in an inert key, and the overridden duplicate `entry:` all stay REJECTED. Passing the true cases by relaxing until nothing is rejected is the failure this would have been failed for, and it did not happen. Module tests 85 -> 116; suite 1761 -> 1792; collected 1942 -> 2012.
- **Two of my own test fixtures were malformed, caught by my own assertions [M]:** `a ::: b` is VALID YAML (it parses as the key `a ::`), so a 'broken file' case was not broken at all and the guard correctly accepted it; and `b: &outer *inner` is INVALID YAML, because an alias node cannot carry an anchor, so a 'legitimate' case was not legitimate. Both surfaced as red tests rather than as false conclusions, and both were replaced with measured equivalents. A patch script also refused to apply when a sanity string did not match, rather than writing something wrong quietly.
- **The lesson, which is the same one three levels down [M]:** every revision of this guard asserted a PROXY for the property it meant - a substring for an invocation, a lexical context for an executed position, a key name for a schema position, and a parse tree for executed semantics. Each proxy failed in both directions and each direction looked like its own bug. The question to ask of any guard is not 'does this catch the thing I just saw' but 'is what I am measuring the thing I mean, or something that correlates with it'.
- **Files changed:** `tests/support/source_scan.py`, `tests/unit/test_secrets_baseline.py`, `agent-state/current_contract.json`, and this handoff. The four route files, both secrets scripts and `.secrets.baseline` remain byte-identical to `80c1684`; the shell and PowerShell scanners are unchanged.
- **Boundary:** Draft PR only. Do not merge, do not flip any ledger flag.

## 2026-08-14 - Builder - F001 follow-up: arity - a value of the wrong SHAPE does not execute

- **Found by scoring predictions I had volunteered [M]:** after standing down I offered three falsifiable predictions about shapes the evaluator was going to test. Two held. The third was WRONG, and the miss was a real false negative: `run:` written as a SEQUENCE was ACCEPTED, although GitHub Actions types `run` as a string and a workflow spelling it that way does not load at all. The guard certified a workflow that never runs.
- **Broader than the report, because the cause was a missing schema fact rather than a missing case [M]:** measuring the mirrors found two more, neither of which had been reported - `entry:` as a sequence, and `args:` as a scalar. All three are one omission: the rule recorded whether a value is a SHELL SCRIPT but not whether it is SCALAR- or LIST-valued. `args` is a sequence of strings; `run` and `entry` are single strings. Terminal sequences were therefore iterated unconditionally, which is right for `args` and wrong for the other two.
- **Fixed as arity, not as the one shape [M]:** each path pattern now carries `(path, is_shell_script, is_list_valued)`, and selection distinguishes an INTERMEDIATE sequence - `steps`, `repos`, `hooks`, always traversed - from a TERMINAL one, which is accepted only where the schema says the value is a list. `args: [[x]]` nested one level too deep is rejected too. Five wrong-shape cases and two paired right-shape controls are permanent tests, so arity cannot be silently dropped.
- **Why this was fixed rather than disclosed [M]:** it is the SAME CLASS as the fail-open case fixed one commit earlier - an invalid config certified as correctly wired. That fix was argued on the explicit ground that 'it fails loudly when the tool runs' is NOT sufficient, because the guard's claim is that the check is WIRED, and it is not. Disclosing this while having just fixed the identical class would have been incoherent. It is also a FALSE NEGATIVE, the silent kind, which this slice has consistently ranked above loud failures.
- **The two predictions that held, recorded because they were falsifiable [M]:** `!!str` tags resolve and are accepted while an unrecognised tag raises `ConstructorError`, a `YAMLError` subclass, and is therefore REJECTED by the fail-closed branch rather than crashing. And PyYAML's YAML 1.1 parsing of a bare `on:` key as boolean `True` does not affect any verdict, because no executed path starts at `on`.
- **One documented limit, pinned rather than argued away [M]:** that same YAML 1.1 behaviour makes a mention under a workflow's trigger block report at a path beginning `True.` instead of `on.`. The verdict is correct; only the message reads oddly. It is NOT repairable rather than merely unfixed - the loaded document cannot say whether the source spelled it `on`, `yes` or `true`, and guessing one back would invent information the parser deliberately discarded. `test_a_yaml_11_boolean_key_is_a_known_diagnostics_artefact` asserts the current text and says how to retire the limit if PyYAML's YAML version ever changes.
- **Independent schema corroboration, obtained from outside my code [M]:** the residual risk I had named as structurally invisible to me - whether either tool has an executed position my patterns miss - was closed by the orchestrator reading the schemas rather than the implementation. pre-commit's hook definition carries exactly two command-bearing fields, `entry` and `args`, with every other hook key metadata, and all three repo kinds share the same hook definition. In a GitHub workflow, `jobs.<id>.steps[].run` is the only key that executes a command; `shell:` only selects the interpreter for a `run:` body. So the four patterns are complete for both schemas, with `uses:` correctly out of scope because it executes another file's content. Recorded as strong corroboration rather than proof: the pre-commit schema consulted is community-maintained, and the Actions answer rests on documentation rather than a parsed schema.
- **Verification [M]:** the twelve-case matrix is UNCHANGED; all five wrong-shape cases REJECT and both right-shape controls ACCEPT; every earlier battery still holds. Module tests 116 -> 124; suite 1792 -> 1800; collected 2012 -> 2020. Ruff lint and format clean across 215 files, Prettier 0, ESLint 0, secrets validator 31 files / 107 entries.
- **Files changed:** `tests/support/source_scan.py`, `tests/unit/test_secrets_baseline.py`, `agent-state/current_contract.json`, and this handoff. The four route files, both secrets scripts, `.secrets.baseline` and every manifest remain byte-identical to `80c1684`; the shell and PowerShell scanners are unchanged.
- **Boundary:** Draft PR only. Do not merge, do not flip any ledger flag.

## 2026-08-14 - Builder - F001 follow-up: type the segments, read one document, and say only what was established

- **Four findings, and the must-fix was in the EXPLANATIONS rather than the verdicts [M]:** an independent evaluator confirmed every verdict correct on the previous head, then found that two rejection branches asserted causes they had not shown. The flagship mode was the one that got it wrong: commenting the invocation out of the real `ci.yml` produced `appears at jobs.secrets.steps.[3].run, and none of those is a position this tool executes` - naming a path that MATCHES the pattern the sentence says is unmatched, and never saying 'commented out'. A maintainer reading that concludes the rule is broken, and a rule that looks broken gets deleted. Same defect class as the coverage overclaim that failed an earlier head: there the guard claimed coverage it lacked, here a reason it had not established.
- **Fixed by establishing each reason from data already computed [M]:** needle in an executed `run:` value but gone after shell-stripping => COMMENTED OUT inside the script, named as such. Needle in raw text but absent from the loaded document => a YAML comment or a duplicate-key override, and `compose` separates those because it preserves duplicates while `safe_load` resolves last-wins. Needle at a live position that simply is not executed => named with its path. Four causes, four distinct messages, and a test asserts they stay distinct.
- **`compose` is consulted for DIAGNOSTICS ONLY, never for the verdict, and that boundary is now asserted [M]:** `test_safe_load_is_authoritative_over_compose` pins that on a duplicated key the composer sees the needle while the verdict still rejects. A boundary that lives only in discipline erodes; one that fails loudly does not.
- **Type the SEGMENTS, not the terminal [M]:** the previous fix asked 'may this KEY hold a list?' - a property of the key, checked once, at the leaf. That left the intermediate direction untouched, so `steps:` as a mapping, `jobs:` as a list, and `repos:`/`hooks:`/`steps:` as lists OF LISTS were all accepted, because a list of lists satisfies 'is a list' at every step while never being a list of mappings. Segments now carry their type - `('jobs', '*', 'steps[]', 'run')`, `('repos[]', 'hooks[]', 'args[]')` - and a sequence segment is iterated exactly ONCE, after which the element must be whatever the next segment demands. One statement, not ten exclusions.
- **A measurement of the previous head that was right for the wrong reason [M]:** the evaluator reported `steps:` as a mapping already rejecting on the prior commit. Checked against that commit's own module: it rejected `steps: {only: {run: ...}}` only because `run` was not a DIRECT key, and ACCEPTED `steps: {run: ...}`. Incidental protection, not typing - the same shape this slice has now found four times. The discriminating variant is a permanent test.
- **One document, because that is what the tools read [M]:** `safe_load` replaces `safe_load_all`. A needle in a second YAML document was accepted although neither pre-commit nor Actions reads past the first. Measured rather than assumed: `safe_load` raises `ComposerError('expected a single document in the stream')`, which lands in the fail-closed branch, and the message now NAMES the multi-document case instead of reporting a generic parse failure. The heuristic that detects it can only produce a less specific message, never a wrong verdict.
- **Fix the CLASS, not the instance, in the explain path [M]:** a recursive alias raised `RecursionError` from the diagnostics walk - which runs only while building a rejection, so the guard crashed precisely when explaining a failure. A cycle guard closes that instance. The class fix is that the whole explanation is now wrapped: any exception at all still yields a REJECTION saying what is known. Because the cycle guard hides the net behind it, a separate test forces the walk to raise something the guard has never seen and asserts the rejection still arrives.
- **Deliberately NOT over-corrected [M]:** the walk already refused a scalar or a mapping where it needed to keep descending, and three such cases (`repos:` as a mapping, `repos:` and `hooks:` as scalars) were correct before this change. They are regression controls now. Eight accept spellings most exposed to the new control flow - multi-item and flow-sequence `args`, both flow mappings, block-scalar `run`, an anchor aliasing a whole `steps:` list, alias-into-`entry`, and a `<<:` merge - all still accept. 16/16 with zero mismatches.
- **Verification [M]:** twelve-case matrix UNCHANGED; six restructurings pass; module tests 124 -> 141; suite 1800 -> 1817; collected 2020 -> 2037, none lost. Ruff lint and format clean across 215 files, Prettier 0, ESLint 0, secrets validator 31 files / 107 entries, URL check clean.
- **One limit disclosed rather than repaired, now in the PR as well as a test [M]:** PyYAML implements YAML 1.1, so a workflow's bare `on:` parses as boolean True and a mention under it reports at a path beginning `True.`. The verdict is unaffected. It is unrepairable rather than unfixed - the loaded document cannot say whether the source spelled it `on`, `yes` or `true`, and guessing one back would invent information the parser discarded. A pinned test is a limit the codebase knows about; a PR reader does not read the tests, so it is disclosed in both.
- **Files changed:** `tests/support/source_scan.py`, `tests/unit/test_secrets_baseline.py`, `agent-state/current_contract.json`, and this handoff. The four route files, both secrets scripts, `.secrets.baseline` and every manifest remain byte-identical to `80c1684`; the shell and PowerShell scanners are unchanged.
- **Boundary:** Draft PR only. Do not merge, do not flip any ledger flag.

## 2026-08-14 - Builder - F001 maintenance: clear the nanoid advisory GHSA-2v37-7h3g-55p8

- **Objective:** `Audit Node dependencies (apps/web)` was the ONLY red CI job in the repository, failing on `main` and on every open pull request against GHSA-2v37-7h3g-55p8 (`nanoid <3.3.18`). Clear it with the smallest package-manager-generated lockfile update - no suppression, and without committing the repository's first internal-host `resolved` URL.
- **Contract:** `agent-state/current_contract.json` (rewritten for this slice, evaluation level 2, base `80c168490ea334c61ca7944c3469045a83d80bb7`, re-derived with `git ls-remote origin main` before starting).
- **Baseline REPRODUCED before anything changed [M]:** `npm audit --audit-level=high` in `apps/web` reported `nanoid <3.3.18`, Severity high, GHSA-2v37-7h3g-55p8, `1 high severity vulnerability`, `fix available via npm audit fix`, at BARE exit code 1. Plain `npm audit` identical, also bare exit 1. A fix that cannot be shown to fix anything is worthless, so the red was established first.
- **Why this was fixable now and genuinely was not before [M]:** the earlier commit `26cc0dd` moved nanoid 3.3.16 -> 3.3.17, which was then the highest 3.x published, leaving the advisory open. 3.3.18 now resolves, so exactly ONE patch step remained. nanoid 4/5 are ESM-only while postcss needs the CommonJS `require('nanoid/non-secure')`, so staying on the 3.x line was the only honest route and it is the one taken.
- **The change is three lines [M]:** `git diff --numstat` reads `3 3 apps/web/package-lock.json` - one hunk, one entry, the `version` / `resolved` / `integrity` of `node_modules/nanoid`. Nothing else in the lockfile moved. Generated by `npm audit fix --package-lock-only`, never hand-authored.
- **The internal-host trap FIRED, and was corrected [M]:** `npm audit fix` rewrote `resolved` to this machine's internal package-feed host. Committing that would have introduced the repository's FIRST non-public resolved URL and leaked internal infrastructure into a public repo. Corrected back to `https://registry.npmjs.org/nanoid/-/nanoid-3.3.18.tgz` with a regex keyed on the version-suffixed tarball path, asserting EXACTLY ONE match before replacing, so the internal hostname was never typed into any file, command, commit message or report.
- **Host census, base and head [M]:** base 336 `resolved` URLs, 336 on `registry.npmjs.org`, ZERO on any other host. Head IDENTICAL: 336 / 336 / ZERO. Non-`resolved` hosts unchanged (github.com 41, opencollective.com 37, tidelift.com 4, eslint.org 2). Confirmed INDEPENDENTLY by the repository's own guard `scripts/check_urls.py` rather than by my own grep alone: 1011 URLs, 51 distinct hosts, all allowlisted, exit 0.
- **Integrity PROVED, with a control that could have failed [M]:** public `registry.npmjs.org` is UNREACHABLE from this machine - npm and an independent fetch path both fail at transport level - so the digest could not be checked against bytes pulled from the public host. Four independent agreements instead: (1) POSITIVE CONTROL ON THE METHOD - converting upstream `dist.shasum` for nanoid 3.3.17 (`f1c3aa253c52547956a52c50bff754316f61037a`) to base64 reproduces `sha1-8cOqJTxSVHlWpSxQv/dUMW9hA3o=`, EXACTLY the value already committed at base and already known to work; (2) the same derivation for 3.3.18 (`f66a2de1199ffde0fcf21c8a5f13106b1c081913`) yields `sha1-9mot4Rmf/eD88hyKXxMQaxwIGRM=`; (3) the fetched 5713-byte tarball RECOMPUTES to that same sha1, so the served bytes are not re-packed; (4) npm independently wrote the identical value, and `npm ci` then verified it against real fetched bytes.
- **`sha1-` is an INHERITED convention, not a choice [M]:** the lockfile carries 335 `sha1-` and 1 `sha512-` entries. Measured cause: a sha512 `dist.integrity` field is ABSENT from the metadata reachable here, so sha1 is what the toolchain supplies rather than a stylistic preference. Recorded for cross-reference but deliberately NOT committed: the 3.3.18 sha512 is `DTg4MJbGMWkfi6VZFdNt2/caMbQy4Ou+Op/hJQvGEWcnVfoA1QA+xzRKAzw9jD6+GVOOeYr/mIcuDSdug6F6+w==`. Strengthening the whole lockfile to sha512 would be a separate deliberate slice, not a side effect of an advisory fix.
- **A DISCLOSED CI GAP found while verifying, larger than the integrity question [M]:** `.github/workflows/ci.yml` runs `npm ci` at the REPOSITORY ROOT only. `apps/web` receives exactly ONE CI operation - `npm audit`, which reads the lockfile WITHOUT installing. Consequently no CI job runs `npm ci`, the 9 web test files (110 tests), the `tsc -b` type-check, or the production build for `apps/web`. A wrong integrity value, a failing web test, or a TypeScript error would all merge green today. That is precisely why the integrity had to be proved locally rather than deferred to CI. NOT fixed here - repairing it inside a lockfile bump would be scope creep - and filed as an observation for a separate slice.
- **A proposed diagnosis CORRECTED rather than confirmed [M]:** two sessions on this machine read different postcss facts within the hour - 8.5.25 requiring `^3.3.16` and 8.5.26 requiring `^3.3.17` - and it was proposed that the package mirror was serving inconsistent metadata, the same explanation previously attached to five weeks of an automation reporting this advisory unfixable. Measured side by side in ONE command: `postcss@8.5.25 -> ^3.3.16`, `postcss@8.5.26 -> ^3.3.17`, `dist-tags.latest -> 8.5.26`, and the copy installed on disk after `npm ci` -> `8.5.25` requiring `^3.3.16`. BOTH readings are correct and simultaneously reproducible; they describe installed-and-pinned versus registry-latest, which are different questions. The mirror was self-consistent on this point. Every conclusion survives either way, because 3.3.18 satisfies both ranges and exactly ONE package depends on nanoid - but recording the instrument as flaky would have entrenched a false explanation of a five-week blockage.
- **postcss NOT upgraded [M]:** it stays pinned at 8.5.25. The fix required no manifest change of any kind.
- **Not a suppression, and provably so [M]:** no dependency `override` was added - `apps/web/package.json` has ZERO diff, blob unchanged at `121d8a1f8a1ccea9116c9bd098732eb26e33aac8`, and the pre-existing `vite`/`esbuild`/`js-yaml` overrides are untouched. No `.npmrc`, no audit configuration, no `--audit-level` relaxation, no GitHub tarball source, no public-registry bypass, no nanoid 4/5 jump, and zero diff to `.github/**`. The green comes from the dependency actually moving above the advisory floor.
- **Files changed:** `apps/web/package-lock.json` (3 lines, one entry), `agent-state/current_contract.json`, and this handoff. Nothing else.
- **Tests and checks run [M]:** post-fix `npm audit` and `npm audit --audit-level=high` both report `found 0 vulnerabilities` at BARE exit 0, read from bare invocations because a pipeline can corrupt `$LASTEXITCODE`. `npm ci` in `apps/web` clean from the COMMITTED lockfile at exit 0, 336 packages added, `nanoid 3.3.18` installed, and the lockfile blob UNCHANGED at `98f786c2bb0d2e9152d66e6b7ba7b5c3b54b580e` before and after, proving npm did not rewrite it. `npm test` 110 passed across 9 files, exit 0. `npm run build` (`tsc -b && vite build`) succeeded, exit 0 - real functional exercise of nanoid 3.3.18, since vite builds through postcss. `scripts/check.ps1 -NodeAudit`: **ALL CHECKS PASSED, exit 0** - Ruff lint, Ruff format (214 files), Pytest `1722 passed, 220 skipped`, Prettier, ESLint, Secrets baseline shape, Secret scan, URL host allowlist, all three Python dependency audits, root Node audit, and for the FIRST TIME `Audit Node dependencies (apps/web)` -> `found 0 vulnerabilities`.
- **Python suite unaffected, structurally [M]:** zero diff to any `.py` file, to `pyproject.toml`, or to any requirements file; the 220 skips are Postgres-backed and expected without `DATABASE_URL` in this environment.
- **Known issues or risks:** the integrity value could not be checked against a byte stream fetched directly from public `registry.npmjs.org`, because that host is unreachable from this machine; the claim rests on the four agreements above, one of which is a control that could have failed. The `apps/web` CI gap is real and remains OPEN.
- **Protected state [M]:** `agent-state/feature_list.json` byte-identical to base at blob `154de1fef2ba20f587c9ec2d1302ebe2bfb5bfa1`; NO ledger flag flipped. Zero diff to `apps/web/package.json`, `.github/**`, root `package.json` and `package-lock.json`, `apps/api/**`, `apps/worker/**`, `tests/**`, `scripts/**`, `migrations/**`, `docs/**` and `config/**`. Verified with the unfiltered `git diff --numstat` run FIRST as a positive control.
- **Evaluator disposition:** pending - builder self-review only.
- **Boundary:** Draft PR only. Do not merge, do not flip any ledger flag.

## 2026-08-14 - Builder - F008 P5 slice: Microsoft Azure provider profiles

- **Objective:** Add the fifth of six F008 providers. Azure is the provider most able to produce BOTH failure directions at once, because Microsoft markets FOUR different free things under overlapping branding - a credit-backed *Azure free account*, a *12 months free services* introductory window, per-service *free plans and free tiers*, and eligibility-gated *programmes* - and one of those free tiers is explicitly called LIFETIME while another has a genuinely safe, non-billing exhaustion behaviour.
- **Contract:** `agent-state/current_contract.json` (rewritten for this slice, evaluation level 2, base `f6c7c402978c916b374ffc5dfc228d26346c8434` re-derived with `git ls-remote origin main` before branching; it matched the brief and `main` had not moved). `main`'s CI was re-derived at JOB level rather than inherited from prose: `Dependency audit` failure, `Python lint, format, tests` success, `Node format and lint` success, `Secret scan` success.
- **Services covered, and why [M]:** seven sources over seven documents on TWO official hosts, no two sharing a page. The split is itself a finding: Microsoft's marketing host publishes the offer TERMS while its documentation host publishes the per-service ALLOWANCES and exhaustion behaviour, so a slice restricted to one host would have had to compose facts across documents to say anything material. (1) **Azure free account** - `/pricing/offers/ms-azr-0044p/`, the only page in the sweep stating the payment-method requirement. (2) **Azure 12 months free services** - `/pricing/free-services/`, the introductory window, from the hub's own footnote. (3) **Azure Cosmos DB** - `learn.microsoft.com/.../cosmos-db/free-tier`, the LIFETIME tier. (4) **Azure App Service** - `learn.microsoft.com/.../app-service/web-sites-monitor`, matrix, and the one safe exhaustion behaviour found. (5) **Azure Static Web Apps** - `learn.microsoft.com/.../static-web-apps/plans`, matrix. (6) **Azure DevOps Services** - `/pricing/details/devops/azure-devops-services/`. (7) **Azure for Students** - `/free/students/`.
- **THE HEADLINE, and I predicted before probing that I would get it wrong [M]:** Azure DOES state a perpetual free tier in a block of its own. `https://learn.microsoft.com/en-us/azure/cosmos-db/free-tier` is titled "Lifetime Free Tier - Azure Cosmos DB" and says "Free tier lasts indefinitely for the lifetime of the account and it comes with all the benefits and features of a regular Azure Cosmos DB account." That satisfies rule 1 of `docs/DATA_MODEL.md` by quotation, so the offer is genuinely `always_free`. **It is still Z1**, because the SAME block that grants the allowance says "The throughput and storage consumed beyond these limits are billed at regular price." Azure reproduces the AWS Step Functions finding independently, on a different provider and a different document: PERPETUAL DOES NOT MEAN Z0. The prediction was recorded in the contract as the one most likely to be wrong precisely because the AWS builder's most valuable disclosed error was banking a tidy "this provider never states a perpetual offer" too early - and the Azure marketing hub reproduces that trap exactly: measured, `https://azure.microsoft.com/en-us/pricing/free-services/` serves the heading "65+ always-free services with an Azure account" with NO allowance prose and zero tables. Stopping at the hub would have produced the same wrong negative.
- **Measured classifier verdicts from the REAL engine [M]:** Azure free account **Z1_BILLING_EXPOSURE** (card required, quoted); Azure Cosmos DB **Z1_BILLING_EXPOSURE** (automatic billing on a perpetual allowance); Azure 12 months free services **UNKNOWN**; Azure App Service **UNKNOWN**; Azure Static Web Apps **UNKNOWN**; Azure DevOps Services **UNKNOWN**; Azure for Students **UNKNOWN**. Zero Z0. The coverage declaration contains ZERO `verified_free` entries.
- **POSITIVE CONTROL, and it was PROVEN LOAD-BEARING rather than merely committed [M]:** `test_the_z0_sweep_is_not_vacuous` asserts that `always_free` + `requires_card=False` + `has_paid_dependencies=False` + `hard_stop` returns **Z0_TRUE_FREE**. `test_the_positive_control_is_load_bearing` then BREAKS the engine (`monkeypatch.setattr(engine, "SAFE_EXHAUSTION", frozenset())`, patched line printed) and measures the asymmetry: under the patched engine the control's own assertion FAILS (`UNKNOWN`, not Z0) while all seven Azure offers still classify non-Z0, so the sweep alone would have reported success against a classifier incapable of the verdict. A control that cannot fail proves nothing; this one demonstrably can.
- **A second prediction I got WRONG, and it changed the slice [M]:** I expected no Azure page to state a SAFE exhaustion behaviour for a free tier. App Service states one in a block of its own - "If an app exceeds the CPU (Short), CPU (Day), or Bandwidth quota, the app is stopped until the quota resets. During this time, all incoming requests result in an HTTP 403 error." That is a non-billing stop, so this offer clears the billing gate ENTIRELY and every blocking condition it has is an UNKNOWN rather than an exposure. **It is TWO unknowns from Z0, not one** - gate 4 reports `requires_card` and `has_paid_dependencies` independently and this document states neither, so resolving the card alone still yields UNKNOWN. MEASURED by exhaustive tri-state enumeration holding the document's own `offer_type` and `exhaustion_behaviour`: exactly **1 of 9** combinations reaches Z0 and it needs BOTH facts. `test_the_safest_azure_offer_is_two_unknowns_from_z0` asserts the EXACT blocking set, with a discrimination control in the same test proving the assertion changes when the set changes.
- **THE LEVEL-2 FAIL, and it was correct [M]:** an earlier revision of this slice said App Service "fails ONLY on gate 4 ... one unknown separates it from Z0", committed in TEN places. Every word was true - gate 4 IS the only gate that fires - and it still reliably read as "one FACT is missing", which is false. No verdict, fixture, profile or classifier was wrong; every published classification was and remains correct; and the error ran in the CONSERVATIVE direction, so no false-free claim was ever publishable. It failed anyway, and rightly: a later slice supplying the card fact and expecting Z0 would get UNKNOWN and hunt for a bug that does not exist. **The test was worse than the prose** - `test_the_safest_azure_offer_fails_only_on_the_card_gate` asserted every blocker was an unknown and that SOME blocker mentioned the card, never that the card was the ONLY blocker, so **it passed while its own name was false**. A test renamed accurately but asserting no more than before would have been worse still: it would replace a false name with an unearned guarantee. Hence the exact-set equality plus the in-test discrimination control, which makes it non-vacuous BY CONSTRUCTION rather than by inspection.
- **All three defects in this slice are ONE class, and that is the most useful thing it produced [M]:** (1) the PR body's "the SAME block that grants the allowance" placed right after the perpetuity quote - true, but read as "the same block as the one just quoted"; (2) every `capture.json` saying "pinned block(s)" where the count was of RETAINED blocks - true of the number, false of the noun; (3) "fails ONLY on gate 4 ... one unknown" - true of the gate, false of the fact count. **A technically true claim whose most natural reading is false.** The class is REVIEW-ONLY by construction: the author already knows the intended reading and cannot un-know it, so being told about the class does not enable finding an instance in one's own work. Three confirmed instances in a single slice is concrete evidence the fresh-context requirement is load-bearing rather than ceremonial. All three ran in the conservative direction, which is NOT a property to rely on - the same habit pointed the other way produces a claim reading MORE free than the evidence supports, the one failure this product cannot absorb.
- **`offer_type='other'` is NOT a safety gate, pinned rather than assumed [M]:** MEASURED - `other` is in neither `TEMPORARY_CONDITIONAL_OFFER_TYPES` nor `SELF_HOSTED_OFFER_TYPES`, and `other` + `requires_card=False` + `has_paid_dependencies=False` + a safe stop classifies **Z0_TRUE_FREE**. The Z0-capable types are `always_free`, `recurring_quota`, `personal_use_free` and `other`. So `docs/DATA_MODEL.md` rule 3's "route the candidate for review" is an instruction to the AUTHOR, not a behaviour of the classifier, and what withholds Z0 from both rule-3 offers (App Service, Static Web Apps) is the unknown facts plus the publication gate - never the offer type. Pinned here rather than filed because it is the same claim as the failed one from the other side: "exactly the two unknowns withhold Z0, and nothing else does" is ONE statement. `test_offer_type_other_is_not_a_safety_mechanism` is a deliberate DRIFT DETECTOR - if a later slice gates `other`, it fails BECAUSE THE ENGINE BECAME SAFER, and its failure message says so explicitly so the next author updates it deliberately rather than deleting it or weakening it to green.
- **A third prediction I got WRONG [M]:** I expected no Azure page to state that a card is NOT required. The Students page publishes the bare list item "No credit card required" - the only such block in the entire sweep. Omitting it would have been exactly the omission-favouring error this brief weights as equal to overstating, so it IS extracted and published whole as a `card_claim` fact. It is deliberately NOT converted into `requires_card=False`: the live page carries TWO offers, and the Azure for Startups list immediately below states the opposite direction ("Spending protection-credit card required only for identity verification and services beyond credit*"). Read alone the bullet names no offer, so pinning it would splice one offer's terms onto a page publishing two. The choice is **verdict-neutral and that is MEASURED, not claimed**: `test_pinning_the_students_card_claim_would_change_no_verdict` shows UNKNOWN either way, and Z2 at best under the most generous possible reading.
- **A fourth thing I got wrong, caught by my own test [M]:** the first revision of every `capture.json` wrote "all N pinned block(s)" into `live_reconciliation` where N was the count of blocks the generator RETAINED, not the count of profile assertions. Those differ - a `<title>` retained for context is read by no assertion - so the claim was wrong on 5 of 7 captures. Found by the test I had just written, which compared the sidecar's number against the profile. Fixed at the SOURCE rather than by hand-editing the sidecars: the generator now says "retained live block(s)", states explicitly that RETAINED is not the same as ASSERTED, and emits `retained_live_block_count`; the whole capture chain was re-run against live. The test now RECOMPUTES the count from the committed bytes with `_DocumentCollector` rather than taking the sidecar's word for it, and separately asserts every asserted block is one of the retained ones.
- **LIVE <-> FIXTURE reconciliation, enforced by the GENERATOR in both directions [M]:** the generator resolves every pinned block against the live parse BY NEEDLE (never by a hand-typed sentence), refuses a needle matching zero or several distinct blocks, refuses any block occurring more than once live (which would yield `ambiguous_assertion`), re-parses the excerpt it is about to write, and REFUSES TO WRITE when anything differs. It emits the resolved strings as Python literals so the profile module was written from the live parse - transcription being exactly where a *composed* quotation creeps in. Measured per source (retained blocks, live rows / fixture rows, cells): free-account 9, 0/0, 0; free-services 4, 0/0, 0; cosmos-db 7, 0/0, 0; app-service 7, 5/5, 10; static-web-apps 4, 13/13, 52; devops 5, 0/0, 0; students 8, 0/0, 0. LIVE -> FIXTURE: zero live pinned blocks missing, zero live target-table rows or cells missing. FIXTURE -> LIVE (supporting only): zero fixture blocks or cells absent from live - the generator raises `ReconciliationError` on the first intruder. It DID refuse once during development, on an ambiguous `Azure for Students` heading needle that matched 5 live headings; that refusal is why the resolver now prefers whole-block equality, which is the engine's own semantics.
- **Guards mutation-tested with predictions recorded BEFORE running [M]:** eleven mutations, predictions written into the parametrisation, each anchor asserted to match EXACTLY ONCE and the patched line printed. Deleted, reworded and truncated pinned blocks all yield `assertion_not_found`; a duplicated block yields `ambiguous_assertion`; an undeclared row `unknown_matrix_rows`; a removed row `missing_matrix_rows`; a renamed tier header `table_not_found`; a duplicated table `ambiguous_table`. THREE controls must NOT fire: an **unrelated added paragraph** (the discrimination control this brief asks for - and it is checked on VALUES, not merely on absence of error: `test_the_unrelated_edit_leaves_every_published_fact_identical` asserts the whole fact set is unchanged), an extra body column, and whitespace/entity noise in a row label. **All eleven matched observation on the first run.** Beyond that, two guards were disabled at the PROFILE level: with the required-assertion guard off, the deleted-block input stopped producing `assertion_not_found` and yielded a candidate silently MISSING its exhaustion behaviour; with the row-completeness guard off, the injected-row input stopped producing `unknown_matrix_rows`. Collateral controls confirm a healthy document still extracts under each patched state. `apps/api/app/ingest/adapters/html.py` was never modified.
- **Fixture integrity proved by BLOB HASH, never numstat [M]:** all twelve `source.html` blob hashes are byte-identical before and after the Prettier and Ruff passes, confirming the `**/fixtures/ingest/*/*/*/source.*` exemption holds and that the mutation battery writes nothing to disk. `unchanged/source.html` and `azure-app-service-quotas/source.html` share blob `61dbaebe4840c85f339472075b2a3d0bb73d640f`, which is what "byte-identical to the official capture" is supposed to mean. Nothing was added to `.prettierignore`.
- **No synthesized structure anywhere [M]:** five of seven profiles are `mode="assertions"`, declare no table selector and read no table; their captures contain ZERO `<table>` elements. Where the LIVE page does carry tables (free-account 2, devops 1) the capture says so and records WHY none is a target - the free-account tables are the country/currency availability grid and fail header-row selection, and the DevOps table has one-cell section rows so the engine rejects it with `irregular_row_width`. The two matrix captures retain their target tables complete: `target_table_rows_removed` and `target_table_cells_removed` are both empty and asserted empty by test, and the two unread Static Web Apps columns are DECLARED in `unpivoted_target_table_columns`.
- **First use of `docs/DATA_MODEL.md` rule 3 in this repository [M]:** App Service and Static Web Apps each identify a Free plan, but neither document states the plan is indefinite (rule 1 fails) and in both the containing plan IS the zero-priced one (rule 2 does not apply). Rule 3 is reached explicitly - "do not infer it from words such as 'monthly', 'included', or 'free'. Use `other` and route the candidate for review" - so both are `offer_type: other`. Recorded so a reviewer can check the determination against the rule rather than against my intuition.
- **Facts deliberately left UNKNOWN, with reasons [M]:** `requires_card` on the six non-account profiles (no block on their own documents states it; carrying the free-account sentence across documents would be composition, the error the Google Cloud slice avoided). `has_paid_dependencies` on all seven - the free-account page lists what the credit may not BUY, which is not the same claim as the offer depending on a paid product, so it is carried as a `credit_exclusions` note instead. `exhaustion_behaviour` on three: the free-services hub and the Students page state no consequence in served HTML (the Students FAQ ANSWERS are client-rendered; only the question headings are served), and the DevOps page states how to obtain MORE capacity - "simply buy the number of pipelines you need" - but never what happens if you do not, so deriving a rule from a purchase instruction was refused.
- **Coverage declaration, honest in BOTH directions [M]:** 3 of 14 categories are `offered_no_z0` with declared provenance (`containers-app-hosting` <- App Service, `nosql-key-value` <- Cosmos DB, `cicd-source-control` <- DevOps); the other ELEVEN are `unknown` and ZERO are `not_offered`. Six of the eleven were probed live and name what the page did or did not say; five say plainly they were NOT probed, and `test_the_eleven_unknowns_are_exactly_the_probed_plus_the_unprobed` proves no third kind exists. Three is exactly the evidence floor, so `test_dropping_one_evidenced_category_breaks_the_evidence_floor` pins that a later withdrawal makes the config UNLOADABLE rather than quietly thin. The reusable Wave-3 helper `assert_declarations_match_signals` is called from this provider's own module, with a NON-VACUITY guard proving it fires on a suppressed free offer.
- **Findings recorded against my own interest [M]:** two categories name a concrete measured allowance they declined to publish - Microsoft Entra External ID ("free for your first 50,000 monthly active users") and Azure App Configuration ("Free tier stores are limited to 1,000 requests per day. When a store reaches 1,000 requests, it returns HTTP status code 429 for all requests until midnight UTC", a SECOND safe stop). Both were left out to keep the covered set fully evidenced and fixture-backed, and both rationales say so explicitly rather than implying the evidence does not exist. `test_two_categories_name_a_concrete_candidate_for_the_next_slice` asserts the numbers are actually there.
- **Pages that could NOT be used, reported rather than worked around [M]:** MEASURED live, `https://azure.microsoft.com/en-us/pricing/details/static-web-apps/` returns HTTP 404, and `https://azure.microsoft.com/en-us/pricing/details/active-directory/` REDIRECTS to a `www.microsoft.com` URL outside this provider's official-domain allowlist, so the repository's own fetch policy refused it (`disallowed_host`) before any content was read. Azure Functions publishes real free grants but its own page states "Free grants apply to paid, consumption subscriptions only", so a grant conditioned on a PAID subscription was refused as evidence of a free offer.
- **Files changed:** `apps/api/app/ingest/adapters/profiles/azure.py` (new), `config/examples/providers/azure.example.yaml` (new), `tests/fixtures/ingest/azure/html/**` (new; seven live captures plus five synthetic negative scaffolds that declare their synthetic provenance), `tests/unit/test_adapter_azure.py` (new), `tests/unit/test_azure_coverage_config.py` (new), `tests/integration/test_ingest_azure.py` (new), `scripts/url-allowlist.txt` (add `azure.microsoft.com` and `learn.microsoft.com`), `docs/PROVIDER_ADAPTERS.md`, `tests/fixtures/ingest/README.md` (attribution), `.secrets.baseline` (refreshed only via `scripts/refresh_secrets_baseline.py`, and only AFTER `git add`: **42 -> 54 tracked files, 156 -> 214 entries**), `agent-state/current_contract.json`, and this handoff. 46 files, +4884/-61.
- **Tests and checks run [M]:** full suite **2372 passed, 2 skipped** against a REAL PostgreSQL 16 on a DEDICATED scratch database `atlas_f008_p5_azure`, created for this slice on the running `freetier-atlas-postgres-1` container so the shared `atlas` database was never touched. Baseline measured on that same database BEFORE any change: **2184 passed, 2 skipped**. The delta is **+188 tests, zero broken**, and 188 is exactly the number of collected node IDs mentioning `azure`, so the delta is fully accounted for: 92 in `tests/unit/test_adapter_azure.py`, 48 in the shared `tests/unit/test_capture_sidecar.py` (the new fixtures are picked up by the existing sidecar-integrity sweep), 26 in `tests/unit/test_azure_coverage_config.py`, 12 in the shared `tests/unit/test_fixture_harness.py`, 9 in `tests/integration/test_ingest_azure.py` and 1 in `tests/unit/test_config_system.py`. The integration tests RAN, they did not skip. `scripts/check.ps1 -NodeAudit`: Ruff lint, Ruff format, Pytest, Prettier, ESLint, Secrets baseline shape, Secret scan, URL host allowlist and all three Python dependency audits **PASS**; exit codes read from BARE calls, never through `Select-Object -First N`.
- **Known limits, stated rather than hidden:** live page digests are NOT reproducible across fetches (Microsoft serves per-build markup and rotating correlation ids), so each capture pins STRUCTURE and says so; `sha256_original` is provenance only and `sha256_stored` is declared to be a tamper-evidence seal on the committed bytes, NOT a link to live. The Static Web Apps header signature deliberately omits the live fourth column "Dedicated plan (Retired effective October 31st, 2025)", because pinning a retiring plan's label would break the profile on the day Microsoft removes a column carrying none of the facts read here.
- **Evaluator disposition:** pending - builder self-review only.
- **Known issues or risks:** none outstanding. At the time of the first verification run, `Audit Node dependencies (apps/web)` failed on GHSA-2v37-7h3g-55p8 (`nanoid <3.3.18`) — the accepted, documented risk that also failed on untouched `main` (verified at JOB level on run 31843330007 for `f6c7c40`: one red job, `Dependency audit`). **That fix landed on `main` mid-slice as PR #71 (`3b382aa`)**, which is the in-flight fix my brief predicted, and this branch was rebased onto it. Re-measured on the rebased head: `npm ci` in `apps/web` clean and `npm audit --audit-level=high` reports `found 0 vulnerabilities` at BARE exit 0, and `scripts/check.ps1 -NodeAudit` reports **ALL CHECKS PASSED**. I touched no manifest, lockfile, override or audit configuration; `apps/web/package-lock.json` is byte-identical to the new base at blob `98f786c2bb0d2e9152d66e6b7ba7b5c3b54b580e`. **My brief's sentence that `Dependency audit` is main's only red job is therefore now out of date — main is at zero red — and the baseline was re-derived at job level rather than inherited from that prose.**
- **Rebase, and why the verification stays composable [M]:** `main` moved from `f6c7c402978c916b374ffc5dfc228d26346c8434` to `3b382aa4f95a6abb02e6739637449302313dd618` after this branch was cut and after the full local run. Only the two LEDGER files conflicted: `agent-state/current_contract.json` (each slice rewrites it, so mine stands, with a rebase note appended) and this file (resolved by taking main's blob VERBATIM and re-appending this entry). **Every source, config, test, doc and fixture blob came through byte-identical**, verified with `git hash-object` rather than predicted: `azure.py` `f0dfcd3e6ac3c9a412e8d8c9cb5ce62b2b875275`, `azure.example.yaml` `efa653b8fe5054c49bf75d935b10f602b4aac77c`, `test_adapter_azure.py` `ac3ff0b0af4ab4dad0e560c76a1a6c1c7260863e`, `test_azure_coverage_config.py` `d4fae961a335034bc84067d0aeac530c7a0b3c81`, `test_ingest_azure.py` `73b7d2565a3b17bbbda4c2f7f88a1381d35b167e`, `PROVIDER_ADAPTERS.md` `ca01c09ba51dc378c673f797bbdc47586fe5f496`, `url-allowlist.txt` `1563755f9fe6d50acf849ac15c7cf02048e111ff`, `.secrets.baseline` `eebf4e016b5ba9eb929b4b4b4813238706c3c6da`, and `git diff --name-only` over `tests/fixtures/ingest/azure` returns EMPTY. The full suite and `check.ps1 -NodeAudit` were both re-run on the rebased head.
- **Protected state [M]:** zero diff, each verified by `git hash-object` BLOB COMPARISON against base rather than by `git diff --numstat`. `agent-state/feature_list.json` `154de1fef2ba20f587c9ec2d1302ebe2bfb5bfa1`; `tests/support/source_scan.py` `3a86f03aeb1786122081118e2e3be72705a44072`; `tests/unit/test_secrets_baseline.py` `183619e003758d2fd05bb1ffd4f649902fd85187`; `apps/api/app/ingest/adapters/html.py` `f1a68632326e80104e89069a4f113b586c4b9176`; `apps/api/app/classify/engine.py` `900d27399040968774456a565ca4e1ddd2f1666b`; `apps/api/app/ingest/fetch.py` `b7633e38b96253dcc2093c6a59641dc64fceb9ce`; `scripts/capture_fixture.py` `8e58c32d4355d02f22ef723276c906d8533227b1`; `apps/web/package-lock.json` `f492053a91df9a977f6fda838ee82c6c38f5eda7`; `.pre-commit-config.yaml` `b7ebd45a00dc4e60a6f6a80d96508a24027a4c8e`; `.github/workflows/ci.yml` `4f80c771f9562be9df687c5cec918b36548e9d60`. **F008 stays `passes:false`.**
- **CI performs zero socket operations [M]:** all live fetching happened in an owner-run generator that lives OUTSIDE the repository, precisely so `tests/unit/test_no_live_fetcher_in_tests.py` stays green by construction - no new `LiveFetcher` construction enters `apps/` or `scripts/`. That AST guard passes.
- **Boundary:** Draft PR only. Do not merge, do not flip any ledger flag.


## 2026-08-15 - Builder - F008 P6 slice: Oracle Cloud provider profiles (LAST provider)

- **Objective:** Add the sixth and last F008 provider. Oracle is the riskiest of the six: it markets "Always Free" more prominently than any other provider here, and unlike the marketing-only cases its Always Free tier really IS perpetual. A false Z0 is the one failure this product cannot absorb, and a wrongly-omitted free offer is a defect too.
- **Contract:** `agent-state/current_contract.json` (rewritten for this slice, evaluation level 2, base `3b382aa4f95a6abb02e6739637449302313dd618` re-derived with `git ls-remote` before branching and again before reporting; `main` had not moved either time).
- **Services covered, and why [M]:** six sources over six DISTINCT documents, three on `docs.oracle.com` and three on `www.oracle.com`, so no claim rests on marketing copy alone. (1) **Oracle Cloud Infrastructure Always Free** - `.../FreeTier/freetier_topic-Always_Free_Resources.htm`, the authoritative technical statement AND the only probed Oracle page with a header-selectable table (`Resource` / `Limit Name` / `Always Free`, 6 body rows). (2) **Oracle Cloud Infrastructure Free Trial** - `.../FreeTier/freetier.htm`, which carries the clearest card sentence Oracle publishes in prose. (3) **Oracle Cloud Always Free services** - `/cloud/free/faq/`, THE FLAGSHIP: perpetuity and card requirement quoted on one document. (4) **Oracle Cloud Free Tier** - `/cloud/free/`. (5) **Oracle Cloud Free Credit Promotion** - `.../signingup_topic-What_Happens_When_the_Promotion_Expires.htm`. (6) **Oracle MySQL HeatWave** - `/mysql/free/`, giving relational-databases an independently evidenced source.
- **THE FAVOURABLE FINDING, evidenced rather than hedged away [M]:** Oracle's Always Free tier is GENUINELY PERPETUAL, quoted from blocks that describe Always Free alone. OCI docs: "All Oracle Cloud Infrastructure accounts (whether free or paid) have a set of resources that are free of charge in the home region of the tenancy, for the life of the account." FAQ: "Always Free services are available for an unlimited time." Both satisfy rule 1 of `docs/DATA_MODEL.md` by quotation, so `always_free` is not inferred from the word "free". Three `always_free` offers are extracted and the slice says so plainly - omitting them would have been the other half of the product rule.
- **THE DECISIVE FINDING, and it is unfavourable [M]:** nothing in this provider is Z0. Oracle states a payment-card requirement in blocks of its own on FOUR independent documents. `.../FreeTier/freetier.htm`: "For security purposes, most users need a mobile phone number and a credit card to create an account. Your credit card will not be charged unless you upgrade your account." FAQ, hub page and MySQL page each carry: "To provide free Oracle Cloud accounts to our valued customers, we need to ensure that you are who you say you are. We use your contact information and credit/debit card information for account setup and identity verification. ..." **Perpetual is not free**, and Oracle is the third provider in this repository to prove it after AWS Step Functions and Azure Cosmos DB.
- **Measured classifier verdicts from the REAL engine, with the SHAPE of each blocking condition [M]:** Oracle Cloud Infrastructure Always Free **UNKNOWN** (card by ABSENCE); Oracle Cloud Infrastructure Free Trial **Z1_BILLING_EXPOSURE** (card by QUOTATION); Oracle Cloud Always Free services **Z1_BILLING_EXPOSURE** (card by QUOTATION); Oracle Cloud Free Tier **Z1_BILLING_EXPOSURE** (card by QUOTATION); Oracle Cloud Free Credit Promotion **UNKNOWN** (card by ABSENCE); Oracle MySQL HeatWave **Z1_BILLING_EXPOSURE** (card by QUOTATION). Four refusals rest on a quoted sentence on that offer's own document; TWO rest on the absence of any payment statement, because those two documents say nothing about payment and importing the requirement from another Oracle page would be cross-document composition. An absence can be flipped by anything that later supplies the field; a quotation cannot. The distinction is asserted in `test_an_absence_based_refusal_is_declared_as_such` so it cannot quietly drift.
- **The card READING, stated so a reviewer can check it rather than trust it [M]:** neither block contains the word "required", and the first hedges with "most users". They are read as `requires_card=True` because each states that supplying a card is part of creating a FREE Oracle Cloud account, and the second goes on to say Oracle periodically checks the validity of "your card". No Oracle document probed states a card is NOT required. Leaving the field absent would withhold Z0 too - via gate 4 rather than gate 3 - so the choice changes no Z0 verdict, only whether the refusal is reported as a definite billing exposure or as an unknown. A fifth document states it as an imperative procedure step ("Select Add payment verification method, and then select Credit Card."); it is NOT a source, because it is a procedure page stating no offer type in a block of its own and pinning one to a button label would be inference.
- **A trap measured and avoided [M]:** the block "You will only be charged for services that you use that exceeds Always Free." looks like automatic billing for Always Free. Its FAQ question - held in a `<div class="cb105w3">` the collector does not capture - is "How do I know how much I am going be charged for Pay As You Go services?". Its own context is a PAID account, so it is pinned as an exhaustion behaviour nowhere in this slice. `test_the_pay_as_you_go_billing_sentence_is_never_an_exhaustion_behaviour` enforces that, and the nearest equivalent on the hub page is carried whole as a note so the boundary stays visible.
- **NOTHING WAS TRANSCRIBED [M]:** every pinned block was resolved against the LIVE document's own parse by a short unique needle, and the fixture was written from the resolved literal by a generator that REFUSES TO WRITE on zero or multiple needle matches, on a resolved block occurring more than once live, or on parsed target-table rows differing from live. The wrapped Python constants were emitted by script and re-evaluated for equality before use, and **60/60 constants in the committed `oracle.py` were then verified byte-identical to the generator's live literals** by an independent second pass. Transcription is where a *composed* quotation creeps in; generating from the resolved literal removes the opportunity rather than guarding against it.
- **LIVE -> FIXTURE reconciliation, the primary direction [M]:** per source (distinct pinned blocks, live rows / fixture rows, fixture blocks): always-free-resources 24, 6/6, 38; free-tier 7, 0/0, 7; always-free-services 8, 0/0, 8; cloud-free-tier 7, 0/0, 7; free-credit-promotion 6, 0/0, 6; mysql-heatwave 8, 0/0, 8. Every pinned block occurs EXACTLY ONCE live and exactly once in its capture; zero live target-table rows missing, none added. FIXTURE -> LIVE, reported as SUPPORTING evidence only and never carrying a pass alone: zero fixture blocks absent from live, on all six. Driving the real `HtmlDocAdapter` over LIVE bytes and over the committed fixture produced **IDENTICAL fact sets for all six** (32, 11, 10, 10, 8 and 10 facts, one evidence location per fact).
- **No synthesized structure anywhere [M]:** five of six profiles are `mode="assertions"`, declare no table selector and read no table; their captures contain ZERO `<table>` elements because the live pages have none (`live_table_count: 0`, asserted). The one matrix capture retains its target table complete: `target_table_rows_removed` and `target_table_cells_removed` are both empty and asserted empty, and the unread `Limit Name` column is DECLARED in `unpivoted_target_table_columns` rather than silently dropped.
- **A block Oracle publishes TWICE is retained, not hidden [M]:** "50,000 Object Storage API requests per month" appears in both the Always-Free-only list and the paid/trial list on the OCI docs page. The generator refuses to pin any block occurring more than once live, so it is not pinned; BOTH occurrences are retained and declared in `duplicate_live_blocks_retained`, and `test_a_block_oracle_publishes_twice_would_be_ambiguous_if_pinned` proves that pinning one yields `ambiguous_assertion`.
- **Mutation predictions versus outcomes [M]:** eleven mutations with predictions recorded in the parametrisation BEFORE running - deleted, reworded and truncated pinned blocks all yield `assertion_not_found`; a duplicated block yields `ambiguous_assertion`; an undeclared row `unknown_matrix_rows`; a removed mapped row `missing_matrix_rows`; a renamed tier header `table_not_found`; a duplicated table `ambiguous_table`; and THREE false-positive controls (an extra body column, whitespace/`&nbsp;` noise in a row label, and an unrelated added paragraph) correctly yield no error. **All eleven matched observation on the first run.** The unrelated-paragraph control additionally asserts the whole FACT SET and the classifier VERDICT are unchanged, because "no error" alone would not catch a silently dropped fact. Beyond the battery, EVERY distinct pinned block on all six sources is deleted and separately duplicated in two parametrised sweeps (`test_deleting_any_pinned_block_rejects_the_document`, `test_duplicating_any_pinned_block_rejects_the_document`) - not one representative block, every one of them - so a decorative pin would be visible.
- **Guards mutation-tested by making the error UNPRODUCIBLE [M]:** two guards were disabled at the PROFILE level with the patched line printed. With the required-assertion guard off, the deleted-perpetuity input stopped producing `assertion_not_found` and yielded a candidate silently MISSING `offer_type` - the exact shape of an unsupported perpetual claim. With the row-completeness guard off, the injected-row input stopped producing `unknown_matrix_rows`. Collateral controls confirm a healthy document still extracts under each patched state. `apps/api/app/ingest/adapters/html.py` was never modified.
- **The POSITIVE CONTROL, and PROOF that it is load-bearing [M]:** `test_the_z0_sweep_is_not_vacuous` shows the real classifier returns `Z0_TRUE_FREE` for `always_free` + `requires_card=False` + `has_paid_dependencies=False` + `hard_stop`. `test_the_positive_control_is_load_bearing` then BREAKS the thing that control guards - `app.classify.engine.SAFE_EXHAUSTION` monkeypatched to `frozenset()`, baseline asserted GREEN first, patched line printed - and shows the CONTROL goes red while the six-offer no-Z0 sweep stays green. That asymmetry is the whole argument: the sweep alone cannot distinguish a real refusal from a classifier that never says yes. A further test independently re-derives that the patch did not leak. Eight parametrised cases additionally prove each gate (card, paid dependencies, automatic billing, unknown card, unknown dependencies, unknown exhaustion, manual upgrade) can block a perpetual offer INDEPENDENTLY, with the all-clear case reaching Z0.
- **A prediction I got WRONG, and it was material [M]:** I wrote that the publication gate withholds all six Oracle offers BECAUSE of the quoted card requirement, in the integration-test docstring and in a code comment. **That is false.** MEASURED against real PostgreSQL: all six are held for review as "uncertain evidence" because their facts are pinned to PROSE and carry no numeric quota rows, so they fail the gate's `schema_complete` and `deterministic` hard conditions. The card's effect is on the zero-cost CLASS, not on publication. I discovered it when my own non-vacuity control asserted a card-required offer would fail to publish and it published instead - correctly labelled `Z1_BILLING_EXPOSURE`, because a Z1 offer is a legitimate catalogue entry that simply is not free. Corrected in the test module docstring, the inline comment, and `docs/PROVIDER_ADAPTERS.md`. The control is now much stronger for it: two candidates identical except `requires_card` both publish, and only the cleared one is labelled `Z0_TRUE_FREE`, so the Z0 label is demonstrably reachable through that exact path.
- **Three further errors, recorded [M]:** (1) my generator defaulted a missing `<title>` to a fabricated `<p>Oracle</p>`, which its own FIXTURE -> LIVE check caught immediately ("fixture blocks absent from live: ['Oracle']"); I added the two missing title needles AND made a defaulted title a hard refusal, because the mechanism, not my care, is what caught it. (2) `_block_containing` in the unit tests compared needles against raw markup without unescaping character references, so every quotation containing an apostrophe matched ZERO blocks - 14 tests red - caught by that helper's own exactly-once assertion rather than by inspection. (3) My first pipeline non-vacuity control asserted `published == 0` on BOTH sides, which is vacuous; it was replaced with the two-candidate control described above.
- **A near-miss I want on the record [M]:** `scripts/check.ps1` first reported Prettier and ESLint FAILING. Both were `'prettier' is not recognized` - npm dependencies were simply not installed in this fresh worktree. Reporting that as a check failure would have been wrong in the alarming direction. Installed and re-ran; the real Prettier failure underneath was 22 of my own fixture JSON files, formatted with the repository's own Prettier, and `source.*` stayed exempt.
- **Coverage declaration, honest in BOTH directions [M]:** NINE of fourteen categories are `offered_no_z0` with declared provenance - far more than any other provider here, because Oracle really does publish a comprehensive Always Free enumeration on two independent documents. FIVE are `unknown` and **ZERO are `not_offered`**: the enumeration omits containers, serverless, auth, CI/CD and AI inference, which is suggestive and is not proof, and Oracle's own FAQ says "As new Always Free services become available, you will automatically be able to use those as well". The `containers-app-hosting` rationale names the strongest counter-example against itself - "APEX Application Development" and "Content Management Starter Edition" are in the list and could arguably be read as application hosting - which is precisely why it is unknown. ZERO are `verified_free`. Each `offered_no_z0` QUOTES the allowance it rests on and states why no Z0 offer is produced; each `unknown` names what was actually probed.
- **Pages that could NOT be used, reported rather than worked around [M]:** `www.oracle.com/cloud/price-list/` served 120 tables of which exactly ONE is header-selectable, and that one is a PAID comparison table; `/cloud/compute/pricing/` and `/cloud/networking/load-balancing/pricing/` both REDIRECT to it, so they are one document and not three. `/database/nosql/pricing/` served 23 tables, one header-selectable and also paid, with no free-tier prose in its served HTML at all. `/cloud/storage/object-storage/pricing/` redirects to a client-rendered cost estimator carrying no allowance. Two candidate URLs returned HTTP 404. Unlike AWS's documentation host, every page actually used here is server-rendered and parses to real content.
- **Files changed:** `apps/api/app/ingest/adapters/profiles/oracle.py` (new), `config/examples/providers/oracle.example.yaml` (new), `tests/fixtures/ingest/oracle/html/**` (new; six live captures plus five synthetic negative scaffolds that declare their synthetic provenance), `tests/unit/test_adapter_oracle.py` (new), `tests/unit/test_oracle_coverage_config.py` (new), `tests/integration/test_ingest_oracle.py` (new), `scripts/url-allowlist.txt` (add `www.oracle.com`, `docs.oracle.com`), `docs/PROVIDER_ADAPTERS.md`, `tests/fixtures/ingest/README.md` (attribution), `.secrets.baseline` (refreshed only via `scripts/refresh_secrets_baseline.py`, and only AFTER `git add`: 42 -> 56 tracked files, 156 -> 231 entries), `agent-state/current_contract.json`, and this handoff. 43 files.
- **Tests and checks run [M]:** full suite **2465 passed, 2 skipped** against a REAL PostgreSQL 16 on a dedicated scratch database named **`atlas_f008_p6_oracle`**, created for this slice so the concurrent Azure slice's `atlas_f008_p5_azure` was not disturbed. The database demonstrably mattered: re-run with `DATABASE_URL` UNSET the same suite reports **2232 passed, 235 skipped**, so 233 tests ran only because a real Postgres was present. The only remaining skips are the two `test_stack_health` cases needing a live API URL. New tests: 191 unit extraction/safety, 26 coverage-config, 8 integration - the integration tests RAN, they did not skip. `scripts/check.ps1 -NodeAudit`: **ALL CHECKS PASSED** - Ruff lint, Ruff format, Pytest, Prettier, ESLint, Secrets baseline shape, Secret scan, URL host allowlist, all three Python dependency audits AND both Node audits. Exit codes read from bare invocations, never through a truncating pipeline.
- **Known limits, stated rather than hidden:** live page digests are NOT reproducible across fetches, so each capture pins STRUCTURE and says so; `sha256_original` is provenance only and `sha256_stored` is declared in the capture to be a tamper-evidence seal on the committed bytes, NOT a link to live. UNKNOWN and deliberately left so: `requires_card` on the OCI Always Free Resources and promotion-expiry documents (neither states any payment condition, and carrying the requirement across documents would be composition); `has_paid_dependencies` on every profile (no Oracle document in this slice states it in either direction). The Always Free Resources exhaustion behaviour is `request_rejected` taken from the ONE quota whose consequence Oracle states in a block of its own (the five-volume-backup limit); it is deliberately NOT generalised to every Always Free quota, and that is said in the module docstring.
- **Evaluator disposition:** pending - builder self-review only.
- **Protected state [M]:** blob-hash verified against base across 29 paths - zero diff to `apps/api/app/ingest/adapters/html.py` (`f1a68632326e`), `_common.py`, `profiles/__init__.py`, `fetch.py`, `runner.py`, `scan.py`, `reconcile.py`, `apps/api/app/classify/**`, `apps/api/app/publish/**`, `app/models/vocab.py`, `tests/support/fixtures.py`, `tests/support/source_scan.py`, `tests/unit/test_secrets_baseline.py`, `tests/unit/test_capture_sidecar.py`, `apps/web/**` (`package-lock.json` still `98f786c2bb0d`), `docker-compose.yml`, `.github/**`, `.prettierignore`, the secrets scripts, and every other provider's profile and config. `agent-state/feature_list.json` byte-identical at blob `154de1fef2ba`; **F008 stays `passes:false`**. The filtered comparison was run UNFILTERED first as a positive control: 43 files, all accounted for.
- **Boundary:** Draft PR only. Do not merge, do not flip any ledger flag.
- **Recommended next action:** fresh-context Level-2 evaluator against this contract. The two highest-value things to attack are (a) the card READING - decide independently whether "most users need ... a credit card to create an account" and the identity-verification block support `requires_card=True`, since that is what turns three perpetual offers from UNKNOWN into Z1; and (b) the two ABSENCE-based refusals, which are the structurally weakest claims in the slice and are labelled as such.

## 2026-08-15 - Builder - F008 P6 Oracle: prose-precision tightening (orchestrator-approved, pre-evaluation)

- **Why now [M]:** no evaluation had started, so changing the artefact cost nothing; the same edit after dispatch would have invalidated an evaluator's work.
- **The class, and that it is NOT review-only [M]:** "technically true, reliably read false" - a statement that is literally accurate but invites a false reading. A previous slice hit it four times and concluded an author structurally cannot self-detect it, because the author already knows the intended reading. Told the class BY NAME, I ran a deliberate pass against my own prose and found TWO. The remedy is therefore two layers, not one: name the class in the brief, require a self-review pass before reporting, and keep the fresh-context check for what that pass misses.
- **Tightening 1, serverless-functions [M]:** the rationale said neither Always Free enumeration lists a serverless-function allowance. True - but MEASURED block-wise, the word "serverless" DOES occur once, in the OCI block "Exadata Infrastructure Type: Serverless", which is the Always Free Autonomous AI Database's infrastructure type and not an allowance of any kind. An evaluator grepping serverless would find it and read a considered exclusion as an oversight. The hit is now named and dismissed in the rationale itself.
- **Tightening 2, containers-app-hosting [M]:** the rationale cited two documents and then said "the list does contain" both counter-examples. Singular and correct, but after naming two documents it reads as "both, on both". MEASURED block-wise: "APEX Application Development" appears on BOTH documents; "Content Management Starter Edition" appears on the FAQ ONLY. The rationale now says which document carries which.
- **Both disclosures are PINNED BY TEST [M]:** 	est_the_containers_rationale_says_which_document_carries_which_entry and 	est_the_serverless_rationale_names_and_dismisses_the_one_serverless_hit. A disclosure with no test is decoration and drifts back on the next edit.
- **Method that produced both [M]:** a line-based search cannot see prose wrapped across lines, so every coverage claim was re-derived BLOCK-WISE over the parsed live blocks - each block is a whole normalised string, so a substring test against it is inherently wrap-safe. That pass also re-confirmed, independently of how the claims were first written: price list 120 tables / exactly 1 header-selectable (a PAID comparison table); NoSQL pricing 23 tables / 1 selectable (also paid) with ZERO free-tier prose blocks; the duplicated Object Storage block occurring exactly twice and byte-identical; and the Pay As You Go question text in the <div class="cb105w3"> the collector does not capture.
- **Secrets baseline NOT refreshed, deliberately [M]:** running the refresh reported "56 -> 56 tracked file(s), 231 -> 231 entr(y/ies)" and produced a timestamp-only diff. That is churn, not a refresh, so it was reverted and the file verified byte-identical to the committed one by blob hash (5f2b35d747c8). No new file was added by this change.
- **Files changed:** config/examples/providers/oracle.example.yaml (two rationales), 	ests/unit/test_oracle_coverage_config.py (two new pinning tests), and this handoff.
- **Gates [M]:** Oracle tests 225 -> **227** passed (the two new pins). scripts/check.ps1 -NodeAudit: **ALL CHECKS PASSED**. Protected state re-verified intact across 29 paths by blob hash; gent-state/feature_list.json still 154de1fef2ba, F008 stays passes:false.
- **Evaluator disposition:** pending - builder self-review only.
- **Boundary:** Draft PR only. Do not merge, do not flip any ledger flag.

## 2026-08-15 - Builder - F008 P6 Oracle: Level-2 remediation F1-F4 (FAILED -> corrected)

- **F1, the blocking defect: a FALSE claim shipped in documentation [M].** oracle.py and docs/PROVIDER_ADAPTERS.md asserted as a general law that perpetual offers are never free ("three providers have now been measured and all three agree"). I did NOT take that on the evaluator's word: I re-derived it by driving the real classifier over every provider fixture in the repository. **REFUTED.** Of **14** perpetual (lways_free) offers across all **7** provider configurations: **5 classify Z0_TRUE_FREE, 5 Z1_BILLING_EXPOSURE, 4 UNKNOWN.** The five Z0 offers are GitHub Actions, Packages and Codespaces plus Cloudflare Pages and Workers.
- **The claim was wrong in BOTH directions at once [M]:** omission-favouring in its conclusion (a blanket "never free" under-reports genuinely free offers, which this product forbids exactly as much as over-claiming), AND under-counting its own support - GCP is a fourth provider whose perpetual offer is non-Z0 by the same utomatic_billing mechanism as AWS and Azure, already in the tree and unmentioned. One entailment slip surfacing as two opposite counting errors.
- **Scope is stated because the numbers are NOT interchangeable [M]:** repository-wide the Z0 count is 5 across two providers; **restricted to the six F008 providers it is 3, all from GitHub alone.** Cloudflare is **F005**, not an F008 provider. Writing "five" while discussing F008 would have been a new false claim inside the correction for the old one, so both figures and their scopes are written out and pinned by test.
- **What survives re-derivation as distinctively Oracle [M]:** measured across all 14 perpetual offers, AWS, Azure and GCP are each withheld by utomatic_billing on exhaustion, while **Oracle is the only provider whose perpetual offer is withheld by a quoted payment-card requirement.** The corrected prose says exactly that, and explicitly says Oracle is NOT the only provider whose perpetual offer is withheld.
- **F2, counts that read stronger than they were [M]:** each capture's live_reconciliation counted every RETAINED block and called them all "pinned". On five of six sources that overstated the guard by one, because the document <title> is retained as furniture but asserted by only one profile. Recomputed from each profile's own assertions: **24, 6, 7, 6, 5, 7** (was 24, 7, 8, 7, 6, 8). Retained-but-not-pinned blocks are now declared in their own field, and **five dead *_TITLE constants that no profile asserted were deleted** from the module and its __all__ - exported constants that look like evidence in a module whose whole discipline is pinned evidence.
- **F3, stale ledger [M]:** current_contract.json still carried pre-rebase figures. Re-derived: **2668 passed / 2 skipped** with PostgreSQL, **2426 / 244** without, so 242 tests run only because a real database is present; baseline **54 -> 68 files, 214 -> 289 entries**. The remediation itself is recorded there under level_2_remediation.
- **F4, my own tightening was imprecise in the same family [M]:** the containers rationale said "APEX Application Development" appears on BOTH documents. Literally true as a substring - and misleading. MEASURED: on the FAQ it is a standalone Always Free LIST ENTRY; on the OCI document the string occurs inside "Oracle APEX Application Development", one of the Autonomous AI Database's WORKLOAD TYPES, which is a different claim entirely. Corrected to state that both list entries appear on the FAQ only, and that the OCI occurrence is a workload type and is not counted.
- **Every corrected statement is pinned, and every pin PROVED LOAD-BEARING [M]:** three prose guards, each mutation-tested by reverting the prose to its pre-correction form, confirming the guard goes RED with the baseline asserted GREEN first, and restoring the file **byte-exactly, verified by git hash-object** - never by git diff --numstat. All three: baseline GREEN, mutated RED, restored byte-exact.
- **The refuted-universal guard forbids ASSERTION, not the string [M].** My first version banned the phrase outright and immediately went red against my own retraction, which quotes the claim in order to name it. A blanket ban would also push the next author into rewording the claim rather than dropping it. The guard now requires any occurrence to sit within a refuting context, which is the property actually wanted.
- **My own new test caught a substring bug in my own new test [M]:** collecting blocking conditions by substring reported GitHub and Vercel as card-blocked, because the engine's gate-4 string "Whether a payment card is required is unknown." CONTAINS the gate-3 string "A payment card is required." Fixed to exact set membership. That is the *stated* versus *unstated* distinction - the same one the absence/quotation reporting exists to keep apart - reappearing as a matching bug.
- **A heuristic of mine that was unreliable, corrected [M]:** I had been treating "same file and entry counts" as proof that a .secrets.baseline refresh was timestamp-only churn. It is not - digests and line numbers can move while the counts hold, which is exactly what happened once the captures gained fields. The reliable test is the diff with the generated_at line excluded, and that is what I used: 134 substantive lines, so the refresh was real.
- **Files changed:** pps/api/app/ingest/adapters/profiles/oracle.py, docs/PROVIDER_ADAPTERS.md, config/examples/providers/oracle.example.yaml, six 	ests/fixtures/ingest/oracle/html/*/capture.json, 	ests/unit/test_adapter_oracle.py, 	ests/unit/test_oracle_coverage_config.py, .secrets.baseline, gent-state/current_contract.json, and this handoff.
- **Gates [M]:** Oracle tests 227 -> **238**. Full suite **2668 passed / 2 skipped** against real PostgreSQL on tlas_f008_p6_oracle. scripts/check.ps1 -NodeAudit: **ALL CHECKS PASSED**. Protected state blob-hash intact across 29 paths; gent-state/feature_list.json still 154de1fef2ba, **F008 stays passes:false**. No Oracle verdict, coverage state or extracted fact changed.
...
## 2026-08-24 - Builder - F001 follow-up: close the apps/web CI gap (install, type-check, test, build)

- **The gap, re-derived not inherited [M]:** `.github/workflows/ci.yml` had four jobs. The `node` job ran `npm ci` with NO `working-directory` (repository root), and root `package.json` declares no `workspaces`, so that install could not reach `apps/web`. The ONLY step in the whole workflow scoped to `apps/web` was `npm audit --audit-level=high`, which reads the lockfile and installs nothing. Consequence: `tsc -b`, `vitest run` (MEASURED 9 files / 110 tests) and `vite build` - all declared by `apps/web` and all executed by `apps/web/Dockerfile` to produce the assets that ship - ran NOWHERE. A TypeScript error or a failing web test merged green, and not one of the 336 declared `integrity` digests was enforced by any workflow, because only `npm ci` checks them.
- **Design, and why NOT the obvious alternative [M]:** a new parallel `web` job rather than more steps on `node`. MEASURED from the real run - `python` 83s, `dependencies` 38s, `secrets` 35s, **`web` 28s**, `node` 14s - so the new job is 55s inside the existing critical path and adds ZERO wall-clock. Folding it into `node` would have serialised it behind Prettier/ESLint instead. MEASURED and it killed the one counter-argument: `gh api .../branches/main/protection` returns 404 `Branch not protected` and `.../rulesets` returns `[]`, so there is NO required status check to preserve and a new job name costs nothing. No dependency cache: the workflow uses `setup-node@v4` with no `cache:` key anywhere, and shaving an off-critical-path install shortens nobody's wait.
- **Why type-check is its own step [M]:** `build` is `tsc -b && vite build`. Folded together, the type gate and the build gate are the same step and NEITHER can be shown to fail on its own. Hence a new `typecheck` script in `apps/web/package.json`. That it cannot desync the lockfile was VERIFIED, not assumed: the lockfile's root entry carries `name, version, license, dependencies, devDependencies, engines` and has no `scripts` key.
- **The conditional steps are load-bearing, and MEASURED to be so [M]:** post-install steps carry `if: !cancelled() && steps.web_install.outcome == 'success'`, following the rule the `dependencies` job already states. Vitest transforms TypeScript with esbuild, which strips types WITHOUT checking them, so a type error does not fail a test run - CI PROOF A shows exactly that (typecheck failure, Vitest SUCCESS in the same job). Without the conditional a type error would mask a genuinely failing test. Conditioned on the install rather than `!cancelled()` alone so a failed `npm ci` does not emit "binary not found" as four findings.
- **Every gate PROVEN red-capable, predictions recorded BEFORE each run [M].** Local, each mutation printing its patched line and asserting its anchor matched EXACTLY once: type error predicted `typecheck!=0, test=0, build!=0` -> got `2, 0, 2`; failing assertion predicted `0, !=0, 0` -> got `0, 1, 0`; build-only break (`index.html` entry, invisible to tsc) predicted `0, 0, !=0` -> got `0, 0, 1` with `[vite:build-html] Failed to resolve`. In REAL CI on scratch PR #76: PROOF A (type error) -> install success, typecheck **failure**, Vitest success, build **failure**; PROOF B (assertion + html) -> install success, typecheck success, Vitest **failure** (`AssertionError: expected 'Unknown' to be 'CI GATE PROBE - MUST FAIL'`, `1 failed | 109 passed`), build **failure**. In both runs the other four jobs stayed green, so the failure is attributable.
- **EINTEGRITY proven LOCALLY, deliberately not pushed [M]:** one `integrity` digest corrupted by a single base64 character -> `npm ci` exit 1, `npm error code EINTEGRITY`, "integrity checksum failed". The CONTROL is what makes it conclusive: the PRISTINE lockfile installs cleanly on the same machine (336 packages), so the failure is discrimination, not a blanket break. Committing a corrupted lockfile to a public repository, even transiently on a deleted branch, was judged the worse trade.
- **Lockfile boundary held [M]:** `apps/web/package-lock.json` blob `98f786c2bb0d2e9152d66e6b7ba7b5c3b54b580e` before AND after a full `npm ci` and after the corruption test - `npm ci` does not rewrite it. Host distribution re-derived after all work: 336 `resolved` URLs, 336 on `registry.npmjs.org`, zero elsewhere.
- **FINDING 1, reported not repaired - the integrity digests are SHA-1 [M].** Re-derived by two independent methods (regex over raw text, then JSON parse): `apps/web/package-lock.json` is **335 sha1 / 1 sha512** (the lone sha512 is `postcss`). Root `package-lock.json` is 76 sha512 / 11 sha1 for contrast. Traced through history: the file was BORN this way in `9f43e07` (F002 slice 3) at 336/0, so it is not a recent regression. This qualifies my own PR's security claim honestly - the new gate enforces 336 real digests, but 335 of them are not collision-resistant. Fixing it means regenerating the lockfile somewhere with direct public-npm access; it cannot be done on this machine without rewriting `resolved` to an internal feed host.
- **FINDING 2, reported not repaired - the lint gate structurally excludes the entire frontend [M].** Root `.prettierignore` contains the line `apps/web/` and root `eslint.config.js` has `ignores: [..., "apps/web/"]`. So the job named "Node format and lint" never lints or format-checks a single line of `apps/web`. `apps/web` declares its own `lint` and `format:check`, which run nowhere in CI. MEASURED: both currently exit 0, so this is a latent gap rather than a live break. It is the direct sibling of the gap this slice closed and is the recommended next increment; it was left out to keep every shipped gate one I had proven red-capable.
- **FINDING 3 - a stale figure inside ci.yml's own comment [M].** The `python` job comment says the DATABASE_URL-gated layer is "162 tests". The Oracle slice MEASURED 2668 passed / 2 skipped with PostgreSQL against 2426 / 244 without, i.e. **242** tests that run only when a database is present. The comment reads as a present-tense description of the system and is now about 80 tests stale.
- **CHECKED AND CLEARED, a negative result worth recording [M]:** `concurrency: cancel-in-progress: true` keyed on `github.ref` could in principle cancel a `main` run rather than let it pass, which would make "main is green" partly an artefact of cancellation. MEASURED across the last 20 `main` runs: ZERO `cancelled` conclusions. Not an issue. Noted alongside it: 10 of those 20 pushes concluded `failure`, an older red streak since resolved - the four most recent are green, so "main is currently green" is true of the current head, not of main's recent history.
- **My own errors, disclosed [M]:** (1) the first proof-battery script set `Continue = "Stop"` while redirecting native output with `*>`; PowerShell surfaces a native command's stderr as a NativeCommandError record and under "Stop" that TERMINATES. Vitest writes its failure report to stderr, so the script aborted the instant the control fired correctly - and aborted with the mutation still applied to the working tree. Caught by reading the output rather than the exit status, restored immediately and blob-verified. The harness broke BECAUSE the control worked. (2) I initially reasoned that extending the existing `node` job was safer because renaming a required check is risky; that premise was never checked and turned out to be false - there is no branch protection at all. The design survived, but for a different reason than the one I started with, and I would have shipped the right answer with a wrong justification had I not queried the API.
- **Files changed:** `.github/workflows/ci.yml` (one inserted job), `apps/web/package.json` (one added script), `agent-state/current_contract.json`, and this entry.
- **No existing check weakened, proven structurally [M]:** removing ONLY the inserted block from `ci.yml` reproduces main's blob `4f80c771f9562be9df687c5cec918b36548e9d60` EXACTLY, so the four pre-existing jobs are byte-unchanged. Verified again after rebasing onto `96b2cb8`.
- **Gates [M]:** full CI green on the PR branch (all five jobs). Locally: secrets route guard + repo baseline **161 passed**; `check_secrets_baseline.py` 68 files / 289 entries OK; `check_urls.py` 1243 URLs / 56 hosts all allowlisted; root `format:check` and root `lint` exit 0; `apps/web` typecheck/test/build all green. Protected state blob-verified IDENTICAL to main across feature_list.json, both lockfiles, .secrets.baseline, source_scan.py, test_secrets_baseline.py and check.ps1. `.secrets.baseline` deliberately NOT refreshed: no file was added and the baseline carries no entry for `ci.yml`, so a refresh would have produced only a spurious timestamp.
- **Evaluator disposition:** pending - fresh-context Level 2.
- **Boundary:** Draft PR only. Do not merge, do not flip any ledger flag. `agent-state/feature_list.json` untouched.
## 2026-08-24 - Builder - F001 guard-symmetry audit: both failure directions, and the two gaps worth closing

- **Objective:** every guard has two failure directions - wrongly ACCEPT (it misses the thing it exists to catch) and wrongly REJECT (it fires on correct work) - and this repository routinely instruments only the direction that motivated the guard. Enumerate the guards this repository OWNS by SEARCHING THE TREE, state both directions for each, MEASURE which direction is actually tested, close the highest-value gaps, and file the rest with reasoning.
- **Contract:** `agent-state/current_contract.json` (F001 / `guard-symmetry-audit`), written before any implementation. Base `5a3fb24`, re-derived with `git ls-remote` at dispatch and unchanged throughout.
- **Baseline, reproduced rather than assumed [M]:** `python -m pytest -q` with `DATABASE_URL` unset gave **2426 passed, 244 skipped**, which independently reproduces the figure in the previous handoff. Two derivations agreeing is the only reason to trust either.

### The inventory, derived by search

Fifteen repository-owned guards were found by searching `scripts/`, `tests/`, `tests/support/`, `.github/` and `apps/api/app/`. The full table with both directions stated for each is in the pull request body. The measured summary:

- **Both directions already tested, and tested well (6):** the secrets-baseline shape/directional validator (`scripts/check_secrets_baseline.py`), the executable-content route guard (`tests/support/source_scan.py` + `tests/unit/test_secrets_baseline.py`), the capture-sidecar and Prettier-exemption guards (`tests/unit/test_capture_sidecar.py`), the AST live-fetcher guard, the dependency-pin sync guard, and the provider coverage-declaration guards. These are exemplary: `test_legitimate_in_place_refresh_passes`, `test_growth_passes`, `test_generated_at_churn_never_fails`, `test_legitimate_restructurings_still_pass` and `test_the_prettier_exemption_is_scoped_to_captures_and_stays_scoped` are all explicit false-alarm controls. The premise that this repository under-instruments the reject direction is **only partly true, and where it is false it is emphatically false** - said here because the brief asserted it and the measurement disagrees in six of fifteen cases.
- **One direction tested (7):** listed as follow-ups below.
- **NEITHER direction tested (1):** `scripts/check_urls.py`. This was the finding.

### GAP 1 - the disclosure guard had no tests at all

`scripts/check_urls.py` is what keeps an internal package-feed hostname out of a public repository, and CI gates on it. **Measured: no test file in the tree imported it, referenced it or invoked it.** It was the only build-gating check this repository owns with zero coverage in either direction - and it is simultaneously the guard with a *designed* false-alarm behaviour (`scripts/url-allowlist.txt` states that a dependency bump introducing a new funding host is meant to fail the check), so both directions are load-bearing.

`tests/unit/test_url_allowlist.py` is new: **20 tests**, grouped by direction and labelled as such.

- **Wrongly REJECT (7):** the committed tree passes end to end; every host in the tree is allowlisted; port / case / userinfo / trailing-dot / IPv6 normalisation never turns a good host bad; suffix rules cover subdomains; a redaction marker yields no host, so a redacted incident report stays committable - a guard that rejected its own redaction marker would be arguing for the disclosure it exists to prevent; `--list` never fails a build; allowlist parsing ignores comments.
- **Wrongly ACCEPT (5):** an unlisted host exits 1; a lockfile `resolved` URL is in scope (the stated highest-value target); a lookalike domain is not swallowed by a suffix rule; userinfo cannot disguise the host after the `@`; no allowlist entry admits an arbitrary host.
- **Two structural blind spots, PINNED not repaired (2):** measured, an authority written with JSON-escaped separators is invisible to `URL_RE`, and one NUL byte in the first 8 KiB skips the **entire** file - which a UTF-16-encoded text file triggers, and UTF-16 is an ordinary Windows artefact. **Neither is fixed here**: changing what a security control matches is a behaviour change to a security control and belongs to the owner, and the brief is explicit that a guard which cannot hold both directions is a finding rather than something to trade away. What the tests do instead is convert both from SILENT holes into MONITORED ones - the limit is asserted to be exactly where the docstring says, and the tree is asserted to contain nothing that exercises it (measured: 0 and 0, over 639 tracked files, byte-wise over whole files so a wrapped line cannot hide). The NUL check is scoped to text suffixes **on purpose**: asserting "no file is ever skipped" would redden the build the day someone commits a screenshot, which is precisely the false alarm this slice exists to argue against.

### GAP 2 - a guard that was provably incapable of detecting what it named

`tests/unit/test_adviser_no_llm_import.py` asserted that the deterministic adviser core does not import `app.adviser.llm`. Every assertion in it had the form "nothing was found" - **which is exactly what a probe that cannot find anything also reports.** Nothing in the repository demonstrated the probe could detect a leak.

- **Positive control added:** the identical probe, run the same way, against `app.adviser.router` - measured to pull in **7** `app.adviser.llm` modules - and required to report them.
- **The demonstration is executed, not argued [M]:** mutation M4 breaks the leak predicate so it can never match. The **pre-existing** test `test_core_does_not_import_llm_package` stays **GREEN**; the new positive control goes **RED**. That is the old test's incapability, reproduced on demand.
- **Coverage drift closed:** the probe named 4 modules; the deterministic core has 8. `explain`, `portability`, `quota_math` and `export` were unguarded. All four were measured clean first, so adding them carries a measured-zero risk of reddening.
- **Wrongly-REJECT direction added too:** a probe that fails to run (missing module, bad cwd) previously reported itself as "the core imported the LLM package", sending the reader to the wrong code. The assertions now key off `stdout` rather than the exit code, and `test_a_probe_failure_is_not_reported_as_a_leak` measures that the two outcomes stay distinguishable.

### Mutation results - every prediction recorded BEFORE the run

All six matched. Each asserts its anchor matched **exactly once** and prints the patched line, because a mutation that does not apply looks identical to a control that does not fire.

| # | What was broken | Predicted | Observed |
|---|---|---|---|
| M1 | suffix rule -> substring match | RED | RED |
| M2 | userinfo stripping takes the wrong side of `@` | RED | RED |
| M3 | violations `return 1` -> `return 0` | RED (x2) | RED (x2) |
| M6 | `is_allowed` rejects everything (false-alarm direction) | RED (x2) | RED (x2) |
| M4 | leak predicate can never match | RED **and** original test GREEN | RED and GREEN |
| M5 | probe list names a missing module | RED | RED |

Every file restored **byte-exactly**, proved by `git hash-object` (`scripts/check_urls.py` 9ce16dd08812 -> 9ce16dd08812; the adviser guard 08f356c45e30 -> 08f356c45e30), never by `git diff --numstat`. The whole set was re-run against the FINAL file state after the tests were edited, because the first run no longer covered them.

- **A nuance in the restoration proof, stated because it matters [M]:** `git hash-object <path>` applies `.gitattributes` filters, so it normalises CRLF before hashing and cannot by itself prove *byte* identity. The harness restores from the original bytes directly, and protected state was additionally verified by RAW byte comparison against `git show HEAD:<path>`. Two mechanisms that cannot fail the same way, which is the only kind of agreement worth anything.

### Deferred, with reasoning

- **`check_urls.py` JSON-escape and UTF-16 blind spots** - a real fix (widen `URL_RE`; decode UTF-16 rather than skipping). Deferred because it changes a security control's matching rules. Both are latent today and now monitored.
- **`test_sidecars_carry_no_credentials`** - substring match on `password`/`apikey`/`access_token` over the whole sidecar. A legitimate provider documentation URL containing `password` would red the build. Real false-alarm surface, untested; low likelihood today.
- **`test_no_live_fetcher_in_tests`** - the accept direction is excellent (AST, not substring). The reject direction is untested: `test_the_sanctioned_exception_is_still_loopback_only` requires the literal `allow_loopback=True`, so a legitimate refactor to a variable reds it.
- **`test_requirements_sync`** - exact string equality on pins, so a whitespace-only respelling (`foo == 1.0`) fails on a semantically identical file.
- **`test_test_only_pins_never_reach_a_shipped_image`** - I first read this as a name overstating its assertion (it checks only `cryptography`, not "test-only pins"). **Re-derived, and I was wrong**: any other dev pin leaking is already caught by the two equality/subset tests, and the cryptography case is the one they cannot catch. Recorded because nearly filing it was the error, not the guard.
- **`test_repo_baseline`** - both directions are trivially symmetric; nothing to add.

### Every error I made

1. **Assumed API symmetry between two sibling scripts.** Wrote `guard.main(list(argv))` because `check_secrets_baseline.main(argv)` takes a list. `check_urls.main()` takes none and reads `sys.argv`. Five tests failed. Inferring instead of measuring - the exact failure this slice audits. The difference is now written into the helper's docstring.
2. **Wrote test data that tripped the guard under test.** `host.invalid-tld` in a UTF-16 sample was a real unlisted host in a soon-to-be-tracked file. Caught ONLY by staging the file and re-deriving `check_urls.py`; before staging it was green, because `git ls-files` does not see untracked files. Fixed to `.invalid` (RFC 2606, already allowlisted).
3. **Wrote a self-defeating assertion, then half-fixed it.** The test asserting no tracked file contains a JSON-escaped scheme contained that byte sequence twice - once in a sample and once in the fixture's own search needle. I fixed the sample, re-ran, and it still failed on the needle. Both are now assembled from parts, and the file is asserted to contain zero occurrences of either needle.
4. **Mis-stated a standard in a comment.** Called `.invalid-tld` "reserved by RFC 2606". It is not; RFC 2606 reserves `.test`, `.example`, `.invalid`, `.localhost`, and all four are already allowlisted - which is exactly why the unlisted host cannot be one of them. Comment corrected to say what is actually true.
5. **A cosmetic flaw in my own harness output.** For M6 the printed "before" and "after" lines are identical, because the replacement begins with the anchor line and the harness prints the line containing the match start. The mutation did apply (both target tests went RED, and the harness asserts the text changed), but the display does not show it. Reported rather than quietly ignored, since "the mutation looks like it did not apply" is the precise trap being guarded against.
6. **Used `Measure-Object -Line` for file lengths early on.** It does not count blank lines (136 vs 164 for one file). Harmless here, but it was a wrong number I briefly believed.

- **Files changed:** `tests/unit/test_url_allowlist.py` (new, 20 tests), `tests/unit/test_adviser_no_llm_import.py` (positive control, failure-distinguishability, probe-list integrity, 4 core modules added: 1 test -> 4), `agent-state/current_contract.json`, and this handoff. **Four files.** No application module, no configuration, no fixture, no provider profile.
- **Tests and checks run [M]:** full suite against a REAL PostgreSQL 16 on a dedicated scratch database `atlas_guard_audit`: **2691 passed, 2 skipped**. The base commit recorded 2668 passed / 2 skipped; this slice adds 20 + 3 = 23 tests, and 2668 + 23 = 2691 exactly, so the count is accounted for rather than merely plausible. Without `DATABASE_URL`: 2426 -> **2449 passed, 244 skipped**. `ruff check .` and `ruff format --check .` clean over 228 files. `python scripts/check_urls.py`: passed, 1244 URLs / 57 distinct hosts (up from 1243 / 56 - the single delta is the `.invalid` sample in the new test, accounted for). `python scripts/check_secrets_baseline.py`: passed, 68 tracked files / 289 entries, nothing lost. **Every exit code read from a bare invocation**, never through `Select-Object -First/-Last`, which corrupts `$LASTEXITCODE`. `scripts/check.ps1` was deliberately NOT used as the oracle: its exit 1 conflates "a secret was found" with an unstaged baseline and a missing toolchain, and another builder holds that file.
- **Protected state [M]:** verified identical to `HEAD` by BOTH filtered blob hash and raw byte comparison - `agent-state/feature_list.json` (`154de1fef2ba`), `tests/support/source_scan.py` (`3a86f03aeb17`), `tests/unit/test_secrets_baseline.py` (`183619e00375`), `apps/web/package-lock.json` (`98f786c2bb0d`, every `resolved` still on the public registry), `.github/workflows/ci.yml` (`4f80c771f956`), `scripts/check.ps1` (`f68e2b972997`), `scripts/check.sh` (`4543d6f11eab`), `scripts/check_urls.py` (`9ce16dd08812`) and `scripts/url-allowlist.txt` (`73a899df8517`). **No guard was weakened, and the allowlist gained no entry.** `agent-state/progress.md` is append-only, proved by RAW BYTE PREFIX: the first 671720 bytes are byte-identical to `HEAD` (sha256 `e394d6ad58cee495`), never by numstat.
- **Known limits, stated rather than hidden:** the two `check_urls.py` blind spots are monitored, not closed. The `.secrets.baseline` was NOT refreshed - no new file needed an entry, and refreshing to produce a timestamp-only diff is churn. The new URL tests add roughly 35-45 seconds to the suite (they walk 639 tracked files three times); that is a real cost, paid for the first coverage the disclosure guard has ever had.
- **Evaluator disposition:** pending - builder self-review only.
...
## 2026-08-24 — F008 follow-up: Observations B and C (staleness + absence-of-ingest materiality)

- **Increment:** `f008-obsB-obsC-staleness-and-absence-materiality`. Branch `stsyg-staleness-materiality-rulings`, rebased three times as `origin/main` moved beneath it (`5a3fb24` → `78d2c73` → `699481b` → **`a554883`**). On each rebase `agent-state/current_contract.json` was taken wholesale and `agent-state/progress.md` rebuilt as the new `main` blob VERBATIM plus this block, re-proved by raw byte prefix each time. Contract written **before** implementation; `required_evaluation_level: 2`.
- **Two owner questions, both answered from measurement rather than memory.** Both rulings were re-derived against the merged tree by running the offline `FixtureFetcher` ingest **and** the publication gate for all seven provider configs into an isolated PostgreSQL (`ftatlas-f008-obsbc`, host port 55471 — never the shared dev stack, four builders are running), then serialising the real category matrix through `read_api.service.serialize_category_matrix` at the current clock and again at `now + 5y`.
- **THE BRIEF WAS WRONG ABOUT obsC, AND THE MEASUREMENT SAYS SO [M].** The brief stated that Wave 3 landing means "the stated reason for the deferral no longer holds". Wave 3 landed *adapters*; it did not land *publications*. Measured: **aws 0, azure 0, gcp 0, oracle 0, vercel 0** published offers reaching a canonical category — only `github` (4) and `cloudflare` (2) publish anything at all, because the publication gate correctly withholds the rest. So **93 of 98** provider×category pairs still derive `unknown`, and the broad "absence of ingest is material" rule the S2 deferral rejected would still floor the queue at 93 items. The 14×N floor did **not** go away.
- **Measured queue cost, obsC (98 pairs total):** every derived-`unknown` material = **93**; declared-non-`unknown` over derived `unknown` = **51**; declared `offered_no_z0` = **36**; `not_offered` = **13**; declared `verified_free` = **2**. **Decision: make only the last one material** (`cloudflare/networking-cdn-dns`, `github/ai-inference-embeddings`). **The measurement changed the design** — the broad reading of "make it material" would have shipped 93 pending items.
- **Measured queue cost, obsB:** **0 today**, **5 at snapshot expiry**. "0 today" is the wrong number for a rule about aged evidence, so it was re-taken at `now + 5y`: `cloudflare/containers-app-hosting`, `cloudflare/serverless-functions`, `github/cicd-source-control`, `github/object-file-storage`, `github/secrets-config-devtools` — i.e. **every published free claim the catalogue has**, all five still displaying `verified_free` on `main` with **zero** review items. The historical "queue volume" objection was worth 5 items.
- **Mechanism for obsB: (c) both.** `(verified_free, stale)` is material *and* stops displaying `verified_free` — it displays `stale`, not the generic `conflicting` collapse, because nothing disagrees; the snapshot expired, which is what `stale` already means and what `apps/web/src/catalogue/vocab.ts` already renders. **Zero web changes needed.**
- **The brief's worry about mechanism (b) was inverted here, and the reason matters [M].** `effective_state` already collapsed **every** material mismatch to `conflicting`, so raising a contradiction necessarily changed the display. Materiality and display were **fused**, and that fusion is what made obsC unfixable without collateral damage. Splitting them via a two-entry read-only table is the actual fix.
- **obsC raises the item but does NOT downgrade the display — a disclosed decision [M].** obsB's evidence **expired**; obsC's declaration is *provenance-backed* (the loader rejects an offered category with no `source`) and an absent publication refutes nothing when the gate withholds by design. Downgrading would suppress a true free claim for an ingest-coverage reason and would have moved two real merged-provider verdicts.
- **ANSWER TO "can an expired free tier still reach a user as free through ANY path?" — NO, IT CANNOT BE CLAIMED CLOSED, AND I MEASURED IT RATHER THAN ASSERTING IT [M].** I seeded one provider with a published `Z0_TRUE_FREE` offer whose backing snapshot is 400 days past a `daily` window, declared `verified_free`, reconciled (1 pending review item, `verified_free` vs `stale`), and then hit **every** public catalogue route through `TestClient` on the real clock. Results — `GET /catalogue/categories` → `state: "stale"` (**closed by this slice**); but `GET /catalogue/providers/{slug}/category-states`, `GET /catalogue/providers/{slug}/offers`, `GET /catalogue/offers/{id}` and `GET /catalogue/search` **all still return `zero_cost_class: "Z0_TRUE_FREE"` with `confidence_label: "high"`**, and `GET /catalogue/providers/{slug}` reports **`freshness: 0.9`**. `serialize_category_states` and `serialize_offer_summaries` read `offer.zero_cost_class` straight off the row and consult **no** staleness signal; the freshness number is frozen into `material_facts` at publication and never decays. **This slice closes the coverage-declaration path, which is what obsB named; it does not close the offer-level Z0 path.** Stated plainly rather than presented as closed. On caching specifically: there is **no** server-side response cache, `ETag` or `Cache-Control` on any catalogue route (the only `no-store` is on the adviser, and the `lru_cache` uses are settings/engine/store singletons, not payloads), and the web app is a Vite SPA with no prerender — so there is no cached or pre-rendered path, only the live serialisers above.
- **Things that made me doubt claims OUTSIDE my slice [M].** (i) The stored `freshness` confidence signal is computed at publication and never decays, so a provider summary can advertise `freshness: 0.9` on evidence 400 days past a daily window — an affirmative staleness claim that is simply false. (ii) A coverage **declaration**'s own backing source is never freshness-checked: `fetch_stale_offer_version_ids` only inspects evidence attached to a published `offer_version`, and `derive_coverage_state`'s `stale` branch additionally requires `published_offer_count > 0`, so a declaration-only free claim can never derive `stale` at all. (iii) Five of the six merged providers publish **zero** offers, so any reading of F008 as "provider expansion delivered a catalogue" is not supported by the data — the adapters and evidence exist, the published catalogue is 6 offers.
- **Both directions, every guard.** Unsafe: a stale free claim is material, stops displaying free, and is **not served as free over HTTP**; an uncorroborated free claim is material. Safe: a **fresh** free claim over its own published Z0 offer is untouched at the pure, ORM **and** HTTP level (the controls differ from the failing cases in exactly one value — the snapshot's `fetched_at`); `offered_no_z0`/`not_offered`/`unknown` over a derived `unknown` are not flagged; every declared hedge stays unflagged; all **11** pre-existing material pairs are enumerated and re-asserted.
- **Before/after verdicts, all six merged providers (+ cloudflare) [M].** At current freshness across **98** pairs: `declared` changed **0**, `derived` changed **0**, **`displayed` changed 0**; only the `mismatch` flag moves, on exactly the two obsC pairs. At expiry: `derived` 0, `displayed` on exactly the five obsB pairs, `mismatch` 5 + 2 = 7. **No real verdict moved.** Reproduced identically after every rebase.
- **Mutation battery: 13 mutations, each with its OWN pytest exit code, predictions recorded BEFORE the first run, each anchor asserted to match EXACTLY ONCE with the patched line printed [M].** M1–M7 cover the rules (exit 1 each); C0, a comment-only control, exits **0**, so the battery is not trivially red. Auditing *which assertion each mutation actually killed* left five gates with no killer at all, so five **step-level** mutations were added: M8 invert the staleness derivation, M9 mark every backed version expired, M10 let staleness unseat an `offered_no_z0` claim, M11 drop a pre-existing material pair, M12 make the display table a plain `dict`. Final run: **13/13 MATCH**. Every target restored byte-exactly, proven by `git hash-object` — `read_api/coverage.py` → `69be03c25404fb736f5bb300da9cce5a82a38bc0`, `read_api/queries.py` → `56b68add6cae29bb69451160981319d81fa55ef4`, `tests/support/coverage.py` → `3b722de03ffbc3b1bdf4cb7ba346ae9ab1b54a65`.
- **A prediction of mine was wrong twice, and the code was right both times [M].** (1) M7's first prediction named three failures that never fired — correctly, because all three route through the DB-free helper *this slice* changed to skip unasserted categories. (2) After adding the three HTTP tests, M1–M5 each produced **extra** failures I had not predicted (`missing=[]` in every case — the guards fired *more* than predicted, never less). Both corrected, causes recorded in the harness, re-run: MATCH. A prediction written against code you have just replaced is not a prediction.
- **CI silently did not run, and that is a finding in its own right [M].** PR #79 initially had **zero** GitHub Actions runs — `gh run list --branch` empty and `check-runs` reporting only GitGuardian, an external app. Actions were enabled repo-wide and four sibling PRs triggered normally the same hour. **Closing and reopening the PR did NOT fix it**; only a **new head commit** (the rebase force-push) fired `synchronize`. All five jobs then appeared and completed **success** by name: `Python lint, format, tests`, `Node format and lint`, `Web type-check, tests, and build`, `Secret scan`, `Dependency audit`. Absence of a red signal was not evidence of a green one, and only a by-name check would have caught it.
- **A `main` baseline run of mine went red, and re-derivation proved it a flake [M].** `tests/unit/test_ingest_fetch.py::test_redirect_to_private_host_rejected_mid_chain` failed once on a **pristine `main`** worktree while two full suites and four builders shared the machine; it passes in isolation and passed on the clean re-run. Reported rather than quietly re-run, because a one-off red on untouched code is exactly what a real regression looks like at first glance.
- **A real schema constraint caught a fixture of mine [M]:** seeding a `not_offered` declaration without a rationale violates `ck_provider_category_coverage_not_offered_requires_rationale` — the schema enforcing the same rule the product does.
- **Files changed:** `apps/api/app/read_api/coverage.py`, `apps/api/app/read_api/schemas.py`, `apps/api/app/ingest/reconcile_coverage.py`, `tests/support/coverage.py`, `tests/unit/test_coverage_derive.py`, `tests/unit/test_coverage_materiality_rulings.py` (new), `tests/integration/test_coverage_review_queue.py`, `agent-state/current_contract.json`, and this handoff.
- **Gates, all re-derived against `a554883` [M]:** `ruff check` + `ruff format --check` clean. Offline pytest **2481 passed / 254 skipped** vs a `main` baseline of **2449 / 244**; live-DB pytest on the isolated 55471 stack **2733 passed / 2 skipped** vs a `main` baseline of **2691 / 2** — both baselines measured by me in a detached worktree at `a554883`. **+42 tests, none lost** (32 that also run offline, 10 DB-gated, which is exactly the 244 → 254 skip rise). `scripts/check.ps1` — the **rewritten** #80 version — **ALL CHECKS PASSED, exit 0**. `.secrets.baseline` refresh after `git add` reported CHANGED but the diff excluding `generated_at` is **0 substantive lines**, so the churn was timestamp-only and was reverted rather than committed (four builders share that file; the checker explicitly accepts the spliced form). No migration (head stays `0011`); `derive_coverage_state` and its precedence unchanged.
- **Attestations.** Did **NOT** merge; DRAFT PR only. `agent-state/feature_list.json` untouched (**F008 stays `passes:false`**) and still blob `154de1fef2ba20f587c9ec2d1302ebe2bfb5bfa1`. `tests/support/source_scan.py`, `tests/unit/test_secrets_baseline.py`, `apps/web/package-lock.json`, `.github/workflows/ci.yml`, `scripts/check.ps1`, `scripts/check.sh`, `apps/web/`, `migrations/`, `config/`, `agent-state/evaluation.json` and `agent-state/evaluations/`, and every ingest adapter: **zero diff vs `origin/main`**, blob-verified. No pre-existing test weakened; the two S2 tests encoding the deferral the owner re-decided were **narrowed, not deleted**, and the flood guard still holds for 91 of the 93 derived-`unknown` pairs.
...
## 2026-08-24 - Builder - F008 S4 prereq: two prose-only assertion rules become machinery

- **Base [M]:** re-derived with `git ls-remote origin main` = `5a3fb24debb502544a0aa952ff39a7d306970a85`, equal to this worktree's HEAD and to the SHA the brief stated. Confirmed rather than trusted. Baseline before starting, offline: **2426 passed, 244 skipped**, exit 0. **Rebased mid-slice** onto `78d2c736c80a369a20f21f00cdf61d80108d34e7` (PRs #74 and #75) after `main` moved; every measurement below was re-derived against the new base.
- **What was actually broken, read rather than assumed [M]:** value validation in `HtmlExtractionProfile.__post_init__` was `allowed = _ASSERTION_CLOSED_VALUES.get(assertion.field)` followed by `if allowed is not None`. A field name the vocabulary did not know **skipped validation entirely** - no warning, no log. That is the same fall-through shape this repository has already been bitten by once, and `tests/unit/test_adapter_html_assertions.py`'s own docstring says so.
- **Why it was worse than "unvalidated" [M]:** `app/publish/revalidate.py` `NON_QUOTA_FIELDS` is the registry of reserved semantic fact fields and `revalidate_quotas` treats **every other fact key as a quota metric**. So a mistyped material condition was not merely unchecked - it was **published as a quota row** while the condition its name resembles stayed absent. Two failures at once, neither of them loud.
- **A hazard nobody had named [M]:** `_rejected` writes `{"error", "detail"}` and `validate()` treats any candidate carrying `error` as rejected. Nothing stopped a profile pinning `error`, `detail`, `provider` or `quotas`, so a profile could forge or mask a rejection. Those four are now refused outright.
- **The design question, MEASURED before choosing [M]:** an enumerated closed list of every fact field is NOT the design and would turn all six providers red. Across the 35 registered HTML profiles there are **96 distinct assertion field names, 102 matrix field names and 23 column field names**. Only the RESERVED half is bounded (16 names); quota metrics are open by construction. The registry therefore admits a field as *either* a registered reserved field with a declared value rule *or* a quota metric admitted by shape - and nothing else falls through.
- **Shape alone was not enough, and my first test run proved it against me [M]:** I predicted `exhaustion_behavior` (US spelling), `offer_typ` and `requires_cards` would be rejected. They were **ACCEPTED** - all three are well-formed snake_case metric names. The prediction was wrong and the guard had a hole exactly where it mattered most. Closed it with a near-miss rule, and chose the threshold by measurement rather than taste: across **all 206 distinct non-reserved field names** registered anywhere in this repository (assertions, matrix rows, columns, structured-JSON fields), **ZERO are within edit distance 2** of any reserved name, while all six typo candidates sit at distance 1. Distance 1 therefore has a measured margin of 2 against real work.
- **THE NAIVE FORM OF THE VERBATIM RULE IS WRONG, and measuring stopped me shipping it [M]:** "assertion value must occur in its pinned block" flags **13** of the six providers' assertions. TEN are `service` mapping a canonical identity (`AWS 12 Month Free Tier`) onto a block that does not contain it; three are quota-metric normalisations (`'1'` from *"Firestore allows exactly one free database per project"*). Those are **mappings**, exactly like `offer_type`, not prose. `docs/PROVIDER_ADAPTERS.md` scopes the rule precisely - *"Free-text values **that reach the UI**, such as `notes`"* - and scoped that way the violation count is **ZERO**. Widening it would have been a false rejection on correct work, which is how a guard gets deleted instead of fixed.
- **Containment, not whole-block equality, and that is deliberate [M]:** all 18 `notes` assertions today are equal to their whole block, so equality would also have passed. Containment is still the correct rule: truncating a quotation is still quoting it, and equality would reject a legitimate partial quotation. Comparison is against the **normalised** block (`text` is `normspace`d in `__post_init__`), i.e. the representation `_apply_assertions` itself compares - so a value quoted from raw markup fails and one quoted from the normalised form passes. `test_the_verbatim_rule_survives_the_profile_normalising_its_own_block` pins both directions, because getting this backwards is precisely how a verbatim guard produces false rejections.
- **Capture grounding, the half that had no failure mode at all [M]:** every `capture.json` *claims* "every one of the profile's pinned blocks occurs EXACTLY ONCE in this capture". A sidecar that says so is not a check. `tests/unit/test_assertion_capture_grounding.py` now measures it with the engine's **own `_DocumentCollector`** over the committed bytes. MEASURED across all **31** html sources declared by the six provider YAMLs: every pinned block matches **exactly once** in its declared scope. Anomalies 0, missing captures 0. It covers `required=False` assertions (the runtime path skips an unmatched optional pin in silence; there are zero today, so the silent path is untravelled rather than safe) and it needs no database, so the rule holds where the integration suites are `skipif(not DATABASE_URL)`.
- **A FINDING against another slice, reported not fixed [M]:** `config/examples/providers/cloudflare.example.yaml` declares official html source `cloudflare-pages-pricing` (F005) whose capture was **never committed** - `tests/fixtures/ingest/cloudflare/html/` holds only `cloudflare-pages-limits` and `cloudflare-workers-limits`. It is recorded in code as `KNOWN_UNBOUND_SOURCES` rather than skipped silently, guarded by **set equality** so a new unbound source fails immediately and a fixed one forces the entry's deletion, and `test_the_only_unbound_source_pins_nothing` proves the exemption is safe (the generic `pricing_document` profile pins nothing) instead of assuming it. Capturing the live page is outside the suite by design, so it is not fixed here.
- **ALL SIX PROVIDERS CLASSIFY EXACTLY AS ON `main` [M]:** a DB-free oracle drives the real `HtmlDocAdapter` over each committed capture, then the real `revalidate_quotas` + `classify_offer` path the publisher uses in memory before it inserts a version. 31 sources, 31 candidates, 0 validation problems. The full report digest is **`31274d8de1509f3d7baaaa57055d91af2fa3574357ec26af821a26421c13d632`** before the change, after rules 1+2, and again after the near-miss rule - byte-identical all three times. Verdicts: GitHub Actions/Packages/Codespaces `Z0_TRUE_FREE`, GitHub trial `Z2`, GitHub Pages `UNKNOWN`; AWS 3x`Z1`/3x`UNKNOWN`; Azure 2x`Z1`/5x`UNKNOWN`; GCP 3x`Z1`/1x`UNKNOWN`; Oracle 4x`Z1`/2x`UNKNOWN`; Vercel 3x`UNKNOWN`.
- **Mutation battery, run at STEP level rather than job level [M]:** a grouped battery cannot distinguish "all gates fired" from "one fired and the rest are decorative", so **every selector is run in its own pytest process and carries its own exit code**. EIGHT enforcement points, each with its own RED demonstration and its own paired GREEN collateral control, **8/8 matched prediction**: E1 unregistered/badly-shaped field, E2 control-plane field, E3 wrong value kind on a reserved field, E4 near-miss of a reserved name, E5 free-text value not quoted from its block, E5b verbatim measured against the NORMALISED block, E6 pinned block grounded exactly once in its capture, E7 the grounding walk actually visits its sources. Guard mutations used: `problem = assertion_field_problem(...)` -> `problem = None`; near-miss distance `1` -> `0`; the verbatim `if` -> `if False:`; **the real GitHub sentence that is the whole basis for three Z0 verdicts reworded into a composition**; and the source walk made to visit nothing. Each anchor was asserted to match **exactly once** and the patched line printed, so a mutation that failed to apply cannot look like a control that failed to fire. Every file restored **byte-exactly, proved by `git hash-object`**, never by `git diff --numstat`.
- **AND ON THE REAL INPUT PATH, not only against hand-built fixtures [M]:** a construction-time rule proven only against a test profile can pass while no production profile ever reaches it. Each of the five construction-time enforcement points was therefore **also violated inside a real merged provider module** (`profiles/oracle.py`), with the oracle being the production import `import app.ingest.adapters` - the very registry `resolve_profile` reads. Baseline import exit 0 in every case; mutated import exit **1** in every case, **5/5**, each raising from the shipped profile by name: `oracle_cloud_free_tier assertion[6]` for the badly-shaped field, the control-plane field, the wrong value kind and the near-miss; `oracle_free_tier assertion[7]` for the paraphrased `notes`. Every file restored byte-exactly and hash-verified.
- **One test file changed, and in the compliant direction [M]:** `test_a_pinned_block_published_twice_yields_ambiguous_assertion` (Azure) built a synthetic probe pinning `notes` to the sentinel `"duplicated"`. The new rule correctly refuses it. The probe now quotes a verbatim clause of the block it pins - the rule was applied to the probe, not relaxed for it. No provider profile was touched.
- **Two existing message-shape assertions kept working by design [M]:** boolean-gate rejections are still worded "requires one of [False, True]" so `test_profile_rejects_assertion_values_outside_closed_field_vocabulary` needed no edit. I found that out by breaking it first: my initial wording changed the message and turned two pre-existing tests red. Rejection behaviour never changed; only my wording did, and I reverted the wording rather than the tests.
- **Files changed (8 + 1 new):** `apps/api/app/ingest/vocab.py` (the registry), `apps/api/app/ingest/adapters/html.py` (both checks), `apps/api/app/ingest/__init__.py` (re-exports), `tests/unit/test_adapter_html_assertions.py`, `tests/unit/test_assertion_capture_grounding.py` (new), `tests/unit/test_ingest_contract.py` (drift guard), `tests/unit/test_adapter_azure.py` (the probe), `docs/PROVIDER_ADAPTERS.md`, `agent-state/current_contract.json`, and this handoff. **Zero fixtures touched**; no `source.html` appears in the diff.
- **Drift guard [M]:** `RESERVED_FACT_FIELDS` (ingest) and `NON_QUOTA_FIELDS` (publish) are the same set seen from two sides and live in two modules because `app.publish` imports `app.ingest`, so the reverse would be a cycle. `test_the_reserved_fact_field_registry_cannot_drift_from_the_publisher` asserts set equality and that the assertable/control-plane halves partition it - a divergence there is the same failure one layer up.
- **Gates [M]:** full offline suite **2574 passed, 255 skipped** on the post-rebase tree (base `0d07294`, which brought #79's staleness work and #81's CI gates); re-derived rather than carried forward from any earlier figure. `ruff check` clean, `ruff format --check` clean across 230 files. Run by module because nine `check.ps1` gates are unrunnable here for want of a toolchain (above): Ruff lint, Ruff format, pytest, `detect_secrets.pre_commit_hook` over the staged set exit 0 with the baseline **not** rewritten. Via `check.ps1` itself: Secrets baseline shape PASS, URL host allowlist PASS (1,255 URLs, 57 hosts). Prettier and ESLint are not applicable to this diff - `.prettierignore` excludes `*.md`, `docs/` and `agent-state/`, and no JS/TS/CSS/YAML was touched. CI runs all of them.
- **Protected state [M]:** blob-hash verified IDENTICAL across **29** paths against the post-rebase base `78d2c73`, including `agent-state/feature_list.json` (`154de1fef2ba`, **F008 stays `passes:false`**), all seven provider profile modules and their configs, `apps/api/app/publish/**`, `apps/api/app/classify/**`, `tests/support/source_scan.py`, `tests/unit/test_secrets_baseline.py`, `apps/web/package-lock.json` (`98f786c2bb0d`), `.github/workflows/ci.yml`, `scripts/check.ps1` and `docker-compose.yml`. The unfiltered `git diff` was run FIRST as a positive control, and the checker itself carries a negative control asserting that a file which DID change is reported as changed - otherwise the loop could pass by comparing nothing.
- **Rebase resolution, by byte construction rather than hand-merge [M]:** exactly two conflicts, both in `agent-state/`. `current_contract.json` taken from THIS branch wholesale (single-slot file, never merged) and re-validated as JSON. `progress.md` rebuilt as `main`'s blob VERBATIM plus this block. **A finding on `main` while doing it:** `main`'s `progress.md` does **not** end with a newline (682,847 bytes ending `...untouched.`), so a naive append would have joined this heading onto the previous slice's last line. A separator was inserted rather than main's bytes altered: prefix equality holds byte-for-byte, the seam reads `` untouched.\n\n## 2026-08-24 ``, and the result ends in `0x0A`.
- **ANSWER TO "is any enforced rule unable to fire on real input?" - YES, THREE, and they are measured not guessed [M]:** (1) **The optional-assertion skip.** `_apply_assertions` does `if not matches: if assertion.required: reject; continue` - a silent `continue` for an unmatched optional pin. Across all 31 assertion-bearing profiles there are **ZERO `required=False` assertions**, so that branch is unreachable on committed input and is exercised only by hand-built test profiles. (2) **`no_assertion_evidence`**, the runtime evidence floor, is reachable only when every assertion is optional and none matched - so it inherits (1)'s unreachability. (3) **`conflicting_assertion`, by BOTH of its branches.** The first compares an assertion against a fact already present from a table; **zero profiles** map a field from a column or matrix row AND also assert that same field, so `facts` never holds an asserted field when `_apply_assertions` runs. The second compares two assertions on one field; **zero profiles** assert the same field twice with different values. So `conflicting_assertion` cannot fire on any committed input at all. None of these is a defect - they are guards ahead of their traffic - but each is currently prose-with-a-test rather than machinery with live evidence, and (1) is exactly why the new capture-grounding guard **deliberately covers optional assertions**: it closes that path before anyone travels it.
- **A FALSE ATTRIBUTION IN MY OWN COMMENT, caught by auditing myself against the Oracle Level-2 failure [M]:** that evaluation (`agent-state/evaluations/F008-p6-oracle-provider-85ef245d.json`) failed on a false universal shipped in documentation, refutable only by executing across ALL provider modules. I re-ran my own universals the same way - every registry (HTML 36, structured/JSON 2, MCP 1) with the test-support profiles imported as pytest imports them. Three claims survived exactly (206 non-reserved names, **zero** within edit distance 1 **or** 2 of a reserved name; exactly ten `service` mappings not contained in their block; zero table/assertion field overlap, zero conflicting same-field assertions, zero optional assertions). **One did not.** My `vocab.py` comment read "all 89 quota-metric assertion fields, all 102 matrix fields and all 23 column fields **across the six merged providers**". The counts are right repository-wide; the attribution is wrong. The six merged providers declare **ZERO** column fields - all 23 belong to Cloudflare and the two generic profiles. True as a number, false as an attribution, and refutable only by executing across every module. Corrected in place, and the property (not the counts) is now re-derived by `test_no_registered_field_name_in_this_repository_is_confusable`, so a comment can no longer be the only thing asserting it.
- **A SECOND identity-versus-position error, caught by reading the exit code rather than the colour [M]:** my first mutation proof for that new repository-wide guard mutated an *assertion* field in `oracle.py` to `notess`. It went RED - but with pytest exit **4**, a collection error, because the CONSTRUCTION guard rejects the profile at import. The red light belonged to a different gate than the one I was claiming to prove. Re-derived against the fields construction does **not** check - `HtmlColumn` mappings, which never reach `assertion_field_problem`. Mutating a Cloudflare column to `notess` (confusable) and to `"Memory Used"` (bad shape) leaves the **production import at exit 0** and turns the repository-wide guard RED at exit **1**, both restored byte-exactly by blob hash. That is the guard firing on its own identity, and it also establishes the guard is not redundant with construction: columns, matrix rows, `required_fields`, JSON-profile and MCP fields are covered by nothing else.
- **ANSWER: does anything PUBLISHED depend on the unbound `cloudflare-pages-pricing`? NO [M].** Measured against the config: **zero** coverage entries cite it. Cloudflare's three evidence-backed states cite `cloudflare-pages-limits` (`containers-app-hosting`), `cloudflare-workers-limits` (`serverless-functions`) and a bare `evidence_url` (`networking-cdn-dns`) - and both named sources DO have committed captures. The unbound source sits behind **nothing**: no coverage claim, no offer, no `verified_free`. Severity is low for a measurable reason rather than by assumption. Two residual notes: it resolves to the GENERIC `pricing_document` profile, which pins no assertions at all, so a live scan would extract table rows with **no evidence pins**; and having no capture, no offline run has ever exercised it.
- **ANSWER: is any branch reachable on real input but untested? [M]** Measured with stdlib `sys.monitoring` (PEP 669, one-shot DISABLE per location) over the whole offline suite - no new dependency, since coverage tooling is deliberately absent from `requirements-dev.txt`. `app/ingest` + `app/publish`: 5,826 executable lines, 4,706 covered, **1,120 uncovered**. **MOST OF THAT IS NOT A FINDING:** `publisher.py`, `config_sync.py`, `reconcile.py`, `scan.py`, `runner.py` and `reconcile_coverage.py` are DB-gated and their tests SKIP without PostgreSQL (245 skipped locally); CI runs them with Postgres, so "uncovered here" is not "untested". Restricted to the pure, non-DB modules that already have dedicated unit-test files, these lines have **never executed anywhere in the suite**:
  - `publish/revalidate.py:313` and `:316` - the `None`-valued and blank-valued skips in `revalidate_quotas`. The *conditions* (312, 315) are covered; the *bodies* are never taken, so no test has ever supplied a `None` or blank fact. Reachable the moment any mapped column is blank on a real page, because `_coerce` returns `None` for an empty cell - and it sits on the **publication** path, where the alternative is a quota row whose raw text reads `"None"`. **I checked before claiming it fires today: ZERO of the 31 real candidates carry a `None` or blank fact**, so this is one blank table cell away from live rather than currently live. That distinction is the whole point; I nearly reported the stronger claim.
  - `adapters/_common.py:44` - `to_bool` returning `None` for a cell that is neither yes nor no. Same shape, on the **material Z0 gate** path (`requires_card` / `has_paid_dependencies` are bool-coerced columns for Cloudflare).
  - `adapters/html.py:859` (a spacer/empty matrix row skipped) and `:475-476` (`_span` returning 0 for a non-numeric `colspan`) - both ordinary real-markup conditions.
  - `"Candidate has no evidence location."` - the last-line evidence floor, in **all four** adapters at once (`html.py:719`, `structured.py:197`, `mcp.py:301`, `rss.py:156`). Never executed anywhere.
  - **Independent corroboration by a different instrument:** execution tracing confirms `conflicting_assertion`'s SECOND branch (`html.py:993, 995-999`) has **no test at all**, matching what profile-data analysis already said about its unreachability. Two derivations that could not have failed the same way. Tracing also **corrected** me: the optional-assertion `continue` is line **975** and IS covered by a hand-built fixture, while line 914 - which I could easily have misread as that skip - is the *matrix* optional-row skip. Position is not identity.
- **D1, THE BLOCKING DEFECT, and it is my slice failing its own thesis in my own file [M]:** `apps/api/app/ingest/vocab.py` declared `__all__` **TWICE** - my 15-name tuple at line 283, and the pre-existing 3-name tuple still sitting at 302. Python executes top to bottom, so the second silently voided the first and **12 of the names the module means to export never reached its declared API**. Found by TWO independent Level-2 evaluators by different methods; I then confirmed it a third way before touching anything, at runtime rather than by reading: `len(vocab.__all__)` was **3**, and a real star-import yielded the same 3. Behaviourally inert today - nothing star-imports it and `app.ingest.__init__` imports explicitly - and **no declared gate catches it**: `ruff` exits 0 because pyflakes `F811` does not cover module-level variable rebinding, and no test asserted this module's `__all__`. The repository DOES treat `__all__` as a contract elsewhere (`test_adapter_oracle.py` iterates `oracle_module.__all__`), so a truncated one is not cosmetic. Fixed by deleting the stale assignment; re-derived at runtime as **15 names, star-import 15, every name resolving, zero duplicates**, and by AST as **exactly one** module-level `__all__` assignment.
- **D1 repair, made MACHINERY rather than a corrected line [M]:** the whole thesis of this slice is that an implicit contract a contributor can silently violate must become a deterministic failure - so fixing the line and moving on would have been the wrong repair. Two new tests in `tests/unit/test_ingest_contract.py`. `test_no_module_in_the_ingest_package_declares_all_twice` walks the WHOLE `app/ingest` package by AST, deliberately not shaped around the single known instance, with a floor asserting it actually parsed >= 15 modules so it cannot pass vacuously. `test_the_ingest_vocabulary_exports_what_it_declares` measures at RUNTIME via a real star-import - the defect was invisible to reading precisely because both assignments looked correct in isolation. **Both mutation-proved:** restoring the EXACT shipped defect (re-appending the 3-name tuple) turns both RED; truncating one name from `__all__` turns the runtime one RED; baseline green in every case, `vocab.py` restored byte-exactly and verified by `git hash-object` (`ee330218ab8f`).
- **D2, a count stated without its scope - the third instance of this class on this project today, and the second by me [M]:** my PR body said the naive verbatim rule "flags 13 existing assertions". Re-derived independently rather than accepting the correction: of **239** string-valued assertions the naive rule flags **64**, of which **51** are closed-vocabulary mappings (`offer_type` 29, `exhaustion_behaviour` 22), leaving **13** once those are set aside, and **0** under the rule as shipped. 13 is right only with its scope stated, so the wording now carries it.
- **D3, an asymmetry I disclosed but never quantified [M]:** rule 2's only live subjects are the `notes` assertions - `display_name` and `service_description` have **ZERO**. Measured at both scopes, because the two differ and the difference is exactly the sort of thing that becomes a false attribution: **18** across the shipped provider profiles, **19** once `tests/support/html_profiles.py` registers its own (the test-support profile adds one). All of them have `value == text` **exactly**, and **zero** are strict substrings, so containment and equality are indistinguishable on today's corpus. Stated plainly: **RULE 2 REJECTS NOTHING ON THE CURRENT CORPUS AND ITS VALUE IS ENTIRELY PROSPECTIVE**, against rule 1's 254 live assertion subjects. That is a guard ahead of its traffic, which this project builds deliberately - but a reader should not have to derive the asymmetry themselves.
 the quotation rule binds declared free-text fields only, so a paraphrase written into a *quota-metric* field escapes it - that boundary is the documented one and is pinned by `test_a_mapped_field_is_not_held_to_the_quotation_rule`, because widening it would fail the ten legitimate `service` mappings. The near-miss rule is edit distance 1 against a 16-name reserved set; a future metric genuinely one edit from a reserved name would be refused, and the message says exactly what to write instead. Capture grounding is a repository-level test, not a runtime check, because the runtime has no concept of "the committed capture". And the three dead branches above were left dead rather than given synthetic traffic.
- **D4, a scope-versus-name mismatch in a test I added, reported by the evaluator and independently re-derived [M]:** `test_no_capture_directory_is_silently_unbound` filtered `if source.get("type") != "html": continue`, so its NAME promised every declared source while its REACH covered only html. Re-measured before acting: **36 declared sources (34 html, 1 rss, 1 mcp)** with **THREE** unbound - `cloudflare-pages-pricing` (html, correctly pinned by me), plus `cloudflare-changelog` (rss) and `cloudflare-docs-mcp` (mcp), neither disclosed and guarded by nothing. All three are Cloudflare (F005) and pre-date this slice, so widening the walk to the other two adapters is a separate change and is NOT done here. Renamed to `test_no_html_source_is_silently_unbound`, with the 34-of-36 scope and the two out-of-reach sources named in its docstring and beside `KNOWN_UNBOUND_SOURCES`. This is the same defect class the module exists for - a name read as a stronger promise than the code delivers - occurring in my own guard.
- **A DOCUMENTED LIMITATION OF MINE WENT STALE, and I re-ran rather than carried it forward [M]:** my earlier text said `scripts/check.ps1` "could not be run to completion" and "reports FAIL for a missing toolchain rather than a real defect". That was true against base `699481b`. PR #80 has since landed `Test-ToolAvailable`, which asks BEFORE running. **Re-run on the rebased tree against `0d07294`, and the observed behaviour is different:** the script completes, exits 1, and every failing branch now names its own cause - *"required tool 'ruff' was not found in .venv/Scripts nor on PATH. This is a missing toolchain, NOT a finding from this check"*, and likewise for `pytest`, `detect-secrets-hook`, `pip-audit` x3, plus *"required path 'node_modules' is missing"* for Prettier and ESLint. The two gates whose tooling IS present both PASS and both are meaningful here: **Secrets baseline shape** (68 tracked files, 289 entries, nothing lost) and **URL host allowlist** (1,255 URLs, 57 hosts, all allowlisted). So the honest statement is no longer "the script cannot be run" but "**nine gates are unrunnable on this machine for want of a toolchain, and the script now says so itself rather than reporting them as findings**". Each of those nine was run directly by module instead - see the gates line. This is a limitation decaying in the direction of making the system look WORSE than it is, which is why it had to be re-derived rather than repeated.

- **Evaluator disposition:** pending - builder self-review only.
- **Boundary:** Draft PR only. Do not merge, do not flip any ledger flag.


## 2026-08-24 - F008 S5: evidence-level staleness, and the adviser that exported an expired free claim [M]

- **Base:** `ee719f7280103c38dba3c4509f6e05d01ae3149d` (re-derived with `git ls-remote origin refs/heads/main`, not trusted from the brief). Branch `stsyg-evidence-level-staleness`. Draft PR only; `agent-state/feature_list.json` untouched and **F008 stays `passes:false`**.
- **THE MEASUREMENT THAT DID NOT EXIST BEFORE [M]:** the adviser exposure was a STRUCTURAL read in the brief, and the PR #79 evaluator explicitly enumerated `/adviser/recommend` and `/adviser/export`, said it would EXPECT them exposed, and **refused to assert what it had not tested**. It is now measured. With evidence past its refresh window: `/adviser/recommend` returned `fully_zero_cost=true`; `/adviser/export` returned `manifest.fully_zero_cost=true` with `Z0_TRUE_FREE` inside `MANIFEST.json` and a README stating verbatim *"\ proof: every component below is classified Z0_TRUE_FREE by the classification engine"*. The entire four-file bundle contained **ZERO** occurrences of the words "stale" or "fresh". That is the artefact that leaves the system, is committed into someone else's repository, and outlives the evidence behind it.
- **THE TRAP, CONFIRMED RATHER THAN ASSUMED [M]:** `build_candidate` re-runs the classifier and marks disagreement as CONTRADICTION, so it reads like a currency guard. Measured `pool.excluded` was **EMPTY in BOTH directions** - stale and fresh. Both sides read the same frozen `material_facts` and agree perfectly on evidence that expired years ago. It compares two classifications and never looks at evidence age. This is now pinned by a test asserting the emptiness in both directions, so nobody re-reads its presence as coverage.
- **A THIRD, INDEPENDENT EXPIRY MECHANISM [M]:** `assess_staleness` decides `stale = (now - fetched_at) > window`. The two prior derivations both moved `age` (backdating `fetched_at`; advancing the clock past a real ingest). This slice moves **`window`** - the other side of the same inequality - by shrinking `Source.schedule`. In the integration tests both offers share ONE `fetched_at`, are read at the real wall clock through the router's own `_now()`, and differ **only** in their source's declared window, so an error in age arithmetic could not manufacture the result. Cross-checked against mechanism A at `now+5y`; the two agree.
- **THE SPLIT, AND WHY IT IS NOT ONE SLICE [M]:** the brief's hypothesis was that the offer surfaces, the adviser and the declaration-source gap sit over one missing abstraction. Measured: it **holds for two and fails for the third**. The offer surfaces and the adviser both read frozen values and need the same `now`-seam resolver. The declaration gap cannot be closed by threading a clock at all - of 7 `verified_free` declarations across all seven shipped provider configs (cloudflare is F005; the six merged F008 providers are aws, azure, gcp, github, oracle, vercel), **5 carry a `source` and 2 carry `evidence_url` ONLY** (`cloudflare|networking-cdn-dns`, `github|ai-inference-embeddings`). No source means no schedule means no snapshot means no `fetched_at`, so there is no timestamp for a clock to be compared against and those two pairs are **time-invariant**. That needs a config-schema change and a data migration, which is a different blast radius. Shipped as slice 1 of three: **this = the abstraction + the adviser; slice 2 = the six catalogue read surfaces; slice 3 = the declaration-source gap.**
- **THE ABSTRACTION, and why it is not a boolean [M]:** `apps/api/app/read_api/currency.py` introduces `EvidenceCurrency(stale, checked, age, window)`. A lone `stale` flag cannot distinguish *"we looked and it expired"* from *"there was nothing to look at"*, and collapsing them is **exactly how the declaration gap hides** - an `evidence_url`-only declaration would read as fresh. `UNCHECKED` is fail-closed for a free claim but is deliberately **not** reported as stale, and its `freshness()` is `None` rather than `0.0`: an absent measurement, not a bad score. **No migration, no schema change, no stored projection** - currency is recomputed per read against a caller-supplied clock, per Q11, because a stored projection becomes the second source of truth that silently goes stale, which is the very defect being fixed. `fetch_stale_offer_version_ids` keeps its exact signature and is reimplemented on top, so PR #79's `/catalogue/categories` path is behaviourally untouched.
- **REAL-INPUT REACHABILITY, stated separately from test coverage [M]:** ran the real ingest+publish pipeline over committed fixtures for all seven providers. **6 published offer versions: 5 `Z0_TRUE_FREE` and 1 `Z2_TEMPORARY_OR_CONDITIONAL` (the GHEC trial), and EVERY Z0 published offer carries evidence with a checkable `fetched_at`.** That measurement is what made the strict gate safe to ship: refusing *"currency could not be established"* withdraws nothing real. Both directions on real input: at `now`, 5 in `z0` with confidence `high` and an empty stale partition; at `now+5y`, **0 in `z0`**, all 5 in `stale`, confidence collapsed to `unknown`, and the recommendation says *"last fetched 1824 days ago, past its 7 days refresh window"*. Offer ids are DB-local auto-increment values and are deliberately not quoted as identifiers.
- **FIRST EVER OBSERVATION OF A CONSTRAINT NOBODY HAD SEEN FIRE [M]:** `ck_provider_category_coverage_claimed_offer_requires_evidence` had only ever been READ. A raw INSERT bypassing the ORM, of a `verified_free` declaration with neither `source_id` nor `evidence_url`, is **rejected** with a `CheckViolation` naming the constraint - while a **negative control** carrying `evidence_url` is accepted, proving the rejection is the constraint and not an unwritable table. A measurement, not a fix; nothing was changed.
- **CONTROLS - all six guards proved able to FAIL [M]:** per guard: baseline GREEN, anchor asserted to match **EXACTLY ONCE**, patched line printed **read back from disk**, RED observed with the failing tests named, then restored and proved byte-exact by **BOTH `git hash-object` AND a raw byte-by-byte comparison** (`git diff --numstat` is not trusted on this repo). All six killed at least one named test: pool gate (5 tests), publishable predicate (1), confidence cap (1), export fail-closed (2), currency resolver (1), worst-combiner (1). Notably the export mutant killed `test_export_fails_closed_even_if_the_pool_gate_were_bypassed` which the pool mutant did NOT - substantiating the export validator as a genuinely **independent** second gate rather than a restatement of the first.
- **BOTH DIRECTIONS ON EVERY SURFACE [M]:** a wrongly-withdrawn free offer is a defect of equal severity, so every refusal has a paired control proving the same code path still serves a genuinely free, freshly-evidenced offer. A stale Z0 is **not dropped**: it becomes an explained `ImpossibleResolution`, still surfaced as the closest candidate, telling the reader the offer exists, how old the evidence is, and to re-verify it.
- **ERRORS I MADE, all caught by instruments rather than by care [M]:**
  - **E1 - the `pool.z0 >= 1` floor FIRED on my very first probe run.** My fixture had no quota, so `classify()` returned UNKNOWN at Gate 4 and the pool was empty. Without the floor I would have reported *"the adviser serves no stale Z0 claim"* - a confident wrong answer, in the comfortable direction.
  - **E2 - I contaminated my own instrument.** The first fixtures were named `probestale`/`probefresh`, so the "does the bundle mention staleness?" word-scan matched the **provider slug** and reported a currency signal that did not exist. Renamed to neutral slugs; the honest answer is zero.
  - **E3 - I nearly measured my own input.** I hardcoded `freshness: 1.0` into a fixture and would have "measured" it coming back out. Corrected by calling `publish.confidence.freshness_ratio` directly to show 1.0 is genuinely what the publisher writes at publish time.
  - **E4 - I asserted a mechanism I had not measured.** I wrote a test asserting `build_export` would RAISE on a stale claim. It does not: `recommend` has already emptied `components`, so the validator sees nothing to reject. I measured what the bundle actually contains and rewrote the test around the true mechanism, with a note naming which gate protects that path and which test covers the other.
  - **E5 - my baseline was too narrow, and it cost me.** I baselined `tests/unit tests/integration` only, so `tests/security` was invisible until the final run, where a monkeypatch lambda broke on the new keyword argument. The full-tree baseline is the one that matters.
  - **E6 - the integration floor caught FIVE VACUOUSLY PASSING TESTS.** My first shared fetch instant was `now`, so even a 1-second window was not exceeded and nothing was stale; five assertions passed by proving nothing. The floor turned that into a loud failure.
- **Gates:** full `pytest tests` **2859 passed, 3 skipped, exit 0** (baseline 2666 on unit+integration alone; 33 tests added, none removed). `ruff check` and `ruff format --check` both clean over `apps/api` and `tests`. Run against an **isolated** PostgreSQL 16 on a non-default port with its own volume - never the shared instance, because a shared instance is a shared assumption.
- **NOT VERIFIED, stated plainly:** whether the six real published offers are stale **today** is a question this slice does not answer - the reachability probe measured induced expiry at `now+5y`, and at `now` all five were current. `apps/web` is unchanged and untested by me; no front-end change was required because `RecommendationView.tsx` already renders `impossible`/`blocking_reason`. `npm ci` and `npm audit fix` were NOT run.
- **KNOWN DEFERRED PROBLEM, recorded so slice 2 does not discover it:** `GET /catalogue/search?zero_cost_class=Z0_TRUE_FREE` filters on the **stored** column in SQL. Once slice 2 changes what the read surfaces display, filter and display can disagree - a stale offer returned by a "show me free things" filter while displaying as unsupported. That belongs to slice 2.
- **Evaluator disposition:** pending - builder self-review only.
- **Boundary:** Draft PR only. Do not merge, do not flip any ledger flag.
## 2026-08-25 - Builder - CI: the disclosure guard must not be silenced by the guard beside it [M]

- **Feature / objective:** F001, slice `ci-disclosure-guard-not-masked`. In `.github/workflows/ci.yml` the `URL host allowlist` step of the `Secret scan` job carried no `if:`, so its condition defaulted to `success()` and GitHub SKIPPED it whenever an earlier step in the job failed. The guard that keeps an internal package-feed hostname out of this PUBLIC repository therefore stopped running at exactly the moment another guard had just fired - when the tree is known to be in an unexpected state and scrutiny should be highest. One finding hid the rest.
- **Change, and it is one key:** `if: ${{ !cancelled() }}` on that step, plus comments. Asserted mechanically by parsing BOTH `origin/main`'s ci.yml and the working tree's and diffing the step-condition maps: **exactly one condition changed**, on exactly the `URL host allowlist` step, `<none>` -> `${{ !cancelled() }}`, with `Scan tracked files against baseline` and `Validate secrets baseline` asserted STILL `<none>` as a negative control. Both parses were floored at 5 jobs / 6 steps and every asserted step name was required to be FOUND, so a typo could not silently assert nothing.
- **`!cancelled()` NOT `always()`, and the difference is not cosmetic [M]:** they differ on exactly one input - a CANCELLED run - and this workflow sets `cancel-in-progress: true` keyed on `ci-${{ github.ref }}`, so that input occurs routinely: any second push to a branch cancels the run in flight. `always()` would execute the step inside that dying run and write a GREEN STEP INTO A CANCELLED RUN, which reads as "the guard ran and passed" to anyone who does not also read the run conclusion. `cancelled` is not `failure`. `!cancelled()` keeps the only behaviour needed - run after a FAILED step - and is the spelling `web` and `dependencies` already use, so the file gains no second idiom for one intent.
- **FIVE DEMONSTRATIONS, read at STEP level, every prediction recorded BEFORE the run [M].** BEFORE and AFTER are a true controlled pair: the planted control file's blob is `1a271598` on BOTH branches, the BEFORE branch's ci.yml blob is `b62af6ba` which is byte-identical to main's, and only the AFTER branch's differs (`124ced29`). The workflow is the ONLY variable.
  - **1. BEFORE, reproduced not cited (PR #84, run 32896122613, ci.yml identical to main's).** `Validate secrets baseline` success -> `Scan tracked files against baseline` **failure** (`Scanning 655 tracked file(s).`, `Base64 High Entropy String` + `Secret Keyword`) -> `URL host allowlist` **skipped**, emitting no output whatsoever. JOB failure.
  - **2. AFTER, identical failure (PR #85, run 32896133739).** Same `Scanning 655 tracked file(s).`, same two secret types -> `URL host allowlist` **success, having RUN**, reporting `URL host check passed: 1260 URL(s), 57 distinct host(s), all allowlisted.` The AFTER log carries EXACTLY ONE more matching line than BEFORE, and that line is the guard's own verdict.
  - **3. AFTER, the guard can still go red on its own merits (PR #86, run 32894516582).** Both secret-scan steps success, `URL host allowlist` **failure**. A control that cannot be shown to fail proves nothing.
  - **4. AFTER, clean tree still green (PR #87, run 32894548986).** Both steps success, `Secret scan` JOB success, all five jobs success. A guard that reddens honest work gets routed around.
  - **5. `!cancelled()` did NOT turn a red job green.** In demonstration 2 the JOB conclusion is **failure** while the later step reports success. Verified at JOB level, which is the exact way an `always()`-shaped change goes wrong unnoticed.
- **Instrument discipline:** every read asserted check-runs `>= 5` and all five job names FOUND; every read carried a NEGATIVE CONTROL looking up a deliberately bogus step name which MUST return `<step-not-found>`, so an all-`skipped` reading is distinguishable from a broken lookup. Mergeable state was read before interpreting any conclusion, because a CONFLICTING PR is never dispatched at all. All GitHub data was parsed from JSON in Python, never through a PowerShell pipeline where `--jq` output arrives as a STRING ARRAY whose `.Length` is a line count.
- **ERRORS I MADE, and the instruments that caught them [M]:**
  - **E1 - MY FIRST CONTROL WAS SILENT, AND I ALMOST BANKED IT.** I planted a 48-character random string whose entropy I had measured at 5.0952 against this repository's configured `Base64HighEntropyString` limit of 4.5, predicted the scan would fail, and it reported **CLEAN over 655 files**. Only the recorded prediction made that visible as a failure rather than as a result. A silent control would have "demonstrated" the fix while proving nothing at all.
  - **E2 - I then diagnosed it with the WRONG INSTRUMENT and briefly believed a false explanation.** I ran `detect-secrets scan` with DEFAULT plugins, saw the line flagged, and formed a quoting hypothesis. But CI runs `detect-secrets-hook --baseline`, which takes its plugin limits and filters FROM THE BASELINE. Testing the wrong tool produced a confident, wrong answer.
  - **E3 - my replacement harness was broken and the NEGATIVE CONTROL is the only reason I knew.** Running the hook in a bare temp directory returned exit 1 for EVERY input including ordinary prose, because "your baseline file is unstaged" is one of the several distinct causes of exit 1 that this workflow's own comments already warn about. Had I omitted the prose-only control I would have read six exit-1s as six findings.
  - **E4 - three synthetic approximations in a row instead of the real bytes.** I tested one-line reconstructions of the planted file rather than the committed blob. Scanning the actual blob reproduced CI's CLEAN result immediately and bisecting it located the boundary in one pass. I should have started there.
  - **E5 - I wrote a PowerShell heredoc.** `git commit -F - <<'EOF'` is not PowerShell syntax; it failed at the parser.
  - **E6 - my log reader crashed on a BOM** because it printed raw log text to a cp1252 console.
- **MEASURED, and worth recording because it makes a whole class of control silently useless [M]:** with this repository's `.secrets.baseline`, `detect-secrets-hook` flags `password = <48 random chars>` when it is the file's ONLY non-blank line, and reports CLEAN as soon as the file has a second non-blank line - **including when that second line is the identical secret line repeated**. So it is the line count, not the prose. Established locally against a COPY of the real baseline (the hook rewrites the baseline it is given), with a prose-only negative control proving the harness live. Pattern detectors are NOT affected: an AWS-shaped key still trips under a full paragraph. I did NOT establish the internal cause inside detect-secrets, and do not assert one.
- **Protected state:** `agent-state/feature_list.json` untouched and byte-identical to CURRENT main by blob hash; F008 remains `passes:false`. `tests/support/source_scan.py` and `tests/unit/test_secrets_baseline.py` untouched and byte-identical. The strict secrets route guard was RUN, not merely left alone: 181 tests pass, because its rule is about `jobs.*.steps[].run` values and an added `if:` key is not at an executed path.
- **Gates:** `pytest tests/unit/test_secrets_baseline.py tests/unit/test_url_allowlist.py tests/test_repo_baseline.py` **181 passed**; `ruff check .` exit 0; `ruff format --check .` 234 files exit 0; `python scripts/check_urls.py` **1260 URL(s), 57 distinct host(s), exit 0** - the same three-way-agreeing value seen in two CI runs. In CI the full suite ran **2857 passed, 2 failed, 3 skipped** on the deliberately-poisoned PR #86, the two failures being the URL guard's own tests, and **all green** on PR #87.
- **ANSWER TO THE BROADER QUESTION - yes, and the worst one is not this job [M]:** parsing every job for steps whose failure silences a later unconditioned step gives 17 masking edges, of which 11 are REAL dependencies on an install step. Six are spurious. FIVE of them are in the `python` job: `Ruff lint` failing silences `Ruff format check`, `Apply database migrations` AND `Pytest`; `Ruff format check` failing silences `Apply database migrations` and `Pytest`. A one-character formatting nit therefore hides the entire 2859-test suite. The sixth is `node`, where `Prettier check` failing silences `ESLint`. `web` and `dependencies` are already correct. NOT changed here - out of contract - but this is the same defect with a larger blast radius.
- **CHECKED, NOT ASSUMED, and deliberately NOT changed:** `Validate secrets baseline` failing still silences `Scan tracked files against baseline`. That dependency is REAL: the validator runs first precisely because `detect-secrets-hook` REWRITES the baseline, so scanning against a baseline already established corrupt makes the hook rewrite the corruption and the step then reports a rewrite - a CONSEQUENCE of the finding one step above, not an independent one. Noise dressed as a finding. Step ORDER is likewise unchanged: with `!cancelled()` on the allowlist step, order no longer determines whether it runs.
- **Second route, found while poisoning the tree:** `tests/unit/test_url_allowlist.py` enforces the same allowlist over the committed tree from inside the `python` job, so the disclosure rule had two CI routes, not one. That route is itself maskable through the `Ruff lint` edge above, so it is not a backstop. After this change one of the two routes is unmaskable.
- **NOT VERIFIED, stated plainly:** I did not observe a CANCELLED run, so the `always()`-versus-`!cancelled()` difference is argued from documented Actions semantics and this repository's existing usage, NOT measured. I did not establish the internal detect-secrets mechanism behind the one-non-blank-line behaviour. I did not test `workflow_dispatch` or `push`-to-main events; every observation is from `pull_request`.
- **Scratch artefacts to close, none merged:** PRs #84, #85, #86 and branches `scratch/ci-control-before-allowlist-masked`, `scratch/ci-control-after-secret-fails`, `scratch/ci-control-after-url-fails`.
- **Evaluator disposition:** pending - builder self-review only.
- **Boundary:** Draft PR only. Do not merge, do not flip any ledger flag.

## 2026-08-25 - Builder - CI: a one-character nit must not silence the `python` and `node` jobs [M]

- **Feature/objective:** F001 CI integrity. In `.github/workflows/ci.yml` NO step in the `python` or `node` job carried an `if:`. GitHub's default is `success()`, true only when no earlier step in the job has failed, so ONE failing step SKIPPED every later step - and a skipped step produces NO EVIDENCE. In a job named "Python lint, format, tests" a one-character formatting nit meant the job had tested NOTHING, and the red run is the one nobody re-reads: after the nit is fixed the next run is green and reads as "tests passed", when the run that carried the defect never reached them. In `node`, a Prettier difference hid every ESLint finding.
- **MY COUNT DIFFERS FROM THE BRIEF I WAS GIVEN, AND THIS IS THE HEADLINE [M].** The brief said 17 edges / 11 real / 6 spurious. **I MEASURED 22 / 15 / 7.** Derived by PARSING the workflow and SIMULATING each step's effective `if:` against the outcomes of the steps before it, under the stated model "A masks B when B is `skipped` in a run where A fails but would have run otherwise", counted over steps carrying a `run:` key. Two separate disagreements with different causes:
  - **22 vs 17 is a POPULATION difference and neither is wrong.** 17 is the SAME measurement restricted to edges whose MASKED step carries no explicit `if:`; that excludes the five deliberate install gates in `web`, which are edges too, merely correct ones. I reproduced 17 EXACTLY as a cross-check before concluding this. I state 22 because the property that matters - a failing step leaving a later step with no evidence - holds in `web` exactly as it does in `python`.
  - **7 vs 6 is a genuine CLASSIFICATION difference.** The brief left `Apply database migrations` -> `Pytest` undecided and asked for it to be settled on evidence. The evidence settles it SPURIOUS. See below.
- **The 15 REAL edges are KEPT [M]:** 14 dependencies on an install step, plus `Validate secrets baseline` -> `Scan tracked files against baseline` in `secrets`, which is real for the reason already documented on that step by PR #87. The 7 spurious edges are removed with the idiom this file already contains at `web`: `if: ${{ !cancelled() && steps.<install>.outcome == 'success' }}`, which removes the spurious edge and KEEPS the real install dependency. Change is 2 `id:` additions and 4 `if:` additions; `Ruff lint` and `Prettier check` deliberately get NO condition, because each is the first step after its install and a condition there would be a NO-OP - dead logic inside a gate.
- **THE JUDGEMENT CALL, SETTLED ON MEASUREMENT NOT INTUITION [M].** `Apply database migrations` -> `Pytest` looks real - tests need the schema. It is NOT: every integration module owns a module-scoped `engine` fixture that calls `command.upgrade(config, "head")` ITSELF. Pointed at a reachable but EMPTY `postgres:16-alpine` with `alembic upgrade head` NEVER run beforehand, `python -m pytest -q` reported **2859 passed, 3 skipped, exit 0**. Pytest builds the schema it needs and does not consume this step's output. The test I applied consistently across the file is *can the later step still produce information that is not a restatement of the earlier failure?* An install edge FAILS it (`pytest` with no packages reports import errors and nothing else); `Validate secrets baseline` -> `Scan` FAILS it; this edge PASSES it - with `DATABASE_URL` pointed at an UNREACHABLE port, `python -m pytest -q` still returned **2602 passed, 3 skipped, 257 errors**, roughly 2600 independent verdicts including the URL-allowlist route. Reasoning written INTO the file so it is not "fixed" later.
- **SECOND ROUTE, VERIFIED BY READING THE TEST RATHER THAN ON REPORT [M]:** `tests/unit/test_url_allowlist.py::test_every_host_in_the_committed_tree_is_allowlisted` and `::test_the_committed_tree_passes_the_guard` run the disclosure guard over the COMMITTED TREE from inside the `python` job's `Pytest` step. That is the SECOND CI route to the rule keeping an internal hostname out of this public repository, the first being `secrets`. Before this change a Ruff nit switched it off. After it, that route survives any lint or formatting failure.
- **THE STALE NUMBER, RE-MEASURED, NOT COPIED [M].** A comment read `-- 162 tests --`: true when written, no date, no command, still reading as present tense - this job's own defect one level up. MEASURED 2026-08-25 with `python -m pytest -q` against `postgres:16-alpine`, taking the count as the DIFFERENCE in `skipped` between the two runs: **without `DATABASE_URL` 2601 passed / 261 skipped; with it 2859 passed / 3 skipped -> 258 gated**, not 162 and not the ~242 the brief recalled. The comment now carries the command, the date, and the INVARIANT that cannot rot: every integration test is gated, so with no database the silently-skipped layer is ALL of them whatever the count is this month. The "with database" arm was independently reproduced by CI twice (2859 passed / 3 skipped on PR #88 and PR #92), so that half is three-way agreed.
- **SEVEN CI DEMONSTRATIONS, read at STEP level with the JOB conclusion recorded alongside, EVERY PREDICTION WRITTEN DOWN BEFORE THE RUN [M].** Three genuine BEFORE/AFTER PAIRS: within each pair the control files are byte-identical by `git hash-object` and `ci.yml` is the ONLY variable. BEFORE arms carry main's ci.yml blob `124ced29` byte-for-byte; AFTER arms carry the slice's `aff7df68`. All 14 byte-identity assertions passed.
  - **PAIR A (#89 BEFORE / #90 AFTER)** - control fails `ruff check` ONLY, plus a Prettier and an ESLint control. BEFORE: `Ruff lint` **failure** -> `Ruff format check`, `Apply database migrations`, `Pytest` ALL **skipped**; `Prettier check` **failure** -> `ESLint` **skipped**. AFTER, same bytes: `Ruff lint` **failure** -> format **success (RAN)**, migrations **success (RAN)**, `Pytest` **success (RAN)**; `Prettier check` **failure** -> `ESLint` **failure (RAN)**. Both JOBS **failure** on both arms.
  - **PAIR B (#91 BEFORE / #92 AFTER)** - control fails `ruff format --check` ONLY. BEFORE: lint **success**, format **failure** -> migrations **skipped**, `Pytest` **skipped**. AFTER: format **failure** -> migrations **success (RAN)**, `Pytest` **success (RAN)**, and its log reads **2859 passed, 3 skipped**. **THE CENTRAL RESULT: the JOB is RED from a quote-style nit while the ENTIRE SUITE RAN AND PASSED.** Before this change that same nit reported nothing whatsoever about the suite.
  - **PAIR C (#93 BEFORE / #94 AFTER)** - a deliberately failing alembic revision, itself ruff-clean so the migration step is the FIRST failure. BEFORE: migrations **failure** -> `Pytest` **skipped**. AFTER: migrations **failure** -> `Pytest` **failure (RAN)**, log **2602 passed, 3 skipped, 257 errors** - matching the local measurement EXACTLY and proving live that a broken migration costs the database-backed tests, not the ~2600 independent verdicts.
  - **#88, clean tree, this slice's ci.yml: ALL FIVE JOBS success.** A gate that reddens honest work gets routed around.
  - **UNMASKING DID NOT TURN ANY RED JOB GREEN.** Verified at JOB conclusion on all six control runs, which is the exact way an `always()`-shaped change goes wrong unnoticed. `continue-on-error` remains absent from the file.
  - **CAN STILL FAIL ON ITS OWN MERITS:** `ESLint` **failure** in #90 and `Pytest` **failure** in #94. A step that is green by construction proves nothing.
- **EVERY INSTRUMENT CARRIES A NEGATIVE CONTROL THAT MUST COME OUT THE OTHER WAY [M].** Edge parser: stripping all `if:` raised the count 22 -> 49; `!cancelled()` everywhere drove it to 0; unmasking one known edge dropped it by EXACTLY 1; an unsupported condition RAISES rather than being silently counted as no-edge. Scope assertion: before-vs-itself reports zero differences, and the declared spurious list is asserted to be a SUBSET of the BEFORE edge set so it cannot name an edge that never existed. Measurement: an UNREACHABLE database must exit NON-zero, and did (257 errors) - proving the skip is driven by the variable's PRESENCE and that a broken database is LOUD. Lockfile assertion proven to fire on a planted off-registry host. CI reads: every run asserted all five job names present, and a deliberately bogus step name returned `<step-not-found>` in all seven reads, so an all-`skipped` reading is distinguishable from a broken lookup. Mergeable state read BEFORE interpreting any conclusion - all seven `MERGEABLE`.
- **SCOPE ASSERTED BY PARSING, NOT BY EYEBALLING THE DIFF [M]:** both files parsed and their step-condition maps diffed. Only `if` and `id` differ ANYWHERE; job set unchanged; step order, names, `run:` bodies, `uses:` and `with:` all unchanged; NO edge introduced; edges 22 -> 15; the removed set EQUALS the seven declared spurious edges; every survivor is an install edge except the one declared `secrets` exception.
- **ERRORS I MADE, and the instruments that caught them [M]:**
  - **E1 - I INHERITED THE BRIEF'S NUMBER INTO THE VERY INSTRUMENT MEANT TO CHECK IT.** I wrote the declared-spurious list in the scope assertion as the brief's SIX, then ran it; S5a/S5b FAILED, reporting seven removed. My own measurement had already established the seventh (`Apply database migrations` -> `Pytest`) minutes earlier. Only the pre-recorded prediction turned that into a caught error instead of a quietly-adjusted number. The correction is legitimate ONLY because the evidence predates it, and that ordering is recorded in the script.
  - **E2 - my first two control files were contaminated and both verification predictions failed.** My PowerShell here-string writer stripped the trailing newline, so `ruff check` reported W292 on BOTH controls - the lint-only control also failed the formatter and the format-only control also failed the linter. Each was meant to isolate exactly one gate. Caught because I verified every control in isolation with a recorded prediction BEFORE using it; had I not, PAIR A and PAIR B would have been measuring two gates at once and the edges would not have been separable.
  - **E3 - I nearly planted the ESLint control as a top-level statement.** `const x = undefinedGlobal;` at module scope would have thrown a ReferenceError while ESLint LOADED `eslint.config.js`, crashing the tool rather than producing a lint finding - a red step for the wrong reason, which would have read as a successful demonstration. Moved inside a never-called function and verified locally: 2 errors, config still loads.
  - **E4 - `Copy-Item` with `-ErrorAction SilentlyContinue` on a source that did not exist.** A silent no-op that I only knew was a no-op because I hashed the target afterwards.
- **Protected state [M]:** `agent-state/feature_list.json` UNTOUCHED and byte-identical to main by blob hash (`154de1fe`); no feature flag flipped. `agent-state/progress.md` append-only ESTABLISHED by RAW BYTE-PREFIX comparison against main's blob, with a FLIPPED-BYTE negative control that must reject, plus a same-length-edit control that a size check would wave through. Both lockfiles re-derived, NOT recalled: root **87 resolved URLs / 1 host**, `apps/web` **336 / 1 host**, both entirely `registry.npmjs.org`, with the assertion proven to fire on a planted off-registry host. `npm install`/`i`/`update`/`audit fix` were NEVER run; `npm ci` was used once and MEASURED not to write either lockfile (`git status` empty afterwards). No file under `apps/api/app/read_api/` or `apps/web/` was touched - a parallel slice is in flight there.
- **Gates:** `ruff check .` exit 0; `ruff format --check .` 234 files exit 0; `python scripts/check_urls.py` **1260 URL(s), 57 distinct host(s)** exit 0; `pytest tests/test_repo_baseline.py tests/unit/test_secrets_baseline.py tests/unit/test_url_allowlist.py -q` **181 passed**; `npm run format:check` and `npm run lint` exit 0. Full suite in CI on the clean slice: **2859 passed, 3 skipped**.
- **NOT VERIFIED, stated as prominently as the verdict [M]:** I did NOT observe a CANCELLED run, so `!cancelled()` versus `always()` rests on documented Actions semantics and on the three jobs in this file that already spell it `!cancelled()` - it is NOT measured. I did not test `workflow_dispatch` or `push`-to-main events; every observation is from `pull_request`. The "no database" arm of the 258 figure is a LOCAL measurement only, because CI always sets `DATABASE_URL`; only the "with database" arm was reproduced in CI. I did not re-audit the 5 `web` install edges beyond confirming they are edges - they were out of contract and are unchanged.
- **Scratch artefacts to close, NONE merged:** PRs **#89, #90, #91, #92, #93, #94** and branches `scratch/ci-masking-paira-before`, `scratch/ci-masking-paira-after`, `scratch/ci-masking-pairb-before`, `scratch/ci-masking-pairb-after`, `scratch/ci-masking-pairc-before`, `scratch/ci-masking-pairc-after`.
- **Commit/PR:** slice branch `stsyg-ci-masking-edges-python-node`, DRAFT PR **#88**, base `main` at `3bdec4a`.
- **Evaluator disposition:** pending - builder self-review only. Required level 2.
- **Boundary:** Draft PR only. Not merged. No feature marked passing.
- **Recommended next action:** independent Level-2 review of PR #88, then close and delete the six scratch PRs and branches listed above.

## 2026-08-25 — F003 slice `evidence-floor-and-z0-gate-coverage` (builder, draft PR)

- **Feature / objective:** F003. Ten safety-relevant lines in the ingest adapters and on the publication path were reachable on ordinary provider input and had **never been executed by any test**. The worst was the **evidence floor** — the guard that refuses a candidate carrying no evidence location. On a product whose first rule is that no unsupported claim that a service is free may ever ship, the guard enforcing "there must be evidence" had never been shown to run. This slice gives every one of those paths a test, each with a paired positive control, and makes it impossible for a new adapter to escape the evidence-floor sweep. **Tests only — zero production-code changes.**
- **STEP ZERO, the re-measurement the finding was never given [M]:** the finding had been traced twice by independent instruments, but **both prior runs had no `DATABASE_URL`** — the offline suite only. The DB-gated layer, including the entire publish pipeline and read API, had never been traced, so the honest claim was "never executed by the offline suite", not "never executed by any test". Re-ran the trace **with `DATABASE_URL` set against a live throwaway `postgres:16-alpine`** (the image CI uses) on an isolated port, with two instruments sharing no implementation (`sys.settrace` global-call filter; `sys.monitoring` LINE with `DISABLE`), each in a **separate pytest process**.
- **VERDICT: THE FINDING STANDS [M].** Nine of the ten target lines were never executed with a database either. The DB-gated layer changes nothing.
- **Instrument controls, all of which had to hold before any verdict was read [M]:** **FLOOR** — refuse to report at all below 200 traced lines overall and 200 in `html.py`; measured **1471 total / 676 in html.py**, clearing it 7x. **ATTACHMENT** — each report reads the tracer back at session finish; both reported attached. **NINE POSITIVE CONTROLS** — the first statement of all five `validate()` methods, the `to_bool` normalisation, the `revalidate_quotas` loop header, the matrix row-loop body, the `_span` attribute lookup: **all nine HIT in both instruments**. **NEGATIVE CONTROL** — a deliberately detached arm reported **0 hits and attached=False for everything**, establishing what a silently-failed instrument looks like and that it is distinguishable from the real runs. **SELF-TEST, BOTH ARMS** — a planted test calling `to_bool('maybe')` made `_common.py:44` report **HIT** in both instruments; a sibling exercising only unambiguous tokens made the **same line** report **NOT HIT** while lines 37–43 still reported HIT. **CROSS-INSTRUMENT** — the two instruments' hit sets are **byte-identical** (1471 lines, all 10 target and all 9 control verdicts agreeing).
- **POPULATION, measured not restated [M]:** 2862 collected. With `DATABASE_URL` set and reachable: **2859 passed, 3 skipped, exit 0**. With it unset: **2601 passed, 261 skipped, exit 0**. DB-gated population = **258**, derived two independent ways that agree (2859−2601 = 258; 261−3 = 258).
- **WHAT THE 258 DB-GATED TESTS ACTUALLY ADD [M]:** across all seven traced files, **exactly ONE line** — `apps/api/app/publish/revalidate.py:284` — and **none of the ten target paths**. The whole publish pipeline does not reach a single one of these guards.
- **A FINDING THE BRIEF DID NOT HAVE — there are FIVE evidence floors, not four [M]:** `apps/api/app/ingest/reference.py:112` carries the identical message and `JsonOfferAdapter` is a real concrete `SourceAdapter`. Discovery over the tree finds exactly **5** concrete adapters. **And the fifth is worse, not better:** it *was* executed — by `test_ingest_contract.py::test_reference_adapter_validate_flags_missing_fields`, which builds a `CandidateFacts` with the default `evidence=()` and then asserts only the two missing-field messages. **Executed and unasserted**: a coverage report called it green while deleting the guard would have failed no test. Four guards were invisible to coverage; the fifth was invisible *despite* coverage. That is why this slice asserts behaviour, not execution.
- **NO DRIFT [M]:** the brief warned every line-number hint had "probably drifted". Re-located all ten independently: **zero drift**. All ten land exactly where hinted (T10 at 476, within the hinted 475–476).
- **Work completed — ONE parameterised sweep, not five tests:** `tests/unit/test_adapter_evidence_floor.py` (NEW). The defect being repaired *is* drift between copies, so five separate tests would have recreated the structure that let it rot. One test, parameterised over a list **discovered by walking `apps/api/app/ingest/**` and collecting concrete `SourceAdapter` subclasses** (never typed out, never copied from the brief), keyed by imported class so a typo is an ImportError. A companion floor asserts the discovered set equals the swept set, plus vacuity floors (≥15 modules walked, ≥5 adapters) so a broken walk cannot pass by finding nothing. A third test compares all five messages **against each other**, which is the only way copy-drift is visible.
- **Every test carries a paired positive control [M].** Evidence floor: the identical candidate WITH evidence must be accepted with **no** problems, and the delta between the arms must be exactly the floor message. Z0 gate: a clean "No" cell must still reach **Z0_TRUE_FREE** and a clean "Yes" must still be caught as **Z1_BILLING_EXPOSURE**, so "ambiguous is not Z0" cannot pass against an engine that never says Z0. `revalidate_quotas`: the well-formed sibling metric must still produce its row. Matrix: the same table without the defect must still extract identically.
- **What `None` MUST mean on the material Z0 gate — decided and written down:** UNKNOWN, never "no card required" (AGENTS.md: *a failed or unknown material Z0 condition prevents Z0 classification*). The engine already honoured the tri-state; what no test covered was the **bridge** — that an ambiguous cell actually *arrives* as `None` rather than being coerced to `False` somewhere between the table cell and `OfferFacts`. `tests/unit/test_z0_gate_ambiguous_cell.py` (NEW) drives the whole chain over six realistic wordings ("Depends on your plan", "Contact sales", …) on both material fields, asserts `is None` **and** `is not False`, and asserts the resulting class is UNKNOWN with the condition named. A blank-cell arm covers the second, independent route to the same UNKNOWN.
- **Publication path:** `tests/unit/test_publish_revalidate.py` (EDIT) — the None-fact and blank-fact skips, asserted against the artefact a reader would see: **no quota row whose raw text is the literal string `"None"`**, which is what one blank table cell would otherwise publish. Includes a **negative control** that `"0"` is real data and survives — a skip written as `if not value` would silently drop a documented zero limit.
- **Ordinary degenerate markup:** `tests/unit/test_adapter_html_matrix.py` (EDIT) — a blank spacer row is skipped (including one *narrower* than the header, proving the skip precedes the width check), and a non-numeric `colspan` yields span **0** so the table is refused rather than silently realigned. Three-armed span assertion (absent → 1, valid → the integer, otherwise → 0) so neither an always-0 nor an always-1 implementation can pass.
- **MUTATION CONTROLS — the tests were proven able to FAIL, and to fail SPECIFICALLY [M]:** removing the guard from each of the five adapters in turn failed **exactly that adapter's** parameterised case and no other adapter's (5/5); rewording one adapter's message failed the cross-adapter wording test; planting a **sixth** adapter failed the discovery floor. **NEGATIVE CONTROL:** an unrelated no-op edit to the same file left the sweep **green**, so "any edit fails" cannot masquerade as specificity. Tree verified clean afterwards by `git status --porcelain apps/`.
- **POST-FIX RE-MEASUREMENT [M]:** all **ten** target lines now report **HIT**, in all three arms (DB settrace, DB monitoring, no-DB settrace), with the floor and all nine positive controls still passing and the two instruments again **byte-identical**. All ten are hit in the **no-DB** arm too, so the new tests are genuinely offline. Suite: **2910 passed, 3 skipped, exit 0** with a database (**+51 tests**); **2652 passed, 261 skipped, exit 0** without one. DB-gated population still 258 (2910−2652; 261−3), confirming no new test is DB-gated.
- **ERRORS I MADE, and what caught them [M]:**
  - **E1 — I predicted `reference.py:112` was NOT executed. It IS.** Caught by the trace, because I had written the prediction down *before* running it. Had I not recorded it, I would have quietly reported "five never-executed floors" and been wrong about a fifth of the finding. The corrected claim is materially different and worse (executed-but-unasserted), and it changed the fix from "assert execution" to "assert behaviour".
  - **E2 — I predicted the DB-gated population was 257,** carrying the previous slice's contract figure forward as if it were current. **Measured: 258.** The brief's "roughly 258" was right and I was wrong. I derived 258 from my own two subtractions rather than adopting the brief's number, which is the only reason this is a measurement and not an anchor.
  - **E3 — `Out-File -Append` injected a stray CR** into `test_adapter_html_matrix.py`, and the `create` tool wrote CRLF into three other files. Caught by a raw byte scan for `\r` (the tests passed and `ruff check` passed regardless — only `ruff format --check` and the byte scan saw it). All files normalised to LF.
- **Protected state [M]:** `agent-state/feature_list.json` **UNTOUCHED**, byte-identical to main by blob hash `154de1fef2ba…`; no feature flag flipped. `agent-state/progress.md` **append-only**, proven by raw byte-prefix comparison against `git show main:agent-state/progress.md` **plus a flipped-byte negative control and a truncation control that both had to come out the other way**. `.github/workflows/ci.yml`, `scripts/check_urls.py`, `tests/unit/test_url_allowlist.py`, `.secrets.baseline` and both lockfiles untouched. `apps/api/app/read_api/*` and `apps/web/*` untouched — a parallel slice is in flight on `stsyg-slice2-catalogue-currency`. **No `npm install`/`i`/`update`/`audit fix` was run; no node command was run at all.**
- **Gates:** `ruff check .` exit 0; `ruff format --check .` 236 files exit 0; `python scripts/check_urls.py` **1260 URL(s), 57 distinct host(s)** exit 0; `pytest -q` exit 0 both with and without `DATABASE_URL`.
- **NOT VERIFIED, stated as prominently as the verdict [M]:** (1) **Coverage is measured at LINE granularity, not branch.** A line reported HIT proves the statement ran, not that every condition reaching it was exercised; the behavioural assertions and the mutation controls are what carry that weight, not the tracer. (2) The trace covers the **seven files named in the contract only** — this slice makes **no claim** about untested paths anywhere else in the tree, and the same defect class very likely exists elsewhere. (3) The `postgres:16-alpine` database was started **empty** and each integration module migrates itself; a database with pre-existing divergent state was not tested. (4) The mutation controls were run against the **sweep file only**, not the whole suite, so I have not measured whether removing a guard also breaks unrelated tests. (5) No CI run has executed these tests yet — every figure above is local.
- **Scratch artefacts to close, NONE merged:** none. No scratch branches and no scratch PRs were created for this slice; all measurement ran locally against a throwaway container `ftatlas-trace-pg` (removed) and scratch files kept outside the repository in the session artifacts directory.
- **Commit/PR:** slice branch `stsyg-evidence-floor-z0-gate-coverage`, DRAFT PR, base `main` at `ba598a9`.
...
### 2026-08-26 addendum — CI has now run (retires "NOT VERIFIED" item 1)

- **All six checks PASS on draft PR #99** at commit `269d537` (run `33005758045`): `Python lint, format, tests`, `Node format and lint`, `Web type-check, tests, and build`, `Dependency audit`, `Secret scan`, `GitGuardian Security Checks`.
- **CI's Pytest step reports `2953 passed, 3 skipped`** — an **exact match** with my local final figure. CI runs **with `DATABASE_URL` set**, so `tests/integration/**` (including the three arms added here) executed there as well as locally.
- **CI's skip list is identical to base**: `test_stack_health.py` x2 (`ATLAS_STACK_BASE_URL` unset) plus the pre-existing `cloudflare-pages-pricing` capture gap. **The new tests added no new skips** — worth checking, because adding required schema fields to shared fixtures could quietly have converted a test into a skip, and it did not.
...
## F008 slice — a permanent guard was reading a per-slice scratchpad

**Branch** `stsyg-decouple-clock-guard-from-contract` — **base** `8462d0d0ddb86805f8a9d4bd1ebc2dab6389f74f` — BUILDER, DRAFT PR, NOT MERGED, no feature marked passing.

### The defect, and whose it was

`tests/unit/test_clock_inheritance.py::test_the_participant_list_matches_the_contract` read
`classification.PARTICIPANT_converted` out of `agent-state/current_contract.json`. That file
describes the slice **currently in flight** and every builder is told to overwrite it wholesale.
A permanent guard was therefore resting on a single-slot scratchpad. This is an **orchestration
design error**, not a previous builder's mistake: the single-slot rule and the guard are each
correct alone and lethal together. Two consecutive builders (PR #102, PR #103) had already paid
the tax by spotting the trap while planning and carrying the block forward verbatim; the third
would eventually not, and the failure would surface as a unit-test error with no code change to
explain it.

### Census — my own measurement, with the population stated

Generated by `git grep -l current_contract` at the base commit.

| Class | Count | Population |
|---|---|---|
| Tracked files mentioning `current_contract` | **21** | whole repository |
| Readers of its **CONTENTS** | **1** | `tests/unit/test_clock_inheritance.py:410-411` |
| Coupled tests, proven by negative control | **1** | out of **2998** collected |
| Non-test artifacts requiring it to EXIST + PARSE | **5** | `scripts/{init,smoke}.{ps1,sh}` + `tests/test_repo_baseline.py` |

**Where mine differs from the brief:** the brief censused *test* files only and found three — correct
for tests. But **five non-test artifacts also require this file**. None reads a key, so the design
answer is unchanged, but it is a real constraint the brief did not state: the file must keep
**existing** and keep **parsing**. A remedy that deleted or emptied it would have broken five things.

**Corroboration — the hazard has ALREADY landed elsewhere.** `apps/api/app/ingest/config_sync.py:714`,
a **production** docstring, cites *"AMENDMENT 8 in `agent-state/current_contract.json`"*. The current
contract contains **zero** occurrences of `AMENDMENT`. That pointer is already dangling. The disease is
not hypothetical and is not confined to the one test. **Reported, not fixed** — production file,
separate finding.

### Design chosen: (a), source-anchored — and what was rejected

The pinned data leaves the volatile file entirely. The durable oracle is **not a second hand-written
list** but **the source itself**: a new test derives every clock-bearing function in `_AUDITED_MODULES`
from the AST and requires each to be classified into exactly one bucket — `PARTICIPANTS`,
`_INTERNAL_CLOCK_REQUIRERS` (new), or `_DEFERRED_INVENTORS` (existing).

*Why not a plain relocated list.* Two hand-written lists in one file are edited in one breath — **weaker**
than the cross-artifact check it replaced. Relocation alone satisfies the letter and loses the bite.

*Why (c) was rejected.* Splitting the contract into per-slice and durable halves needs a **new permanent
test reading the same volatile file**, taking the coupled count from one to two — it *manages* the
coupling instead of removing it. And such a test can only assert **presence**, which a builder satisfies
by pasting a stale block: precisely the drift the guard exists to catch. "Overwrite this file except for
the parts you must not" **is** the invisible coupling that caused the incident.

*(b) skip-when-absent* was ruled out by the orchestrator and was not proposed.

The precedent was already here: `_DEFERRED_INVENTORS` is a durable, test-owned ledger checked in **both**
directions against source. The remedy follows a pattern this file already trusts.

### Evidence — mutation, not coverage

Every mutation was **proven on disk** (exact search text counted before, absent/replaced after,
`git diff --numstat` non-empty) and every restoration verified by `git hash-object` against
`git rev-parse HEAD:<path>`. Every run enforced a **collection floor**; pytest exit 5 is treated as
nothing-collected.

| # | Mutation | Result | Collected |
|---|---|---|---|
| Baseline | none | 2709 passed / 289 skipped, exit 0 | **2998** |
| NC | `classification` block deleted at base | **1 failed** — `test_the_participant_list_matches_the_contract` | 2998 |
| M1a | participant dropped from pinned list | **KILL** — "classified nowhere" | 22 |
| **M1b** | **dropped *and* re-optionalised as a RELOCATOR** | **KILL** — "OPTIONAL … not a documented deferral" | 22 |
| M1b (full suite) | same | **1 failed / 2707 passed** | **2997** |
| M2b | non-participant added to pinned list | **KILL** — "module is not audited" | 1 (floor 1) |
| Control | mutations reverted | **GREEN 23/23** | 23 |

**M1b is the load-bearing one.** It restores the *original defect* — the RELOCATOR shape that forwards
`None` onward with no `or` fallback — rather than breaking arity. Nothing raises `TypeError`; no argument
is deleted. Across the **full suite** that defect produces **exactly one failure in 2997 collected**, and
that one failure is the new guard. Every other clock test stays green, confirming the fallback-shape guard
genuinely cannot see a relocator, and that this single test is what stands between the codebase and the
defect — the same role the contract-coupled guard held, now anchored to source.

### The builder tax is gone — demonstrated, not asserted

This slice's contract was written **wholesale with no `classification` block at all**, and the suite is
green (43/43 across the affected modules). At base that same contract would have failed. **Builders no
longer need to carry anything forward.** One residual constraint remains and is unrelated to this defect:
`current_contract.json` must still *exist* and be *valid JSON*, which any contract satisfies.

### Errors I made, and how they were caught

1. **Wrong mutation anchor count.** I asserted `PARTICIPANT_converted` occurred once; it occurs **twice**
   (key plus a prose mention). The harness **aborted** rather than measuring an unapplied mutation. I
   re-anchored on the structural key `"PARTICIPANT_converted":` (count 1). This is exactly the failure mode
   the pre-count guard exists for.
2. **A real gap found by mutation, not by reasoning.** M2 was killed by a *pre-existing* guard, not mine.
   Investigating why revealed that a participant pinned from a module **outside** `_AUDITED_MODULES` is
   invisible to the closure loop, which only visits audited modules. I added an explicit scope assertion and
   re-ran with `-k` (floor 1) to prove the new guard fires **independently**. Had I accepted the first kill,
   I would have shipped a guard with a hole and called it verified.
3. **Predicted and falsified as recorded.** P5 (LOW confidence) predicted a pure source derivation would
   *not* reproduce the seven participants. It yields **11**. That is why the remedy is closure-into-buckets
   rather than equality-with-a-derived-set.

### Files changed

- `tests/unit/test_clock_inheritance.py` — guard decoupled and strengthened; `json` import dropped; a stale
  docstring cross-reference to a test name that never existed corrected.
- `agent-state/current_contract.json` — restructured (the subject of the slice).
- `agent-state/progress.md` — this entry (append-only).

No production file modified. No database was needed or created, so none was removed.

### NOT VERIFIED — stated as prominently as the verdict

- **289 integration tests SKIPPED throughout.** No `DATABASE_URL` was used. This slice has **no evidence**
  about database-backed behaviour. It changes no production code, so the risk is judged low — but it is
  **unmeasured, not measured-safe**.
- **The dangling `AMENDMENT 8` pointer in `apps/api/app/ingest/config_sync.py` is reported and NOT fixed.**
- **I did not survey whether other permanent tests read other per-slice `agent-state` files** (`evaluation.json`
  in particular). The census covered `current_contract.json` only, which is what the brief scoped.
- **CI had not run at the time of writing**; the local suite is the only signal.

### Recommended next action

Correct the standing builder instruction: the "carry the `classification` block forward" exception is no
longer needed. Consider a separate slice for the dangling `AMENDMENT 8` reference, and a census of
`evaluation.json` for the same coupling shape.

---

## 2026-08-31 — F008 bucket stale flag: assess it on the latest version, not every ancestor

**Feature:** F008 · **Slice:** `bucket-stale-flag-latest-version` · **Branch:**
`stsyg-bucket-stale-flag-latest-version` · **Base:** `ab3dfc00bcb9fe66685d1da0c0e272943bc0b79a`
(asserted with `git rev-parse HEAD`; remote `main` was the same SHA, so nothing was owed a rebase)

### THE MEASUREMENT FIRST: this defect was LATENT, not live

**No real provider bucket's `derived_state` differs today.** Measured before a single production line
was edited, and stated before anything else because the whole slice was gated on it.

| Measurement | Result | Population |
|---|---|---|
| M1 version census, real publish of the committed corpus | **0 offers with ≥2 versions**; distribution `{1 version: 6 offers}` | 7 provider configs → 6 published offers, 6 `offer_version` rows |
| M2 bucket diff, current rule vs latest-version rule | **0 differences** | **105 bucket-observations** = 5 buckets × 21 clocks |
| M2 harness cross-check (transcribed vs shipped function) | **0 mismatches** | same 105 |

The mechanism is `publisher.py:377`: a re-publish whose `content_hash` is unchanged returns
`version_created=False` and **reuses** the prior version. A single pass over fixed committed fixtures
therefore cannot mint a version 2, and where every offer has exactly one version the two rules are
identically equal at every possible clock.

**The vacuity guard did not fire and was not needed** — 6 published offers > 0, so "no difference
found" is a real negative and not an empty-catalogue artefact. It was written first precisely so that
a zero result could be distinguished from a vacuous one.

The clock sweep was not a sample. It is every distinct `fetched_at + window` boundary in the corpus,
each at −1s / +0s / +1s, plus far-past, far-future and real-now. Those boundaries are the only moments
at which any verdict can change, so 21 clocks is the complete space of behaviourally distinct moments.

**Blast radius is non-zero, which is why this is still worth fixing.** Under the current rule the
corpus produces `verified_free` in 54 of those 105 observations and `stale` in 51. A bucket that
reaches `verified_free` is exactly a bucket that one expired ancestor could take away.

**It is reachable, and reaching it reproduces the defect.** Separately and *explicitly labelled as a
constructed scenario* (M3, own database, own section — it cannot and does not speak to present-day
incidence):

- **P3 confirmed directly, not inferred:** re-publishing the corpus with **unchanged** fixtures gave
  `published=0` and left the version count untouched — the content-hash short-circuit, executed.
- **Reachability:** re-publishing with one quota figure changed created `version_number = 2` through
  the real publisher.
- **Manifestation:** with the ancestor's evidence aged 400 days and the latest version's evidence
  current, `cloudflare/serverless-functions` derived **`stale` under the current rule and
  `verified_free` under the latest-version rule** — direction **WRONGLY WITHHELD**.

### My counts differ from the brief's, and the difference matters

The brief put `latest_version(` at "9 times elsewhere" in `service.py`. **By AST — not string match,
because a match can be a docstring, a comment or a quotation of the thing being retracted — there are
9 call sites in total and one of them is line 677, inside `serialize_category_matrix` itself. So
8 elsewhere, not 9.** Population: 206 `Call` nodes in the file.

That off-by-one strengthens the case. The function already computes `latest` (677) and `latest_id`
(678) and appends `latest_id` to `bucket_versions` (679) — and then line 693 ignored it.

**A second inconsistency the brief did not name.** `evidence_currency` (line 726) rolls up
`bucket_versions`, which holds **latest ids only**, while `has_stale_evidence` scanned **every** id.
One cell of one JSON response could therefore report `evidence_currency` current **and**
`derived_state` stale simultaneously. That is one row contradicting itself, which is a sharper
demonstration than disagreement with eight call sites elsewhere, and it now has its own guard.

### The change

`apps/api/app/read_api/service.py`, one expression:
...
**One rule, not two.** `CoverageSignals.has_stale_evidence` already documents itself as *"At least one
source **backing this pair** is past its refresh window"* — and a superseded version does not back the
pair. The fix makes the code mean what it already said.

**On the conservatism defence, which I considered and reject.**
`fetch_stale_offer_version_ids` calls itself deliberately conservative because overstating staleness
"only ever makes the catalogue admit uncertainty". Conservatism is about uncertainty regarding *the
current claim*. A superseded version carries none: it is evidence about a claim that has been
**replaced**. Importing it is a category error wearing caution's clothes. It is also not conservative
in *effect* — because `derive_coverage_state` returns `stale` before `verified_free`, that "caution"
withheld the free badge from **every offer in the bucket**, and `coverage.py` has already ruled on
that direction: *"Suppressing a true free claim is a defect too."*

**Scope discipline — two axes, only one changed.** Across **offers**, one offer whose *latest* version
is stale still marks the whole cell; that is what bucket-wide means and it is untouched (guarded).
Across **versions of one offer**, an ancestor no longer speaks for the current claim; that was the
defect. The S7 docstring conflated them and asserted the coarse rule was "deliberately not touched" —
that sentence became false, so rewriting it was part of the change, not tidying.

`fetch_stale_offer_version_ids` itself is deliberately **not** narrowed: other surfaces rely on its
documented semantics and an integration test transcribes them, so changing it is a different slice.

### Verification

| Run | Collected | Result |
|---|---|---|
| Baseline at `ab3dfc00`, clean DB | **2998** | 2995 passed, 3 skipped, exit 0 |
| HEAD, clean DB | **3005** | 3002 passed, 3 skipped, exit 0 |

Delta **+7 collected / +7 passed = exactly the 7 tests added** (4 unit, 3 integration). The baseline is
recorded as *executed*, not merely exit 0: 2995 tests ran and only 3 skipped, so the integration layer
genuinely exercised a database.

**Mutations — both restore behaviour, neither breaks arity.** `M-orig` is byte-identical to the
expression at `ab3dfc00` (verified against `git show`), so "M-orig killed" is precisely "these guards
fail at base".

| Mutation | Restores | Verdict | Killed by (full suite, 3005 collected) |
|---|---|---|---|
| **M-orig** `or any(v.id in stale_versions for v in offer.versions)` | the original defect | **KILLED**, 3 failed | `test_an_expired_ancestor_does_not_withhold_the_bucket_badge`, `test_a_superseded_stale_version_does_not_withhold_the_bucket_badge`, `test_a_cell_cannot_report_current_evidence_and_a_stale_state_at_once` — **all three new; zero pre-existing** |
| **M-over** `or False` | over-correction into never-stale | **KILLED**, 5 failed | 3 new (`test_an_expired_latest_version_still_marks_the_bucket_stale`, `test_a_stale_latest_version_still_marks_the_bucket_stale`, `test_one_offer_whose_latest_is_stale_still_marks_the_whole_bucket`) **+ 2 PRE-EXISTING** (`test_coverage_review_queue.py::test_a_stale_snapshot_unseats_a_published_free_claim`, `::test_the_public_api_refuses_to_serve_an_expired_free_claim`) |
| Control | mutations reverted | **GREEN** | 3002 passed / 3005 collected, exit 0 |

Under the full original defect the entire suite yields **exactly 3 failures in 3005 collected**, and
all three are the new guards. Those three tests are what stands between this codebase and the defect.

**M-over is also caught by two pre-existing guards, so its kill was NOT accepted at face value.**
Re-run in isolation with a collected floor, each new arm fires on its own: the integration module alone
(3 collected) kills M-orig via the permit arm and M-over via the withhold arm; the unit module alone
(50 collected) kills M-orig via 2 arms and M-over via 2 arms. The withhold arm is therefore
load-bearing by measurement, not by assertion — which matters because it passes at base and would
otherwise be decoration.

Every mutation run asserted its anchor was uniquely present before substitution (absent ⇒ abort),
compiled the mutant to prove it was valid Python rather than an arity break, and treated exit 5 /
`collected == 0` as *nothing collected* rather than as survival.

`ruff check` and `ruff format --check`: both clean (240 files).

### Errors I made, and how they were caught

1. **I polluted the database the test suite reads, and it produced a false KILL.** The M1 census
   *committed* the published corpus into the same `atlas` database the integration suite uses. The
   first full-suite mutation run then reported M-orig "KILLED — 48 failed" and M-over "KILLED — 51
   failed", with failures strewn across `test_publish_pipeline`, `test_ingest_config_sync` and
   `test_domain_migration` — none of which touch this flag. **The control run caught it: with both
   mutations reverted the suite still failed, 46 failed / exit 1.** A control that fails is the only
   reason those numbers were not written down as a result. Remedied with a dedicated `suite` database
   whose emptiness is *asserted* (`provider=0 offer=0 offer_version=0`) before use, on which the
   control is green. **Had I skipped the control I would have reported a 48-test kill that was ~96%
   noise** — the exact false-KILL failure this project has already paid for once.
2. **I edited the test tree in the middle of a measurement.** The integration module was created
   between M-orig and M-over of that same run, so the two arms collected 3002 and 3005 items and were
   never comparable. Caught by reading the collected counts rather than only the verdicts. The whole
   run was discarded and redone against a frozen tree.
3. **The brief's call-site count was off by one and I did not inherit it.** 8 elsewhere, not 9.
4. **A botched edit sequence briefly left imports duplicated** in the new integration module (hoisted
   to module scope, then re-added function-locally). Caught by `ruff` F811/F401, not by review.

### Files changed

- `apps/api/app/read_api/service.py` — the flag expression, and the S7 docstring paragraph that the
  change falsified, rewritten to draw the across-offers / across-versions distinction.
- `tests/unit/test_read_api.py` — 4 arms: permit, withhold, the untouched across-offers axis, and the
  cell-self-contradiction guard.
- `tests/integration/test_coverage_bucket_staleness.py` — new; 3 arms on offer versions the **real
  publisher** created via a second publish of changed content, since no committed fixture is
  multi-version.
- `agent-state/current_contract.json` — taken wholesale (single slot), predictions frozen before
  measurement.
- `agent-state/progress.md` — this entry (append-only).

`agent-state/feature_list.json` untouched. **No feature marked passing.** Level 2 evaluation required
and pending: this is a public coverage badge (`apps/web/src/catalogue/CategoryMatrix.tsx:174,185`).

### Predictions, recorded before measurement and scored after

P1 (HIGH) zero multi-version offers — **held**. P2 (HIGH) zero differing buckets — **held**.
P3 (MEDIUM) unchanged re-publish creates no version — **held, and executed rather than inferred**.
P4 (MEDIUM) changed content creates version 2 and reproduces the defect — **held**.
P5 (LOW) at least one bucket reaches `verified_free`, so blast radius is non-zero — **held** (54 of
105). P6 (MEDIUM) the defect mutation is killed only by new tests — **held for M-orig (3 of 3 new);
partially falsified for M-over**, which two pre-existing review-queue guards also catch. That
falsification is why the isolation re-runs above exist.

### NOT VERIFIED — stated as prominently as the verdict

- **CI has not run.** The local suite on one PostgreSQL 16 container is the only signal. An absent CI
  signal reads exactly like a green one and must not be treated as one.
- **The corpus's published surface is small: 6 offers, 5 buckets, 2 providers** (only cloudflare and
  github published; aws, azure, gcp, oracle and vercel published nothing under the current gate). The
  "no bucket differs today" claim is therefore true of a **small** catalogue. It is a real negative,
  not a large one, and a bigger corpus could contain a multi-version offer this measurement never saw.
- **Nothing here measures production data.** The measurement is of the committed corpus published
  locally. If a deployed instance has been ingesting over time it *will* have multi-version offers,
  and for it this defect was live, not latent. I could not check that and did not.
- **The ageing step in M3 and in the integration arms is constructed** (a direct `UPDATE` on
  `snapshot.fetched_at`). The version rows are real publisher output; the elapsed time is simulated.
- **`fetch_stale_offer_version_ids` still returns ancestors** and is unchanged. Other callers of it
  were **not** audited for the same category error. That is a real follow-up, not a closed question.
- **No browser/UI verification.** `CategoryMatrix.tsx` was read to confirm `derived_state` reaches the
  page; the rendered badge was not exercised. `apps/web` tests were not run (`npm ci` not invoked).
