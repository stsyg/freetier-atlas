...
## 2026-08-14 - Evaluator (fresh-context Level 2) - F008 P4 slice: AWS provider profiles - PASSED

- **Subject:** PR #70, draft, branch `stsyg-ingest-aws-provider-profiles`, head `9c3d3ffbaf4325ebac922ba7940a0db51f2da9a9`, base `main` = `80c168490ea334c61ca7944c3469045a83d80bb7`. Both SHAs re-derived with `git ls-remote` BEFORE starting and again at the end; **neither moved**, so nothing was folded in.
- **Disposition: PASSED.** Zero blocking issues. All ten builder claims independently re-derived and matched. Recorded in `agent-state/evaluation.json` (entry 5). `agent-state/feature_list.json` NOT touched; F008 stays `passes:false`.
- **Independence [M]:** every material claim was re-derived with evaluator-authored probes driving the repository's OWN `_DocumentCollector`, `HtmlDocAdapter`, `resolve_profile`, `revalidate_quotas` and `classify`. The PR's own tests were used only as mutation TARGETS - to observe whether they fail when the thing they guard is broken - never as the source of a pass.
- **THE HEADLINE RESULT IS GENUINE [M]:** AWS Step Functions is evidenced `always_free` by a verbatim block of its own AND is still `Z1_BILLING_EXPOSURE`. Both quotes are EXACT whole-block matches occurring exactly once with zero partial/superset matches, so neither is composed or paraphrased. Verdicts re-derived through the real engine: AWS Free Tier **UNKNOWN**, free plan **Z1**, 12 Month **UNKNOWN**, DynamoDB **UNKNOWN**, API Gateway **Z1**, Step Functions **Z1**. **No AWS offer reaches Z0.**
- **THE ZERO IS A MEASUREMENT, PROVEN BY MUTATION [M]:** patching `classify/engine.py` gate 6 to return `UNKNOWN` instead of `Z0_TRUE_FREE` made `test_the_z0_sweep_is_not_vacuous` **FAIL** (`assert 'UNKNOWN' == 'Z0_TRUE_FREE'`) while `test_no_aws_offer_can_reach_z0` **still PASSED**. That proves both halves at once: the sweep alone is vacuous, and the builder's positive control is the load-bearing thing that rescues it. **The control must never be deleted as redundant.**
- **Z1 follows from the EVIDENCE, not a hand-set field [M]:** six further evaluator mutations, each restored in a `finally` block and verified BY BLOB HASH. Deleting the "charged per state transition" block -> `assertion_not_found`; rewording "indefinitely" -> `assertion_not_found`; rewording the card sentence to drop "free plan" -> `assertion_not_found`; duplicating a pinned block -> `ambiguous_assertion`; disabling the engine's card gate -> 3 failed; disabling its `automatic_billing` gate -> 1 failed. **FALSE-POSITIVE CONTROL:** adding an unrelated `<p>` produced `error=None` and `offer_type='always_free'`, so the guards DISCRIMINATE rather than rejecting any edit.
- **`requires_card=True` is a quotation [M]:** the pinned block contains both "free plan" and "paid plan", so it is not a paid-plan sentence generalised. Adversarial sweep of all six profiles found **ZERO** `requires_card=False` / `has_paid_dependencies=False` assertions, columns or matrix rows. No AWS profile claims a card is NOT required.
- **Reconciliation re-measured [M]:** distinct pins / capture rows recomputed from the committed fixtures: 3, 6/6; 8, 0/0; 5, 0/0; 9, 0/0; 3, 0/0; 4, 0/0 - all six match. Every pinned block occurs EXACTLY ONCE in its fixture. Each `asserted_block_sha256` array recomputed from the profile texts matched byte-for-byte, and each `sha256_stored` matched the SHA-256 of the committed `source.html`. Only `aws_free_tier_plan` reads a table: 1 table, row 0 `in_thead=True`, headers `Benefits`/`Free plan`/`Paid plan`, 6 body rows. The other five fixtures parse to **ZERO** `<table>` elements.
- **Tests reproduced EXACTLY [M]:** dedicated `postgres:16-alpine` on an isolated port, fresh venv. WITH `DATABASE_URL`: **2089 passed, 2 skipped**. WITHOUT: **1864 passed, 227 skipped**. Delta 225, matching the claim. AWS files re-run separately: 67 unit, 19 coverage-config, 7 integration - the integration tests RAN, they did not skip.
- **Protected state verified by BLOB/TREE HASH, never numstat [M]:** `html.py` `f1a68632326e...`, `fetch.py`, `docker-compose.yml`, `test_secrets_baseline.py`, `classify/` tree `3d9ec530d62e...`, `apps/web/` tree `5e5024079d4a...`, `.github/` tree `76093b10ce46...` - all IDENTICAL to base. `feature_list.json` blob `154de1fef2ba...` identical, **F008 `passes:false`**. `progress.md` is a strict byte-prefix append, 564898 -> 578943, final byte `0x0A`. Final sweep: 37 fixture/protected files checked by blob hash, **0 mismatches**.
- **Secrets baseline scrutinised, CLEAN [M]:** 31 -> 42 files, 107 -> 156 entries, exactly as claimed. All 11 new files are committed AWS fixture `capture.json` files; zero removed; **zero pre-existing files changed entry count**, so nothing masked an existing detection. All 49 new entries are `Hex High Entropy String` **SHA-256 digests** (`sha256_stored`, `sha256_original`, `asserted_block_sha256`). The "it is just fixtures" explanation was NOT taken on its word: all six `sha256_stored` values were recomputed from the committed bytes and matched. 4107 added diff lines scanned against 11 credential-shape patterns -> **zero matches**; zero internal package-feed hostname leaks.
- **CI baseline re-derived, not trusted [M]:** `main` at `80c1684` fails exactly ONE job, `Dependency audit` (GHSA-2v37-7h3g-55p8, `nanoid <3.3.18`, high). PR #70 fails the SAME one job and passes the other three. Unattributable: `apps/web` is byte-identical to base, so the PR cannot have caused or worsened it.
- **Four disclosed errors: all four corrections verified landed.** The Always Free omission produced the genuine sixth source; the duplicate count is FOUR document-scope blocks and the capture lists exactly those four; the secrets refresh is 31->42 / 107->156 as stated. The `$LASTEXITCODE` item is a process disclosure and is corroborated only indirectly - see the evaluator error below, which shows the warning is well-founded.
- **Findings the builder did NOT disclose (all informational, none blocking):** (1) `capture.json` uses two different counting conventions in one file - `asserted_block_sha256` is per-ASSERTION (Step Functions: 7 entries for 4 distinct blocks) while `live_reconciliation` counts DISTINCT blocks; both are internally consistent and the reconciliation figure is the correct one, but the array length invites mis-reading as the pin count. (2) `test_no_aws_offer_can_reach_z0` is vacuous in isolation, demonstrated directly - the control is what makes it mean anything. (3) No new instance of the known project-level gaps was introduced.
- **What I GOT WRONG myself, recorded [M]:** (a) I counted FIVE duplicated Terms blocks and briefly treated the builder's "four" as an under-count. **Wrong - I ignored assertion SCOPE.** "AWS Free Tier Terms" appears once as `title` and once as `heading`, different scopes that cannot collide. Four is correct; the builder was right. (b) My first PowerShell mutation script aborted on an argument-quoting bug BEFORE its restore line and left `classify/engine.py` PATCHED on disk. I caught it only because I had a blob-hash PRE-check, then hard-restored with `git checkout` and re-verified. This is exactly the trap the brief warned about, and it is why the rewritten battery restores in a `finally`. No mutated state reached any report or commit. (c) I read coverage provenance from `source_id` when the field is `source`, briefly producing a false "no provenance" reading; all three `offered_no_z0` rows do declare a source. (d) I assumed `tests/conftest.py` and `bs4` existed; neither does.
- **Boundary honoured:** did NOT merge, did NOT open a PR, did NOT touch `agent-state/feature_list.json`, and made no change to the branch under test. Findings written only to the evaluator's own branch.
- **Recommendation:** **PASS** - merge is justified on the evidence. F008 stays `passes:false`; the ledger flag belongs to the epic close-out, not to this increment.
