...
## 2026-08-14 - Evaluator - PR #71 Level-2: nanoid advisory GHSA-2v37-7h3g-55p8

- **Subject:** PR #71, draft, branch `stsyg-nanoid-advisory-clear`, head `9d2737255ae0e43b3a310c26204664ecb582f758`, base `80c168490ea334c61ca7944c3469045a83d80bb7`. Both SHAs re-derived with `git ls-remote` at start AND at end. During the evaluation `main` advanced to `0b3dbdecd72a8e801de6b13e1ee60e784a5d073b` (PR #70 merged); the subject head did NOT move and `merge-base(head, new main)` is still `80c1684`, so every comparison remains correctly based. I deliberately did NOT use current `main` or its refreshed secrets baseline as an oracle.
- **Disposition: PASSED.** Zero blocking issues. Fresh-context Level-2, read-only with respect to the branch under test: nothing on the PR branch was touched, no merge, no PR opened, `agent-state/feature_list.json` not modified.
- **THE CENTRAL QUESTION, and it is now CLOSED [M]:** whether the committed `sha1-` integrity pins the true public bytes. It was genuinely open, because this machine's npm registry is an **internal mirror** - I verified that by SHA-256 of the hostname rather than by printing it (local host hash `2283DBF20C50` vs public `registry.npmjs.org` hash `0DC11026AC75`, not equal) - and direct HTTPS to the public registry fails at TLS here, as did the agent web-fetch path. Every builder-side derivation therefore ran through ONE mirror, which is one measurement repeated, not independent agreement.
- **How I closed it: I used CI as a measurement instrument rather than as a gate [M].** I pushed a temporary evaluator-owned probe workflow to a throwaway branch created from the base commit, which checked out the PR head read-only on a GitHub-hosted runner with `REGISTRY=https://registry.npmjs.org/`. Probe run `31840784470`, conclusion success. Measured: `dist.shasum` for `nanoid@3.3.18` = `f66a2de1199ffde0fcf21c8a5f13106b1c081913`; the public tarball is **5713 bytes** and `sha1sum`s to that same hex; its base64 is `9mot4Rmf/eD88hyKXxMQaxwIGRM=`, **byte-identical to the committed `integrity`**. The throwaway branch has been deleted; run logs persist as durable evidence.
- **`npm ci` is the strong form, and it passed [M]:** `npm ci` verifies each package's declared integrity against the bytes actually downloaded and aborts `EINTEGRITY` on mismatch. From the COMMITTED lockfile against the REAL public registry it exited 0, added 287 packages, installed nanoid **3.3.18**, and left the lockfile byte-unchanged. That is byte-level proof of the digest, not a version check.
- **Why the positive control is non-vacuous [M]:** the 3.3.17 digest was committed to this repository *before this work began*, so reproducing it (`f1c3aa253c52547956a52c50bff754316f61037a`, matching the already-committed `sha1-8cOqJTxSVHlWpSxQv/dUMW9hA3o=`) recovers a pre-existing datum the builder did not author. The negative control 3.3.16 (`a04d8ec4...`) DIFFERS, so the method discriminates and would have failed had the derivation been wrong.
- **Green CI proves the VERSION, not the DIGEST [M]:** confirmed and recorded, because it is the error that propagated through two contexts before reaching me. `npm audit` performs no install, downloads no tarball and hashes nothing. Run `31839541333` is real evidence that the pinned version clears the advisory floor per the public advisory database; it is NOT evidence about which bytes execute.
- **Claims re-derived:** advisory reproduces on base (`GHSA-2v37-7h3g-55p8`, range `<3.3.18`, high, CVSS 5.9, **bare exit 1**) and is gone at head (`found 0 vulnerabilities`, **bare exit 0**, plain and `--audit-level=high`); diff is exactly `3 3 apps/web/package-lock.json` plus two ledger files, one hunk, only `version`/`resolved`/`integrity`; **336 resolved URLs, 336 on `registry.npmjs.org`, 0 elsewhere**, identical on base so the boundary was preserved rather than merely not obviously broken; 110/110 tests in 9 files and `tsc -b && vite build` green on the runner; zero `.py` diff; `feature_list.json` blob `154de1fe...` identical on both sides; `progress.md` a strict prefix-preserving append, +23 / -0.
- **Claim I could NOT fully re-derive, stated rather than glossed [M]:** `scripts/check.ps1 -NodeAudit` exits **1** in this worktree, not 0. Every failure is an absent local toolchain (no `.venv`; `ruff`/`pytest`/`pip-audit`/`detect-secrets-hook` off PATH; `prettier`/`eslint` need a root install never performed here), NOT a defect in the change. The two checks that ARE the subject both pass (`root tooling` and `apps/web` audits, 0 vulnerabilities), as do `Secrets baseline shape` and `URL host allowlist` (1012 URLs, 51 hosts). The remaining components are independently evidenced for this exact commit by the real CI jobs. I attribute the local exit 1 to my environment, and I record that I did not reproduce the literal exit 0.
- **Findings the builder did NOT disclose:** (1) the lockfile pins **sha1** integrity repository-wide (335 `sha1-` + 1 `sha512-`, identical on base and head) while the registry's own canonical integrity for this package is `sha512-`; npm enforces whatever the lockfile declares so the install is still verified, but sha1 is a weaker supply-chain primitive and the pattern is itself a fingerprint of mirror-mediated lockfile generation - pre-existing, worth a separate slice. (2) nanoid is **dev-only** (via postcss), so exposure is build-time, which lowers real-world severity without changing the disposition. (3) PR #71 is now `mergeable=CONFLICTING`, but `git merge-tree` puts the conflicts ONLY in `current_contract.json` and `progress.md`; **the lockfile does not conflict** and #70 changed 44 files with zero under `apps/web`.
- **Precision correction on a prohibition [M]:** `apps/web/package.json` DOES already contain an `overrides` block (`vite`, `esbuild`, `js-yaml`). It is pre-existing, byte-identical between base and head (blob `121d8a1f...`), and none of them is nanoid, so the prohibition holds **for this delta** - but the flat claim "no dependency overrides" does not describe the file's contents and should be phrased as "no override added, and none for nanoid".
- **Three things I got wrong myself, recorded [M]:** (1) I first reported the CI run as FOUR checks and flagged the five-check claim as suspect; wrong instrument - the Actions jobs list excludes external check-runs, and at check-run level `total_count=5`, all success, the fifth being GitGuardian. The builder was right and I was not. (2) My first internal-hostname leak scan returned `internal_host_leak=true` for the DIFF and commit message. That was a **false positive from my own matcher**; I refused to report it, disambiguated by hashing each discovered host, and established the genuine internal host appears in none of the diff, commit message or PR body - exhaustive enumeration shows the only hosts present anywhere are `cloud.google.com` and `registry.npmjs.org`. Reporting the first result would have raised a false supply-chain alarm. (3) I mutated my own working tree with `git checkout <head> -- .` to run `check.ps1`; restored and verified by **blob hash** for all three files (`6b341f42...`, `d970247e...`, `f492053a...`), never by numstat.
- **Disclosed CI gap: CONFIRMED, not grounds for failing this PR.** No CI job installs `apps/web`; the node job runs `npm ci` at the repository ROOT then Prettier and ESLint, and `apps/web` receives only `npm audit`. Corroborated from a second direction: the root `package.json` declares **no workspaces**, so a root install cannot reach `apps/web`. Its 110 tests, `tsc -b` and production build therefore never run in CI - my probe was their first CI-grade execution.
- **postcss: resolution CONFIRMED correct.** Pinned/installed postcss is 8.5.25 declaring `nanoid ^3.3.16`; registry-latest 8.5.26 declares `^3.3.17`. 3.3.18 satisfies both, and postcss is the ONLY package in the tree declaring a nanoid dependency, so no second consumer imposes a conflicting range. No manifest change needed.
- **Files changed by me:** `agent-state/evaluation.json` (appended a fifth entry) and this handoff, on my own evaluator branch only.
- **Recommended next action:** resolve the ledger-only conflict, then merge. On the evidence this clears the repository's only red CI job.
- **Boundary:** evaluator branch only. Did not merge, did not open a PR, did not touch `feature_list.json`, did not modify the PR branch.